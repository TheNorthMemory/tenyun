...
declare interface CreateTriggerRequest {
  ...
  TriggerName: string;
  /** 触发器类型，目前支持 cos 、cmq、 timer、 ckafka、apigw类型。创建函数 URL 请在此填写 http，请参考[创建函数 URL ](https://cloud.tencent.com/document/product/583/100227)。创建 cls 触发器请参考[CLS 创建投递 SCF 任务](https://cloud.tencent.com/document/product/614/61096)。 */
  Type: string;
...
declare interface DeleteTriggerRequest {
  ...
  TriggerDesc?: string;
  /** 函数的版本，默认为 $LATEST，建议填写 [$DEFAULT](https://cloud.tencent.com/document/product/583/36149)方便后续进行版本的灰度发布。如果删除的触发器类型为 APIGW 触发器,该字段为必填参数 */
  Qualifier?: string;
}
