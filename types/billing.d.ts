...
declare interface DescribeDealsByCondRequest {
  ...
  /** 第多少页，从0开始，默认是0 */
  Offset?: number;
  /** 订单状态枚举值：1： 待支付2： 已支付3： 发货中4： 已发货5： 发货失败6： 已退款7： 已取消8： 订单过期9 ： 订单已失效10： 产品已失效11： 代付拒绝12： 支付中13： 退款中30： 服务市场订单待用户确认完成默认值：4（已发货，成功的订单）其中，状态2、3、5、12、13、30 在控制台统一显示“处理中”；状态4 在控制台中显示交易成功；状态9、10 在控制台中无对应状态 */
  Status?: number;
  /** 子订单号 */
  OrderId?: string;
  /** 大订单号 */
