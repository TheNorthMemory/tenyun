...
/** 美颜配置 */
declare interface BeautyConfig {
  /** 美颜效果 */
...
declare interface EraseTimeArea {
  /** 开始时间，单位:毫秒 */
  BeginMs: number;
  /** 结束时间，单位:毫秒 */
  EndMs: number;
...
declare interface MediaInputInfo {
  /** 输入来源对象的类型，支持： COS：COS源 URL：URL源 AWS-S3：AWS 源，目前只支持转码任务 VOD：点播专业版 */
  Type: string;
  ...
  VODInputInfo?: VODInputInfo | null;
}
...
/** 智能擦除模板隐私保护配置 */
declare interface SmartErasePrivacyConfig {
  /** 隐私保护擦除方式。- blur 模糊- mosaic 马赛克 */
  PrivacyModel: string;
  /** 隐私保护目标，（在API Explorer上使用时无需传入数组，添加相应项并填入对应值即可）。- face 人脸- plate 车牌 */
...
/** 智能擦除任务 */
declare interface SmartEraseTaskInput {
  /** 智能擦除模板id。 */
  Definition?: number;
  /** 智能擦除自定义参数，当 Definition 填 0 时有效。 该参数用于高度定制场景，建议您优先使用 Definition 指定智能擦除参数。 */
  RawParameter?: RawSmartEraseParameter | null;
  ...
  /** 文件的目标存储，不填则继承上层的 OutputStorage 值。 */
  OutputStorage?: TaskOutputStorage | null;
  /** 文件的输出路径，可以为相对路径或者绝对路径。若需定义输出路径，路径需以`.{format}`结尾。变量名请参考 [文件名变量说明](https://cloud.tencent.com/document/product/862/37039)。相对路径示例：文件名_{变量名}.{format}文件名.{format}绝对路径示例：/自定义路径/文件名_{变量名}.{format}**注意**：目前不支持`BatchProcessMedia`接口。 */
  OutputObjectPath?: string;
}

/** 智能擦除任务结果 */
declare interface SmartEraseTaskResult {
  /** 任务状态，有 PROCESSING，SUCCESS 和 FAIL 三种。 */
  Status?: string;
  /** 错误码，空字符串表示成功，其他值表示失败，取值请参考 [媒体处理类错误码](https://cloud.tencent.com/document/product/862/50369) 列表。 */
  ErrCodeExt?: string;
  ...
  Message?: string;
  /** 智能擦除输入。 */
  Input?: SmartEraseTaskInput | null;
  /** 智能擦除任务输出。 */
  Output?: AiAnalysisTaskDelLogoOutput | null;
  /** 任务进度。 */
  Progress?: number;
...
/** 智能擦除模板去水印配置 */
declare interface SmartEraseWatermarkConfig {
...
/** 智能字幕全文识别结果。 */
declare interface SmartSubtitleTaskFullTextResult {
  /** 任务状态，有 PROCESSING，SUCCESS 和 FAIL 三种。 */
  Status?: string;
  /** 错误码，空字符串表示成功，其他值表示失败，取值请参考 [媒体处理类错误码](https://cloud.tencent.com/document/product/862/50369) 列表。 */
  ErrCodeExt?: string;
  ...
  Input?: SmartSubtitleTaskResultInput | null;
  /** 智能字幕任务输出信息。 */
  Output?: SmartSubtitleTaskTextResultOutput | null;
  /** 任务进度。 */
  Progress?: number | null;
}

/** 智能字幕识别片段。 */
declare interface SmartSubtitleTaskFullTextSegmentItem {
  /** 识别片段置信度。取值：0~100。 */
  Confidence?: number;
  /** 识别片段起始的偏移时间，单位：秒。 */
  StartTimeOffset?: number;
...
/** 智能字幕识别结果。 */
declare interface SmartSubtitleTaskTextResultOutput {
  /** 智能字幕识别片段列表。 */
  SegmentSet?: SmartSubtitleTaskFullTextSegmentItem[] | null;
  /** 识别字幕结果 */
  RecognizeSubtitleResult?: SubtitleResult[] | null;
  /** 翻译字幕结果 */
  TransSubtitleResult?: SubtitleResult[] | null;
  /** 字幕文件存储位置 */
  OutputStorage?: TaskOutputStorage | null;
}
...
declare interface SmartSubtitlesResult {
  /** 任务的类型，取值范围：- AsrFullTextRecognition：语音全文识别- TransTextRecognition：语音翻译- PureSubtitleTrans: 纯字幕翻译- OcrFullTextRecognition：文字提取字幕 */
  Type?: string;
  /** 语音全文识别结果，当 Type 为 AsrFullTextRecognition 时有效。 */
  ...
  /** 文字提取字幕结果，当 Type 为 OcrFullTextRecognition 时有效。 */
  OcrFullTextTask?: SmartSubtitleTaskFullTextResult | null;
}
...
declare interface TaskOutputStorage {
  /** 媒体处理输出对象存储位置的类型，支持：COS：COS存储AWS-S3：AWS 存储，只适用于AWS任务，且要求同区域 VOD：点播专业版 */
  Type: string;
  ...
  VODOutputStorage?: VODOutputStorage | null;
}
...
declare interface TimeSpotCheck {
  ...
  /** 抽检间隔，表示在一次检测结束后，等待多长时间后，再次检测。取值范围（单位 s）：- 最小值：10- 最大值：3600 */
  CheckInterval?: number;
  /** 片头跳过时长。取值范围（单位 s）：- 最小值：1- 最大值：1800 */
  SkipDuration?: number;
  /** 循环次数。取值范围:- 最小值：0- 最大值：1000取值为 0 或为空时，表示循环至视频结束。 */
...
/** 智能擦除模板去字幕配置 */
declare interface UpdateSmartEraseSubtitleConfig {
...
/** 智能擦除模板去水印配置 */
declare interface UpdateSmartEraseWatermarkConfig {
  /** 水印擦除方式。**自动擦除：**通过A模型自动识别视频中的水印，擦除后生成新的视频。适用于动态水印。当使用自动擦除时，若您不指定AutoAreas，将对视频全屏进行自动擦除；若指定AutoAreas，将改为对您指定的区域进行自动擦除。**指定区域擦除：**针对位置较固定的静态水印，建议您直接指定擦除区域。当您选择指定区域擦除时，请至少传入一个指定区域。- auto 自动擦除- custom 指定区域擦除 */
  WatermarkEraseMethod?: string;
  /** 水印擦除模型。基础版：效果一般，性价比高，适合动画或背景较干净的视频。高级版：效果更好，适合短剧等现实风格视频。- basic 基础版- advanced 高级版 */
  WatermarkModel?: string;
...
declare interface CreateAigcImageTaskRequest {
  ...
  ModelName?: string;
  /** 指定模型特定版本号。默认使用系统当前所支持的模型稳定版本。Hunyuan，可选 [3.5-preview、3.0]。Vidu，可选 [q2]。Kling，可选 [2.1、O1、3.0、3.0-Omni]。Qwen，可选 [0925]。 */
  ModelVersion?: string;
  ...
  /** 用于传入参考的资源图片信息，默认支持传入一张图片。支持多图输入的模型：Kling 2.1，可支持最多 4 张图片输入作为资源图。Kling 3.0-Omni，可支持最多 10 张图片输入作为资源图。Kling O1，可支持最多 10 张图片输入作为资源图。Vidu q2，可支持最多 7 张图片输入作为资源图。Hunyuan 3.5-preview 可最多支持5张参数图，3.0 可支持最多 3 张图片输入作为资源图。注意：推荐图片小于7M，各模型限制不同。图片格式支持：jpeg, png, webp。 */
