...
declare interface ChatCompletionsRequest {
  ...
  Model: string;
  /** 聊天上下文信息。说明：1. 长度最多为 40，按对话时间从旧到新在数组中排列。2. Message.Role 可选值：system、user、assistant。其中，system 角色可选，如存在则必须位于列表的最开始。user 和 assistant 需交替出现（一问一答），以 user 提问开始和结束，且 Content 不能为空。Role 的顺序示例：[system（可选） user assistant user assistant user ...]。3. Messages 中 Content 总长度不超过 16000 Token，超过则会截断最前面的内容，只保留尾部内容。建议不超过 4000 Token。 */
  Messages: Message[];
...
declare interface Hunyuan {
  ...
  /** hunyuan-pro {@link ChatProRequest} {@link ChatProResponse} */
  ChatPro(data: ChatProRequest, config?: AxiosRequestConfig): AxiosPromise<ChatProResponse>;
  /** hunyuan-standard {@link ChatStdRequest} {@link ChatStdResponse} */
