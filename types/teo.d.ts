...
declare interface BindSecurityTemplateToEntityRequest {
  ...
  Operate: string;
  /** 指定绑定或解绑的策略模板 ID 或站点全局策略- 如需绑定至策略模板，或从策略模板解绑，请指定策略模板 ID。- 如需绑定至站点全局策略，或从站点全局策略解绑，请使用 @ZoneLevel@domain 参数值。注意：解绑后，域名将使用独立策略，并单独计算规则配额，请确保解绑前套餐规则配额充足。 */
  TemplateId: string;
...
declare interface DescribeTopL7AnalysisDataRequest {
  ...
  /** 结束时间。 */
  EndTime: string;
  /** 查询的指标，取值有： l7Flow_outFlux_country：按国家/地区维度统计 L7 EdgeOne 响应流量指标； l7Flow_outFlux_province：按中国大陆境内省份维度统计 L7 EdgeOne 响应流量指标； l7Flow_outFlux_statusCode：按状态码维度统计 L7 EdgeOne 响应流量指标； l7Flow_outFlux_domain：按域名维度统计 L7 EdgeOne 响应流量指标； l7Flow_outFlux_url：按 URL Path 维度统计 L7 EdgeOne 响应流量指标; l7Flow_outFlux_resourceType：按资源类型维度统计 L7 EdgeOne 响应流量指标； l7Flow_outFlux_sip：按客户端 IP 维度统计 L7 EdgeOne 响应流量指标； l7Flow_outFlux_referers：按 Referer 维度统计 L7 EdgeOne 响应流量指标； l7Flow_outFlux_ua_device：按设备类型维度统计 L7 EdgeOne 响应流量指标; l7Flow_outFlux_ua_browser：按浏览器类型维度统计 L7 EdgeOne 响应流量指标； l7Flow_outFlux_us_os：按操作系统类型维度统计 L7 EdgeOne 响应流量指标； l7Flow_request_country：按国家/地区维度统计 L7 访问请求数指标； l7Flow_request_province：按中国大陆境内省份维度统计 L7 访问请求数指标； l7Flow_request_statusCode：按状态码维度统计 L7 访问请求数指标； l7Flow_request_domain：按域名维度统计 L7 访问请求数指标； l7Flow_request_url：按 URL Path 维度统计 L7 访问请求数指标; l7Flow_request_resourceType：按资源类型维度统计 L7 访问请求数指标； l7Flow_request_sip：按客户端 IP 维度统计 L7 访问请求数指标； l7Flow_request_referer：按 Referer 维度统计 L7 访问请求数指标； l7Flow_request_ua_device：按设备类型维度统计 L7 访问请求数指标; l7Flow_request_ua_browser：按浏览器类型维度统计 L7 访问请求数指标； l7Flow_request_us_os：按操作系统类型维度统计 L7 访问请求数指标。 */
  MetricName: string;
  /** 站点 ID 集合，此参数必填。 */
  ZoneIds?: string[];
...
declare interface ModifySecurityPolicyRequest {
  ...
  /** 子域名/应用名。注意：当同时指定本参数和 TemplateId 参数时，本参数不生效。请勿同时指定本参数和 TemplateId 参数。 */
  Entity?: string;
  /** 指定模板策略 ID，或指定站点全局策略。- 如需配置策略模板，请指定策略模板 ID。- 如需配置站点全局策略，请使用 @ZoneLevel@Domain 参数值注意：当使用本参数时，Entity 参数不生效。请勿同时使用本参数和 Entity 参数。 */
  TemplateId?: string;
}
