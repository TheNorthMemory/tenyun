...
/** 高级回源优化配置参数。 */
declare interface AdvancedOriginRoutingParameters {
  /** 回源优化方向，取值有：MainlandChinaAndGlobalAdaptive：自适应。域名需同时开启「智能加速」后生效，EdgeOne将根据客户端与源站的实际地理位置，自动匹配最优回源优化方向，无需手动指定。例如 www.example.com 开启「高级回源优化」，但「智能加速」关闭或仅部分匹配条件开启时，「高级回源优化」将不生效。 */
  Direction: string;
}
...
declare interface RuleEngineAction {
  ...
  /** 高级回源优化配置参数，当 Name 取值为 AdvancedOriginRouting 时，该参数必填。 */
  AdvancedOriginRoutingParameters?: AdvancedOriginRoutingParameters | null;
...
declare interface CreateAccelerationDomainRequest {
  ...
  IPv6Status?: string;
  /** 指定域名绑定的共享 CNAME 地址，不传时使用默认 CNAME。绑定共享 CNAME 要求所有域名的调度策略保持一致，以下配置将影响调度策略，在不一致时绑定共享 CNAME 将按照以下方式处理：- IPv6 访问：不允许创建域名，请修改 IPv6Status 已保持与共享 CNAME 绑定的其余域名配置一致；- DDoS 防护：如果选择的共享 CNAME 已启用 DDoS 防护，则创建域名时，将默认为该域名启用 DDoS 防护。- 中国大陆网络优化（国际加速）：不允许创建域名，请保持当前域名的中国大陆网络优化（国际加速）配置与共享 CNAME 绑定的其余域名一致后重试。注：共享 CNAME 当前仍在内测中，如需使用，请联系我们开通。 */
  SharedCNAME?: string;
}
...
declare interface CreatePurgeTaskRequest {
  /** 站点 ID。若您希望快速提交不同站点下的 Targets Url，可以将其填写为 *，但前提是调用该 API 的账号必须具备主账号下全部站点资源的权限。 */
  ZoneId: string;
  /** 节点缓存清除类型，取值有：purge_url：URL刷新；purge_prefix：目录刷新；purge_host：Hostname 刷新；purge_all：站点下全部缓存刷新（取该值时不支持 ZoneId 入参为 *）；purge_cache_tag：cache-tag 刷新。缓存清除类型详情请查看[清除缓存](https://cloud.tencent.com/document/product/1552/70759)。 */
  Type: string;
...
declare interface DescribeRealtimeLogDeliveryTasksRequest {
  ...
  /** 分页查询限制数目。默认值：20，最大值：1000。 */
  Limit?: number;
  /** 过滤条件，Filters.Values 的上限为 20。该参数不填写时，返回当前 zone-id 下所有实时日志投递任务信息。详细的过滤条件如下：task-id：按照实时日志投递任务 ID进行过滤。不支持模糊查询。task-name：按照实时日志投递任务名称进行过滤。支持模糊查询，使用模糊查询时，仅支持填写一个实时日志投递任务名称。entity-list：按照实时日志投递任务对应的实体进行过滤。不支持模糊查询。示例值：domain.example.com 或者 sid-2s69eb5wcms7。task-type：按照实时日志投递任务类型进行过滤。不支持模糊查询。可选项如下： cls: 推送到腾讯云 CLS； custom_endpoint：推送到自定义 HTTP(S) 地址； s3：推送到 AWS S3 兼容存储桶地址； log_analysis：推送到 EdgeOne 日志分析。 */
  Filters?: AdvancedFilter[];
}
...
declare interface ImportZoneConfigRequest {
  /** 站点 ID。 */
  ZoneId: string;
  /** 待导入的配置内容。要求采用 JSON 格式，按照 UTF-8 方式进行编码。配置内容可通过站点配置导出接口（ExportZoneConfig）获取。您可以单独导入「站点加速-全局加速配置」或「站点加速-规则引擎」，传入对应的字段即可，详情可以参考下方示例。注意：ExportZoneConfig 导出的 AccelerationDomain（加速域名配置）、Origin（源站配置）暂不支持通过本接口导入，若 Content 中包含上述配置内容，将导致导入失败。 */
  Content: string;
}
