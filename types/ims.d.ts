...
declare interface LabelResult {
  ...
  Suggestion?: string;
  /** 该字段用于返回检测结果所对应的恶意标签。返回值：**Normal**：正常，**Porn**：色情，**Abuse**：谩骂，**Ad**：广告；以及其他令人反感、不安全或不适宜的内容类型。 */
  Label?: string;
...
declare interface ObjectResult {
  ...
  /** 该字段用于返回针对当前恶意标签的后续操作建议。当您获取到判定结果后，返回值表示系统推荐的后续操作；建议您按照业务所需，对不同违规类型与建议值进行处理。返回值：**Block**：建议屏蔽，**Review** ：建议人工复审，**Pass**：建议通过 */
  Suggestion?: string;
  /** 该字段用于返回检测结果所对应的恶意标签，表示模型推荐的审核结果，建议您按照业务所需，对不同违规类型与建议值进行处理。返回值：**Normal**：正常，**Porn**：色情，**Abuse**：谩骂，**Ad**：广告；以及其他令人反感、不安全或不适宜的内容类型。 */
  Label?: string;
...
declare interface OcrResult {
  ...
  Suggestion?: string;
  /** 该字段用于返回OCR检测结果所对应的优先级最高的恶意标签，表示模型推荐的审核结果，建议您按照业务所需，对不同违规类型与建议值进行处理。返回值：**Normal**：正常，**Porn**：色情，**Abuse**：谩骂，**Ad**：广告；以及其他令人反感、不安全或不适宜的内容类型。 */
  Label?: string;
...
declare interface CreateImageModerationAsyncTaskRequest {
  ...
  DataId?: string;
  /** 该字段表示待检测图片文件内容的Base64编码，图片大小不超过10MB，建议分辨率不低于256x256，否则可能会影响识别效果。备注： 该字段与FileUrl必须选择输入其中一个。 */
  FileContent?: string;
  /** 该字段表示待检测图片文件的访问链接，图片支持PNG、JPG、JPEG、BMP、GIF、WEBP格式，大小不超过100MB，建议分辨率不低于256x256；图片下载时间限制为3秒，超过则会返回下载超时；由于网络安全策略，送审带重定向的链接，可能引起下载失败，请尽量避免，比如Http返回302状态码的链接，可能导致接口返回ResourceUnavailable.ImageDownloadError。备注：该字段与FileContent必须选择输入其中一个。 */
  FileUrl?: string;
...
declare interface ImageModerationRequest {
  /** 该字段表示使用的策略的具体编号，该字段需要先在内容安全控制台中配置。备注：不同Biztype关联不同的业务场景与识别能力策略，调用前请确认正确的Biztype。 */
  BizType?: string;
  /** 该字段表示您为待检测对象分配的数据ID，传入后可方便您对文件进行标识和管理。取值：由英文字母（大小写均可）、数字及四个特殊符号（_，-，@，#）组成，长度不超过64个字符。 */
  DataId?: string;
  /** 该字段表示待检测图片文件内容的Base64编码，由于云API对请求包体有大小限制，图片的Base64编码内容大小不得超过10MB。备注：该字段与FileUrl必须选择输入其中一个。 */
  FileContent?: string;
  /** 该字段表示待检测图片文件的访问链接，URL源图大小不超过30MB。备注：该字段与FileContent必须选择输入其中一个。 */
  FileUrl?: string;
  /** GIF检测专用，用于表示GIF截帧频率（每隔多少张图片抽取一帧进行检测）；默认值为0，此时只会检测GIF的第一帧或不进行切分处理。备注：Interval与MaxFrames参数需要组合使用。例如，Interval=3, MaxFrames=400，则代表在检测GIF时，将每间隔2帧检测一次且最多检测400帧。 */
  Interval?: number;
  /** GIF检测专用，用于标识最大截帧数量；默认值为1，此时只会检测输入GIF的第一帧不进行切分处理（可能会造成处理超时）。备注：Interval与MaxFrames参数需要组合使用。例如，Interval=3, MaxFrames=400，则代表在检测GIF时，将每间隔2帧检测一次且最多检测400帧。 */
  MaxFrames?: number;
  /** 该字段表示待检测对象对应的用户相关信息，若填入则可甄别相应违规风险用户。 */
  User?: User;
  /** 该字段表示待检测对象对应的设备相关信息，若填入则可甄别相应违规风险设备。 */
...
declare interface ImageModerationResponse {
  ...
  Suggestion?: string;
  /** 该字段用于返回检测结果（LabelResults）中所对应的优先级最高的恶意标签，表示模型推荐的审核结果，建议您按照业务所需，对不同违规类型与建议值进行处理。返回值标签示例：Normal：正常，Porn：色情，Abuse：谩骂，Ad：广告；（说明：文档仅示例了部分风险类型，更多返回类型请以实际值为准或提交工单进行咨询）。 */
  Label?: string;
  /** 该字段用于返回检测结果所命中优先级最高的恶意标签下的子标签名称，如：色情--性行为；若未命中任何子标签则返回空字符串。 */
  SubLabel?: string;
  /** 该字段用于返回当前标签（Label）下的置信度，取值范围：0（置信度最低）-100（置信度最高 ），越高代表图片越有可能属于当前返回的标签；如：色情 99，则表明该图片非常有可能属于色情内容；色情 0，则表明该图片不属于色情内容。 */
  Score?: number;
  /** 该字段用于返回分类检测模型的详细检测结果；表示模型推荐的审核结果，建议您按照业务所需，对不同违规类型与建议值进行处理。详细返回值信息可参阅对应的数据结构（LabelResults）描述。返回值标签示例：Normal:正常，Porn:色情，Abuse:谩骂，Ad:广告（说明：文档仅示例了部分风险类型，更多返回类型请以实际值为准或提交工单进行咨询） */
  ...
  /** 该字段用于返回基于图片风险库（风险黑库与正常白库）识别的结果,详细返回值信息可参阅对应的数据结构（LibResults）描述。备注：图片风险库目前暂不支持自定义库。 */
