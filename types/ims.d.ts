...
declare interface ImageModerationRequest {
  ...
  /** 该字段表示您为待检测对象分配的数据ID，传入后可方便您对文件进行标识和管理。取值：由英文字母（大小写均可）、数字及四个特殊符号（_，-，@，#）组成，**长度不超过64个字符**。 */
  DataId?: string;
  /** 该字段表示待检测图片文件内容的Base64编码，由于云API对请求包体有大小限制，图片的**Base64编码内容大小不得超过10MB**。备注：**该字段与FileUrl必须选择输入其中一个**。 */
  FileContent?: string;
  /** 该字段表示待检测图片文件的访问链接，URL源图**大小不超过30MB**。备注：该字段与FileContent必须选择输入其中一个。 */
  FileUrl?: string;
