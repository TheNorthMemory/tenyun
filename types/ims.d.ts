...
declare interface CreateImageModerationAsyncTaskRequest {
  ...
  DataId?: string;
  /** 该字段表示待检测图片文件内容的Base64编码，图片**大小不超过10MB**，建议**分辨率不低于256x256**，否则可能会影响识别效果。备注： **该字段与FileUrl必须选择输入其中一个**。 */
  FileContent?: string;
  /** 该字段表示待检测图片文件的访问链接，图片支持PNG、JPG、JPEG、BMP、GIF、WEBP格式，**大小不超过100MB**，建议**分辨率不低于256x256**；图片下载时间限制为3秒，超过则会返回下载超时；由于网络安全策略，**送审带重定向的链接，可能引起下载失败**，请尽量避免，比如Http返回302状态码的链接，可能导致接口返回ResourceUnavailable.ImageDownloadError。备注：**该字段与FileContent必须选择输入其中一个**。 */
  FileUrl?: string;
...
declare interface ImageModerationRequest {
  ...
  /** 该字段表示您为待检测对象分配的数据ID，传入后可方便您对文件进行标识和管理。取值：由英文字母（大小写均可）、数字及四个特殊符号（_，-，@，#）组成，**长度不超过64个字符**。 */
  DataId?: string;
  /** 该字段表示待检测图片文件内容的Base64编码，图片Base64编码内容大小不超过10MB，建议**分辨率不低于256x256**，否则可能会影响识别效果。备注： **该字段与FileUrl必须选择输入其中一个**。 */
  FileContent?: string;
  /** 该字段表示待检测图片文件的访问链接。图片支持PNG、JPG、JPEG、BMP、GIF、WEBP格式，且**大小不得超过30MB**，建议**分辨率不低于256x256**。**图片下载时间限制为3秒，超时将进行一次重试，若再次失败，则会返回下载超时**。由于网络安全策略，**送审带重定向的链接**可能引起下载失败，请尽量避免使用返回302或其他非200状态码的HTTP链接，会导致接口返回 ResourceUnavailable.ImageDownloadError 错误。备注：该字段与FileContent必须选择输入其中一个。 */
  FileUrl?: string;
