...
declare interface BusinessCertificateInfo {
  /** 识别出的名称 */
  Name?: string;
  /** 识别出的字段名称对应的值 */
  Value?: string;
...
declare interface InvoiceItem {
  ...
  /** 该发票中所有字段坐标信息。包括字段英文名称、字段值所在位置四点坐标、字段所属行号，具体内容请点击左侧链接。字段在原始图的坐标可以根据Polygon转换得出。 */
...
/** 作文批改建议 */
declare interface MarkEssaySuggestions {
  /** 作文批改序号 */
  ID?: number;
  /** 批改类型：主要包括：词汇、语句 */
  Type?: string;
  /** 子类型，基于Type返回二级类型词汇： 错别字、使用拼音、词语误用、词语冗余、词汇贫乏、多字/漏字语句：语法硬伤、逻辑问题、表达不佳、标点误用、优美句子 */
  SubType?: string;
  /** 原文内容 */
  Origin?: string;
  /** 批改后的内容 */
  ...
  /** 点评内容 */
  Message?: string;
  /** array[][]二维数组，原文内容可能存在跨行的句子，会有多组坐标返回切图区域的4个角点坐标, 是个长度为8的数组[0,1,2,3,4,5,6,7](0,1) 左上角坐标(2,3) 右上角坐标(4,5) 右下角坐标(6,7) 左下角坐标 */
  Positions?: Positions[];
}
...
/** 这是OCR在高精度识别下返回的坐标值，采用的是由一个数组表示4个顶点的坐标构成，如[0,1,2,3,4,5,6,7]- (0,1) 左上角坐标- (2,3) 右上角坐标- (4,5) 右下角坐标- (6,7) 左下角坐标 */
...
declare interface TextDetection {
  ...
  /** 语种信息。注：仅ConfigID配置为MulOCR时支持。zh:中文; en:英文; tha:泰语; may:印尼语; jap:日语; kor:韩语; spa:西班牙语; fre:法语; ger:德语; por:葡萄牙语; vie:越南语; may:马来语; rus:俄语; ita:意大利语; hol:荷兰语; swe:瑞典语; fin:芬兰语; nor:挪威语; hun:匈牙利语; ara:阿拉伯语; hi:印地语。 */
  Language?: string;
}
...
declare interface VehicleRegCertInfo {
  /** 识别出的字段名称(关键字)，支持以下字段：【注册登记信息栏/摘要信息栏、转移登记摘要信息栏】车辆型号、车辆识别代号/车架号、发动机号、制造厂名称、轴距、轮胎数、总质量、外廓尺寸、轴数、车辆出厂日期、发证日期、使用性质、车辆获得方式、车辆类型、国产/进口、燃料种类、车身颜色、发动机型号、车辆品牌、编号、转向形式、机动车所有人1、身份证明名称1、号码1、登记机关1、登记日期1机动车所有人2、身份证明名称2、号码2、登记机关2、登记日期2机动车所有人3、身份证明名称3、号码3、登记机关3、登记日期3机动车所有人4、身份证明名称4、号码4、登记机关4、登记日期4机动车所有人5、身份证明名称5、号码5、登记机关5、登记日期5机动车所有人6、身份证明名称6、号码6、登记机关6、登记日期6机动车所有人7、身份证明名称7、号码7、登记机关7、登记日期7【登记栏】机动车登记证书编号、标题（目前支持转让登记、抵押登记、解除抵押）[标题：转让登记]姓名/名称、身份证明名称/号码、获得方式、机动车登记编号、转让登记日期[标题：抵押登记]抵押权人姓名/名称、身份证明名称/号码、抵押登记日期[标题：解除抵押]解除抵押日期 */
  Name?: string;
  /** 识别出的字段名称对应的值，也就是字段name对应的字符串结果。 */
  Value?: string;
}
...
declare interface ClassifyStoreNameRequest {
  ...
  ImageUrl?: string;
  /** 是否输出具体场景标签。注：开启后耗时会增加。 */
  SceneType?: boolean;
  /** 输入商户名称，辅助判断具体场景标签。注：仅SceneType配置为true时支持。 */
  StoreInfo?: string;
}
...
declare interface ClassifyStoreNameResponse {
  /** 门头照标签 */
  StoreLabel?: string[];
  /** 具体场景标签，输出3个最匹配的场景， 注：仅SceneType配置为true时支持。 */
  SceneType?: string;
  /** 唯一请求 ID，每次请求都会返回。 */
  RequestId?: string;
}
...
declare interface DescribeMarkEssayAgentJobRequest {
  /** 任务唯一ID。由服务端生成。 */
  JobId?: string;
}

declare interface DescribeMarkEssayAgentJobResponse {
  /** 图片旋转角度(角度制)，文本的水平方向为 0；顺时针为正，逆时针为负。 */
  Angle?: number;
  /** 配置结构化文本信息。 */
  SentenceSuggests?: MarkEssaySuggestions[];
  /** 任务执行错误码。当任务状态不为 FAIL 时，该值为""。 */
  ErrorCode?: string;
  /** 任务执行错误信息。当任务状态不为 FAIL 时，该值为""。 */
  ErrorMessage?: string;
...
declare interface ExtractDocBasicResponse {
  /** 图片旋转角度(角度制)，文本的水平方向为 0；顺时针为正，逆时针为负 */
  Angle?: number;
  /** 配置结构化文本信息 */
  StructuralList?: GroupInfo[];
...
declare interface GeneralBasicOCRRequest {
  ...
  /** 保留字段。 */
  Scene?: string;
  /** 识别语言类型。支持自动识别语言类型，同时支持自选语言种类，默认中英文混合(zh)，各种语言均支持与英文混合的文字识别。可选值：zh：中英混合zh_rare：支持英文、数字、中文生僻字、繁体字，特殊符号等auto：自动mix：多语言混排场景中,自动识别混合语言的文本jap：日语kor：韩语spa：西班牙语fre：法语ger：德语por：葡萄牙语vie：越语may：马来语rus：俄语ita：意大利语hol：荷兰语swe：瑞典语fin：芬兰语dan：丹麦语nor：挪威语hun：匈牙利语tha：泰语hi：印地语ara：阿拉伯语 */
  LanguageType?: string;
  /** 是否开启PDF识别，默认值为false，开启后可同时支持图片和PDF的识别。 */
  IsPdf?: boolean;
  /** 需要识别的PDF页面的对应页码，仅支持PDF单页识别，当上传文件为PDF且IsPdf参数值为true时有效，默认值为1。 */
...
declare interface HandwritingEssayOCRRequest {
  /** 图片/PDF的 Url 地址。要求图片经Base64编码后不超过10M，分辨率建议600*800以上，支持PNG、JPG、JPEG、BMP、PDF格式。图片下载时间不超过 3 秒。图片存储于腾讯云的 Url 可保障更高的下载速度和稳定性，建议图片存储于腾讯云。非腾讯云存储的 Url 速度和稳定性可能受一定影响。 */
  ImageUrl?: string;
  /** 图片/PDF的 Base64 值。要求Base64不超过10M，分辨率建议600*800以上，支持PNG、JPG、JPEG、BMP、PDF格式。图片的 ImageUrl、ImageBase64 必须提供一个，如果都提供，只使用 ImageUrl。 */
  ImageBase64?: string;
...
declare interface HandwritingEssayOCRResponse {
  /** 图片旋转角度(角度制)，文本的水平方向为 0；顺时针为正，逆时针为负 */
  Angle?: number;
  /** 还原文本信息 */
  WordList?: WordItem[];
...
declare interface MainlandPermitOCRRequest {
  ...
  /** 是否返回头像。默认不返回。 */
  RetProfile?: boolean;
  /** 图片正反面FRONT：正面BACK：反面 （仅支持来往内地通行证反面识别，不支持港澳台通行证反面识别）默认为FRONT */
  CardSide?: string;
}
...
declare interface QrcodeOCRRequest {
  /** 图片的 Base64 值。要求图片经Base64编码后不超过 10M，支持PNG、JPG、JPEG、PDF格式。图片的 ImageUrl、ImageBase64 必须提供一个，如果都提供，只使用 ImageUrl。 */
  ImageBase64?: string;
  /** 图片的 Url 地址。要求图片经Base64编码后不超过 10M，支持PNG、JPG、JPEG、PDF格式。建议图片存储于腾讯云，可保障更高的下载速度和稳定性。 */
  ImageUrl?: string;
}
...
declare interface QuestionSplitLayoutOCRRequest {
  ...
  /** 是否开启切边增强和弯曲矫正,默认为false不开启 */
  EnableImageCrop?: boolean;
  /** false: 使用当前默认模型返回结构化信息更全面，但检测精度一般true: 使用多模态推理模型，速度更快精度更高，但只返回题目最外层边框API默认false, demo默认使用的是true */
  UseNewModel?: boolean;
}
...
declare interface QuestionSplitOCRRequest {
  ...
  /** false: 使用当前默认模型(结构化信息更全面，但手写答案坐标精度一般) true: 使用多模态推理模型，推理效果更强（题目框选、手写答案坐标定位能力更优，但不返回题目选项和题目类型信息）API默认false, demo默认使用的是true */
  UseNewModel?: boolean;
}
...
declare interface RecognizeGeneralCardWarnRequest {
  ...
  /** 图片base64 */
  ImageBase64?: string;
  /** 卡证类型参数，包含以下范围： General：通用卡证IDCard：身份证 Passport：护照 BankCard：银行卡VehicleLicense：行驶证DriverLicense：驾驶证BizLicense：营业执照 HmtResidentPermit：港澳台居住证ForeignPermanentResident：外国人永居证MainlandPermit：港澳台来往内地通行证SocialSecurityCard：社保卡 */
  CardType?: string;
  /** 是否开启PDF识别，默认值为false，开启后可同时支持图片和PDF的识别。 */
  IsPdf?: boolean;
  /** 需要识别的PDF页面的对应页码，仅支持PDF单页识别，当上传文件为PDF且IsPdf参数值为true时有效，默认值为1。 */
...
declare interface SubmitMarkEssayAgentJobRequest {
...
declare interface SubmitMarkEssayAgentJobResponse {
...
declare interface Ocr {
  ...
  /** 作文批改Agent（提交任务） {@link SubmitMarkEssayAgentJobRequest} {@link SubmitMarkEssayAgentJobResponse} */
