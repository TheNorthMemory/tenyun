...
declare interface IntentionQuestionResult {
  /** 意愿核身错误码。- 取值范围： 0: "成功" -1: "参数错误" -2: "系统异常" -101: "请保持人脸在框内" -102: "检测到多张人脸" -103: "人脸检测失败" -104: "人脸检测不完整" -105: "请勿遮挡眼睛" -106: "请勿遮挡嘴巴" -107: "请勿遮挡鼻子" -201: "人脸比对相似度低" -202: "人脸比对失败" -301: "意愿核验不通过" -302: "用户回答阶段未检测到张嘴动作" -800: "前端不兼容错误" -801: "用户未授权摄像头和麦克风权限" -802: "核验流程异常中断，请勿切屏或进行其他操作" -803: "用户主动关闭链接/异常断开链接" -998: "系统数据异常" -999: "系统未知错误，请联系人工核实" - 若在人脸核身过程失败、未进入意愿确认过程，则该参数返回为空，请参考人脸核身错误码结果（DetectInfoText.ErrCode) */
  FinalResultDetailCode?: number | null;
  /** 意愿核身错误信息。 */
  FinalResultMessage?: string | null;
...
declare interface BankCard2EVerificationResponse {
  /** 认证结果码。- 计费结果码： '0': '认证通过'。 '-1': '认证未通过'。 '-4': '持卡人信息有误'。 '-5': '未开通无卡支付'。 '-6': '此卡被没收'。 '-7': '无效卡号'。 '-8': '此卡无对应发卡行'。 '-9': '该卡未初始化或睡眠卡'。 '-10': '作弊卡、吞卡'。 '-11': '此卡已挂失'。 '-12': '该卡已过期'。 '-13': '受限制的卡'。 '-14': '密码错误次数超限'。 '-15': '发卡行不支持此交易'。- 不计费结果码： '-2': '姓名校验不通过'。 '-3': '银行卡号码有误'。 '-16': '验证中心服务繁忙'。 '-17': '验证次数超限，请次日重试'。 */
  Result?: string;
  /** 业务结果描述。 */
  Description?: string;
  /** 唯一请求 ID，每次请求都会返回。 */
...
declare interface BankCard4EVerificationResponse {
  /** 认证结果码。- 收费结果码：'0': '认证通过'。'-1': '认证未通过'。'-6': '持卡人信息有误'。'-7': '未开通无卡支付'。'-8': '此卡被没收'。'-9': '无效卡号'。'-10': '此卡无对应发卡行'。'-11': '该卡未初始化或睡眠卡'。'-12': '作弊卡、吞卡'。'-13': '此卡已挂失'。'-14': '该卡已过期'。'-15': '受限制的卡'。'-16': '密码错误次数超限'。'-17': '发卡行不支持此交易'。- 不收费结果码：'-2': '姓名校验不通过'。'-3': '身份证号码有误'。'-4': '银行卡号码有误'。'-5': '手机号码不合法'。'-18': '验证中心服务繁忙'。'-19': '验证次数超限，请次日重试'。'-20': '该证件号暂不支持核验，当前仅支持二代身份证'。 */
  Result?: string;
  /** 业务结果描述。 */
  Description?: string;
  /** 唯一请求 ID，每次请求都会返回。 */
...
declare interface BankCardVerificationResponse {
  /** 认证结果码。- 收费结果码：'0': '认证通过'。'-1': '认证未通过'。'-5': '持卡人信息有误'。'-6': '未开通无卡支付'。'-7': '此卡被没收'。'-8': '无效卡号'。'-9': '此卡无对应发卡行'。'-10': '该卡未初始化或睡眠卡'。'-11': '作弊卡、吞卡'。'-12': '此卡已挂失'。'-13': '该卡已过期'。'-14': '受限制的卡'。'-15': '密码错误次数超限'。'-16': '发卡行不支持此交易'。- 不收费结果码：'-2': '姓名校验不通过'。'-3': '身份证号码有误'。'-4': '银行卡号码有误'。'-17': '验证中心服务繁忙'。'-18': '验证次数超限，请次日重试'。'-19': '该证件号暂不支持核验，当前仅支持二代身份证'。 */
  Result?: string;
  /** 业务结果描述。 */
  Description?: string;
  /** 唯一请求 ID，每次请求都会返回。 */
...
declare interface CheckBankCardInformationResponse {
  /** 认证结果码，收费情况如下。- 收费结果码：0: 查询成功-1: 未查到信息- 不收费结果码：-2：验证中心服务繁忙-3：银行卡不存在-4：认证次数超过当日限制，请次日重试 */
  Result?: string;
  /** 业务结果描述。 */
  Description?: string;
  /** 开户行。 */
...
declare interface DetectAIFakeFacesResponse {
  /** 检测到的图片是否存在攻击。- Low：无攻击风险。- Mid：中度疑似攻击。- High：高度疑似攻击。 */
  AttackRiskLevel?: string;
  /** 检测到疑似的攻击痕迹列表。- 说明：未检测到攻击痕迹时，返回空数组。- 此出参仅作为结果判断的参考，实际应用仍建议使用AttackRiskLevel的结果。 */
...
declare interface ParseNfcDataResponse {
  ...
  /** 性别。 */
  Sex?: string | null;
  /** 类型。- 取值范围：01：身份证。 03 ：中国护照。04 ：军官证。05 ：武警证。06：港澳通行证 。07 ：台胞证 。08：外国护照 。09 ：士兵证。09 ：临时身份证。11：户口本 。12 ：警官证 。13：外国人永久居留证。14：港澳台居民居住证。15：回乡证。16：大陆居民来往台湾通行证。99：其他证件。 */
  IdType?: string | null;
  /** 英文姓名。 */
  EnName?: string | null;
  /** 签发机关 */
  ...
  /** 旅行证件类的核验结果。- JSON格式如下：{"result_issuer ":"签发者证书合法性验证结果 ","result_pape r":"证件安全对象合法性验证 结果 ","result_data" :"防数据篡改验证结果 ","result_chip" :"防证书件芯片被复制验证结果"} 。- 取值范围： 0:验证通过，1: 验证不通过，2: 未验证，3:部分通过，当4项核验结果都为0时，表示证件为真。 */
