...
declare interface AttackRiskDetail {
  /** 疑似的攻击痕迹类型，具体类型如下：SuspectedSpoofingAttack：翻拍攻击SuspectedSynthesisImage：疑似合成图片SuspectedSynthesisVideo：疑似合成视频SuspectedAnomalyAttack：人脸特征疑似非真人SuspectedAdversarialAttack：疑似对抗样本攻击SuspectedBlackIndustry：疑似黑产批量模板攻击SuspectedWatermark：疑似存在水印 */
  Type?: string | null;
}
...
declare interface DetectDetail {
  ...
  /** 枚举活体检测类型：0：未知1：数字活体2：动作活体3：静默活体4：一闪活体（动作+光线）5: 远近活体枚举值：0： 未知1： 数字活体2： 动作活体3： 静默活体4： 一闪活体（动作+光线）5： 远近活体 */
...
declare interface DetectInfoText {
  ...
  /** 描述当前请求活体阶段被拒绝的详细原因，该参数仅限PLUS版本核身服务返回。- 详情如下：01-用户全程闭眼02-用户未完成指定动作03-疑似翻拍攻击04-疑似合成攻击05-疑似黑产模板06-疑似存在水印07-反光校验未通过08-疑似中途换人09-人脸质量过差10-距离校验不通过11-疑似对抗样本攻击12-嘴巴区域疑似存在攻击痕迹13-眼睛区域疑似存在攻击痕迹14-眼睛或嘴巴被遮挡 */
...
declare interface CheckPhoneAndNameResponse {
  /** 认证结果码，收费情况如下。收费结果码：0: 验证结果一致。1: 验证结果不一致。不收费结果码：-1:查无记录。-2:引擎未知错误。-3:引擎服务异常。-4:姓名校验不通过。-5:手机号码不合法。-6: 认证次数超过当日限制，请次日重试。-13：该号段不支持验证 */
  Result?: string;
  /** 业务结果描述。 */
  Description?: string;
  /** 唯一请求 ID，每次请求都会返回。 */
...
declare interface DetectAIFakeFacesRequest {
  /** 传入需要检测的人脸图片或人脸视频（当前仅支持单人脸检测），使用base64编码的形式，如您的场景视频和图片都有，我们更建议您使用视频进行检测，为了提供更好的检测效果，请您注意以下输入数据的限制与建议：图片的Base64值：建议整体图像480x640的分辨率，脸部大小在 100X100 以上，由手机前置摄像头拍摄。Base64编码后的图片数据大小建议不超过3M、最大不可超过10M，仅支持jpg、png格式。请使用标准的Base64编码方式(带=补位)，编码规范参考RFC4648。视频的Base64值：Base64编码后的大小建议在8M以内、最大不可超过10M，支持mp4、avi、flv、mov格式，由手机前置摄像头拍摄。视频建议时长为2～5s，最大不可超过20s。视频分辨率建议为480x640（最大支持720p），帧率在25fps~30fps之间。请使用标准的Base64编码方式(带=补位)，编码规范参考RFC4648。若您未使用Encryption进行加密传输，则本字段为必填参数。 */
  FaceInput?: string;
...
declare interface GetDetectInfoEnhancedResponse {
  ...
  /** 本次请求是否配置开启意愿核身校验。 false：未开启意愿核身校验 true：已开启意愿核身校验 说明：若请求开启了意愿核身校验，可结合IntentionVerifyType中具体使用的校验模式从对应的出参Result中获取最终的核验结果；若请求没有开启意愿核身校验，则可在出参Text中获取最终的核验结果。 */
  IsVerifyIntention?: boolean;
  /** 本次请求意愿核身校验使用的具体模式。0：问答模式1：点头确认模式2：朗读模式若未使用意愿核身功能，该字段返回值可以不处理。注意：此字段可能返回 null，表示取不到有效值 */
...
declare interface GetEidTokenRequest {
  /** EID商户ID。商户ID通过人脸核身控制台自助接入申请。商户ID与您通过腾讯云人脸核身控制台完成自助接入时所使用的腾讯云账号绑定。必须使用申请该商户ID时登录的腾讯云账号所对应的腾讯云API密钥调用该接口。 */
  MerchantId: string;
...
declare interface MobileNetworkTimeVerificationResponse {
  /** 认证结果码，收费情况如下。收费结果码：-0: 成功。-2: 手机号不存在-3: 手机号存在，但无法查询到在网时长。不收费结果码：-1: 手机号格式不正确。-4: 验证中心服务繁忙。-5：认证次数超过当日限制，请次日重试。-13：该号段不支持验证 */
  Result?: string;
  /** 业务结果描述。 */
  Description?: string;
...
declare interface MobileStatusResponse {
  /** 认证结果码，收费情况如下。收费结果码：0：成功。不收费结果码：-1：未查询到结果。-2：手机号格式不正确。-3：验证中心服务繁忙。-4：认证次数超过当日限制，请次日重试。-13：该号段不支持验证 */
  Result?: string;
  /** 业务结果描述。 */
  Description?: string;
  /** 状态码。- 取值范围：0：正常。1：停机。2：销号。4：不在网。5：在网但不可用 。99：未知状态。 */
  StatusCode?: number;
  /** 唯一请求 ID，每次请求都会返回。 */
  RequestId?: string;
}
...
declare interface PhoneVerificationCMCCResponse {
  /** 认证结果码，收费情况如下。收费结果码：0: 认证通过。-4: 信息不一致（手机号已实名，但姓名和身份证号与实名信息不一致）。不收费结果码：-6: 手机号码不合法。-7: 身份证号码有误。-8: 姓名校验不通过。-9: 没有记录。-11: 验证中心服务繁忙。-13：该号段不支持验证 */
  Result?: string;
  /** 运营商名称。取值范围为["移动","联通","电信",""]。 */
  Isp?: string;
  /** 业务结果描述。 */
...
declare interface PhoneVerificationResponse {
  /** 认证结果码。- 收费结果码0: 三要素信息一致。-4: 三要素信息不一致。- 不收费结果码-6: 手机号码不合法。-7: 身份证号码有误。-8: 姓名校验不通过。-9: 没有记录。-11: 验证中心服务繁忙。-12: 认证次数超过当日限制，请次日重试。-13：该号段不支持验证 */
  Result?: string;
  /** 业务结果描述。 */
  Description?: string;
