...
declare interface BatchTarget {
  ...
  /** 子机 ID。表示绑定主网卡主 IP。 */
  InstanceId?: string;
  /** 绑定 IP 时需要传入此参数，支持弹性网卡的 IP 和其他内网 IP，如果是弹性网卡则必须先绑定至CVM，然后才能绑定到负载均衡实例。注意：参数 InstanceId、EniIp 只能传入一个且必须传入一个。如果绑定双栈IPV6子机，必须传该参数。如果是跨地域绑定，则必须传该参数，不支持传InstanceId参数。 */
  EniIp?: string;
...
declare interface TargetGroupHealthCheck {
  ...
  Port?: number;
  /** 健康检查超时时间。 取值范围：[2, 60]单位：秒默认值：2响应超时时间要小于检查间隔时间。 */
  Timeout?: number;
  /** 检测间隔时间。取值范围：[1, 600]单位：秒默认值：5 */
  GapTime?: number;
  /** 检测健康阈值。取值范围：[2, 10]单位：次默认值：3 */
  GoodLimit?: number;
  /** 检测不健康阈值。取值范围：[2, 10]单位：次默认值：3 */
  BadLimit?: number;
...
declare interface CreateLoadBalancerRequest {
  ...
  ExclusiveCluster?: ExclusiveCluster;
  /** 性能容量型规格。若需要创建性能容量型实例，则此参数必填，取值范围： clb.c2.medium：标准型规格 clb.c3.small：高阶型1规格 clb.c3.medium：高阶型2规格 clb.c4.small：超强型1规格 clb.c4.medium：超强型2规格 clb.c4.large：超强型3规格 clb.c4.xlarge：超强型4规格 中国站用户若需要创建共享型实例，则无需填写此参数。国际站用户不传该参数默认购买的是标准型实例。如需了解规格详情，请参见[实例规格对比](https://cloud.tencent.com/document/product/214/84689)。 */
  SlaType?: string;
