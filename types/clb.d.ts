...
declare interface CreateLoadBalancerRequest {
  ...
  ExclusiveCluster?: ExclusiveCluster;
  /** 性能容量型规格。若需要创建性能容量型实例，则此参数必填，取值范围： clb.c2.medium：标准型规格 clb.c3.small：高阶型1规格 clb.c3.medium：高阶型2规格 clb.c4.small：超强型1规格 clb.c4.medium：超强型2规格 clb.c4.large：超强型3规格 clb.c4.xlarge：超强型4规格 国内站用户若需要创建共享型实例，则无需填写此参数。国际站用户不传该参数默认购买的是标准型实例。如需了解规格详情，请参见[实例规格对比](https://cloud.tencent.com/document/product/214/84689)。 */
  SlaType?: string;
