...
declare interface GenerateDataKeyRequest {
  ...
  /** 数据密钥 的描述，最大100字节 */
  Description?: string;
  /** KMS 独享版对应的 HSM 集群 ID。当KeyId 没有传入时有效，如果指定HsmClusterId,会默认在此集群下生成根密钥，然后利用创建的根密钥产生数据密钥。如果没有指定HsmClusterId，则会在公有云共享集群下创建一个根密钥，然后利用创建的根密钥产生数据密钥。 */
  HsmClusterId?: string;
}
...
declare interface ImportDataKeyRequest {
  ...
  /** 数据密钥 的描述，最大100字节 */
  Description?: string;
  /** 当导入密文数据密钥时，无需传入根密钥,如果传入也会忽略。当KeyId 为空，如果指定了独享集群HsmClusterId，则会在独享集群下创建一个根密钥，根据生成的根密钥加密数据密钥。如果没有指定独享集群HsmClusterId,则会在公有云共享集群下创建一个根密钥，根据生成的根密钥加密数据密钥。如果KeyId 不为空，根据指定的根密钥加密数据密钥。 */
  KeyId?: string;
  /** KMS 独享版对应的 HSM 集群 ID。当KeyId 没有传入时有效，如果指定了独享集群HsmClusterId，则会在独享集群下创建一个根密钥，根据产生的根密钥加密数据密钥。如果没有指定独享集群HsmClusterId,则会在公有云共享集群下创建一个根密钥，根据产生的根密钥加密数据密钥。 */
  HsmClusterId?: string;
}
