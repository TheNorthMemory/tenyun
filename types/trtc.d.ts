...
declare interface AsrParam {
  ...
  FilterModal?: number;
  /** 是否过滤句末的句号（目前仅支持填写基础语言引擎和标准语言引擎），范围为[0, 1]，默认值为 0。0：不过滤句末的句号；1：过滤句末的句号。 */
  FilterPunc?: number;
}
...
declare interface AudioFormat {
  ...
  Format?: string;
  /** 生成的音频采样率，默认24000支持如下采样率80001600024000 */
  SampleRate?: number;
...
declare interface McuAudioParams {
  ...
  /** 音频用户白名单，start时，为空或不填表示混所有主播音频，填具体值表示混指定主播音频；update时，不填表示不更新，为空表示更新为混所有主播音频，填具体值表示更新为混指定主播音频。使用黑白名单时，黑白名单必须同时填写。都不填写时表示不更新。同一个用户同时在黑白名单时，以黑名单为主。注：如果是跨房pk时，跨房混流需要指定音频白名单，否则pk主播的音频上行会被拉到两次，产生重音。 */
...
declare interface McuLayout {
  ...
  /** 是否保留上行SEI，1：保留 0：不保留取值范围：[0, 1]默认值：1 */
...
declare interface McuLayoutParams {
  ...
  RenderMode?: number;
  /** 是否保留上行sei，1：保留 0：不保留，只对动态布局生效，自定义布局不生效取值范围：[0, 1]默认值：1 */
  EnableStreamSEI?: number;
}
...
declare interface McuVideoParams {
  ...
  BackGroundColor?: string;
  /** 整个画布的背景图片url，优先级高于BackGroundColor。支持png、jpg、jpeg格式。图片分辨率限制不超过2K，图片大小限制不超过5MB。注：1，您需要确保图片链接的可访问性，后台单次下载超时时间为10秒，最多重试3次，若最终图片下载失败，背景图片将不会生效。2，url可支持字符集：【'0-9','a-z','A-Z','-', '.', '_', '~', ':', '/', '?', '#', '[', ']','@', '!', '&', '(', ')', '*', '+', ',', '%', '=', ';', '|'】，您需要确保url字符在可支持字符集内，若存在可支持字符集外的字符，背景图片将不会生效。 */
  BackgroundImageUrl?: string;
  /** 混流布局的水印参数。 */
...
declare interface McuWaterMarkImage {
  /** 水印图片URL地址，支持png、jpg、jpeg格式。图片分辨率限制不超过2K，图片大小限制不超过5MB。注：1，您需要确保图片链接的可访问性，后台单次下载超时时间为10秒，最多重试3次，若最终图片下载失败，水印图片将不会生效。2，url可支持字符集：【'0-9','a-z','A-Z','-', '.', '_', '~', ':', '/', '?', '#', '[', ']','@', '!', '&', '(', ')', '*', '+', ',', '%', '=', ';', '|'】，您需要确保url字符在可支持字符集内，若存在可支持字符集外的字符，水印图片将不会生效。 */
  WaterMarkUrl: string;
  /** 水印在输出时的宽。单位为像素值。 */
...
declare interface RowValues {
  ...
  /** 数据值 */
  RowValueFloat?: number[];
}

/** 语音转文字参数 */
declare interface STTConfig {
  /** 语音转文字支持识别的语言，默认是"zh" 中文可通过购买「AI智能识别时长包」解锁或领取包月套餐体验版解锁不同语言. 详细说明参考：AI智能识别计费说明语音转文本不同套餐版本支持的语言如下：基础语言引擎： "zh": 中文（简体）"zh-TW": 中文（繁体）"en": 英语"16k_zh_edu"：中文教育"16k_zh_medical"：中文医疗"16k_zh_court"：中文法庭标准语言引擎： "8k_zh_large": 普方大模型引擎. 当前模型同时支持中文等语言的识别，模型参数量极大，语言模型性能增强，针对电话音频中各类场景、各类中文方言的识别准确率极大提升. "16k_zh_large": 普方英大模型引擎. 当前模型同时支持中文、英文、多种中文方言等语言的识别，模型参数量极大，语言模型性能增强，针对噪声大、回音大、人声小、人声远等低质量音频的识别准确率极大提升. "16k_multi_lang": 多语种大模型引擎. 当前模型同时支持英语、日语、韩语、阿拉伯语、菲律宾语、法语、印地语、印尼语、马来语、葡萄牙语、西班牙语、泰语、土耳其语、越南语、德语的识别，可实现15个语种的自动识别(句子/段落级别). "16k_zh_en": 中英大模型引擎. 当前模型同时支持中文、英语识别，模型参数量极大，语言模型性能增强，针对噪声大、回音大、人声小、人声远等低质量音频的识别准确率极大提升.高级语言引擎："zh-dialect": 中国方言"zh-yue": 中国粤语"vi": 越南语"ja": 日语"ko": 韩语"id": 印度尼西亚语"th": 泰语"pt": 葡萄牙语"tr": 土耳其语"ar": 阿拉伯语"es": 西班牙语"hi": 印地语"fr": 法语"ms": 马来语"fil": 菲律宾语"de": 德语"it": 意大利语"ru": 俄语"sv": 瑞典语"da": 丹麦语"no": 挪威语"pl": 波兰语"uz": 乌兹别克语"hu": 匈牙利语"he": 希伯来语"ur": 乌尔都语"af-ZA": 南非荷兰语"nl-BE": 比利时荷兰语"nl-NL": 荷兰语(本土)bigmodel全新大模型语音识别引擎"bigmodel_auto": 多语言自动识别"bigmodel_zh": 中文"bigmodel_en": 英语"bigmodel_yue": 粤语"bigmodel_ar": 阿拉伯语"bigmodel_de": 德语"bigmodel_fr": 法语"bigmodel_es": 西班牙"bigmodel_pt": 葡萄牙语"bigmodel_id": 印尼语"bigmodel_it": 意大利语"bigmodel_ko": 韩语"bigmodel_ru": 俄语"bigmodel_th": 泰语"bigmodel_vi": 越南语"bigmodel_tr": 土耳其语"bigmodel_hi": 印地语"bigmodel_ms": 马来语"bigmodel_pl": 波兰语"bigmodel_cs": 捷克语"bigmodel_fil": 菲律宾语"bigmodel_fa": 波斯语"bigmodel_el": 希腊语"bigmodel_ro": 罗马尼亚语"bigmodel_hu": 匈牙利语"bigmodel_mk": 马其顿语注意：如果缺少满足您需求的语言，请联系我们技术人员。 */
  Language?: string;
  /** 发起模糊识别为高级版能力,默认按照高级版收费注意：不支持填写"zh-dialect", "16k_zh_edu", "16k_zh_medical", "16k_zh_court", "8k_zh_large", "16k_zh_large","16k_multi_lang", "16k_zh_en" */
  AlternativeLanguage?: string[];
  /** 自定义参数，联系后台使用 */
  CustomParam?: string;
  /** 语音识别vad的时间，范围为240-2000，默认为1000，单位为ms。更小的值会让语音识别分句更快。 */
...
declare interface TranscriptionParam {
  ...
  MaxIdleTime?: number;
  /** 自定义数据模式： 0表示不开启自定义数据，1表示开启自定义数据。不填默认为0，表示不开启自定义数据。 */
...
declare interface Voice {
  ...
  /** 语速调节，0.5 为半速慢放，2.0 为两倍速快放，1.0 为正常语速，区间：[0.5, 2.0]，默认1.0 */
  Speed?: number;
  /** 音量调节，默认1.0，10 为最大音量，建议保持默认值 1.0，区间：(0, 10] */
  Volume?: number;
  /** 音高调节，负值声音更低沉，正值声音更尖锐，0 为原始音高，区间 [-12, 12], 默认0 */
  Pitch?: number;
...
declare interface StartAITranscriptionRequest {
  ...
  /** 调用方传入的唯一Id，服务端用来任务去重，重复的任务会发起失败。服务端固定使用SdkAppId+RoomId+RoomIdType+RobotUserId来去重，如果传入了SessionId，也会使用SessionId去重。注意：TranscriptionMode为0时，需要保证一个房间内只发起一个任务，如果发起多个任务，则机器人之间会相互订阅，除非主动停止任务，否则只有10小时后任务才会超时退出，这种情况下建议填写SessionId，保证后续重复发起的任务失败。 */
  SessionId?: string;
  /** TRTC房间号的类型，0代表数字房间号，1代表字符串房间号。不填默认是数字房间号。 */
  RoomIdType?: number;
  /** 语音识别配置。 */
...
declare interface TextToSpeechRequest {
  ...
  /** 多音字/生僻字发音纠正词典条目。指定特定词语在本次请求中使用的发音。 */
  PronunciationDict?: PronunciationDict[];
  /** 字幕级别枚举值：0： 无字幕1： 句子级别字幕2： 词级别字幕，目前只有flow_01_ex支持词级别字幕默认值：0 */
  AlignmentMode?: number;
  /** json字符串，用于拓展用法 */
  ExtraParams?: string;
}
...
declare interface TextToSpeechSSERequest {
  ...
  /** 多音字/生僻字发音纠正词典条目。指定特定词语在本次请求中使用的发音。 */
  PronunciationDict?: PronunciationDict[];
  /** 字幕级别枚举值：0： 无字幕1： 句子级别字幕2： 词级别字幕，目前只有flow_01_ex支持默认值：0 */
  AlignmentMode?: number;
  /** json字符串，用于拓展用法 */
  ExtraParams?: string;
}
