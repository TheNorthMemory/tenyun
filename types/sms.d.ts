...
declare interface AddSmsSignRequest {
  ...
  SignName: string;
  /** 签名类型。其中每种类型后面标注了其可选的 DocumentType（证明类型）：0：公司，可选 DocumentType 有（0，1）。1：APP，可选 DocumentType 有（0，1，2，3，4） 。4：商标，可选 DocumentType 有（7）。5：政府/机关事业单位/其他机构，可选 DocumentType 有（2，3）。注1：必须按照对应关系选择证明类型，否则会审核失败。注2：签名类型2（网站）、3（公众号）、6（小程序）已不再支持，具体可参考 关于腾讯云短信签名申请规则更新的公告。注3：申请国内短信签名已不再支持签名类型1（APP），具体可参考 关于腾讯云短信签名申请规则更新的公告。 */
  SignType: number;
  /** 证明类型：0：三证合一。1：企业营业执照。2：组织机构代码证书。3：社会信用代码证书。4：应用后台管理截图（个人开发APP）。7：商标注册书。注1：证明类型5（网站备案后台截图）、6（小程序设置页面截图）、8（公众号设置页面截图）已不再支持，具体可参考 关于腾讯云短信签名申请规则更新的公告。注2：申请国内短信签名已不再支持证明类型4（应用后台管理截图），具体可参考 关于腾讯云短信签名申请规则更新的公告。 */
  DocumentType: number;
  ...
  ProofImage: string;
  /** 委托授权证明。选择 SignPurpose 为他用之后需要提交委托的授权证明。图片需先进行 base64 编码格式转换，将转换后的字符串去掉前缀data:image/jpeg;base64,再赋值给该参数。注：只有 SignPurpose 在选择为 1（他用）时，这个字段才会生效。 */
