...
declare interface AddSmsTemplateRequest {
  ...
  /** 模板内容。 */
  TemplateContent: string;
  /** 短信类型，1表示营销短信，2表示通知短信，3表示验证码短信。注：原“普通短信”类型模板目前仍支持提交申请，但为进一步提升短信发送质量、提高短信模板审核通过率，建议按“通知短信”类型或“验证码短信”类型申请新增模板，可参考[关于腾讯云短信模板类型优化公告](https://cloud.tencent.com/document/product/382/106171)。 */
  SmsType: number;
  /** 是否国际/港澳台短信：0：表示国内短信。1：表示国际/港澳台短信。 */
  International: number;
  /** 模板备注，例如申请原因，使用场景等。 */
...
declare interface SendSmsRequest {
  ...
  TemplateId: string;
  /** 短信签名内容，使用 UTF-8 编码，必须填写已审核通过的签名，例如：腾讯云，签名信息可前往 [国内短信](https://console.cloud.tencent.com/smsv2/csms-sign) 或 [国际/港澳台短信](https://console.cloud.tencent.com/smsv2/isms-sign) 的签名管理查看。发送国内短信该参数必填，且需填写签名内容而非签名ID。</dx-alert> */
  SignName?: string;
