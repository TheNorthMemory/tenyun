...
declare interface HostNameIndexSettings {
  ...
  Enabled?: boolean;
  /** 初始序号。取值范围与 IndexLength 参数有关：- IndexLength 为 0 时，取值范围为 [0, 99999999]。- IndexLength 为 [1, 8] 时，取值范围为为 [0, 10^IndexLength-1]，最大值即为指定位数的最大数字。初始序号默认值如下：- 首次启用递增序号：初始序号默认值为 0 （ 展示位数与 IndexLength 有关，例如 IndexLength 为 4，展示值为 0000）- 非首次开启递增序号：顺延之前的递增序号，例如上次使用递增至序号 069，则新的初始序号默认值为 070。注意：修改初始序号可能会造成伸缩组内序号重复。 */
  BeginIndex?: number;
  /** 递增序号长度，默认为0，表示不指定序号长度。 取值范围：0-8，最大为整数8。 - 长度设置为3，序号展示形式为：000、001、002 ... 010、011 ... 100 ... 999，序号上限为999- 长度设置为0，序号展示形式为：0、1、2 ... 10、11 ... 100 ... 1000 ...10000 ... 99999999，序号上限为99999999注意：递增序号持续超出上限会导致扩容失败，请不要设置过小的递增序号长度。 */
  IndexLength?: number;
}
...
declare interface HostNameSettings {
  ...
  /** 云服务器的主机名后缀。 点号（.）和短横线（-）不能作为 HostNameSuffix 的尾字符，不能连续使用。 不支持 Windows 实例。 其他类型（Linux 等）实例：字符长度为[1, 39]，且与 HostName 的长度和不能超过 41，允许支持多个点号，点之间为一段，每段允许字母（不限制大小写）、数字和短横线（-）组成。 */
  HostNameSuffix?: string | null;
  /** 云服务器的主机名分隔符。默认的分隔符是点号（.），可选短横线（-）或空字符串。分隔符用于拼接主机名，递增序号，后缀。假设主机名为 testGpu4090 ，递增序号为 0007，后缀为 server：- 分隔符为英文点号（.），最终拼接为 testGpu4090.007.server- 分隔符为短横线（-），最终拼接为 testGpu4090-007-server- 分隔符为空字符串，最终拼接为 testGpu4090007server */
...
declare interface InstanceNameSettings {
  ...
  /** 云服务器的实例名分隔符。默认的分隔符是点号（.），可选短横线（-）或空字符串。分隔符用于拼接实例名，递增序号，后缀。假设实例名为 testGpu4090 ，递增序号为 0007，后缀为 server：- 分隔符为英文点号（.），最终拼接为 testGpu4090.007.server- 分隔符为短横线（-），最终拼接为 testGpu4090-007-server- 分隔符为空字符串，最终拼接为 testGpu4090007server */
...
declare interface ScheduledAction {
  ...
  /** 停用期望数更新。表示定时任务触发时期望实例数正常更新。该值为 True 时，定时任务触发时不会主动修改期望实例数，但可能会因最大最小值机制修改期望实例数。以下案例的前提都是停用期望数更新为 True：- 定时任务触发时，原期望数为 5，定时任务将最小值改为 10，最大值改为 20，期望数改为 15，由于停用期望数更新，15不生效，但原期望数 5 小于最小值 10，最终新期望数为 10。- 定时任务触发时，原期望数为 25，定时任务将最小值改为 10，最大值改为 20，期望数改为 15，由于停用期望数更新，15不生效，但原期望数 25 大于最大值 20，最终新期望数为 20。- 定时任务触发时，原期望数为 13，定时任务将最小值改为 10，最大值改为 20，期望数改为 15，由于停用期望数更新，15不生效，期望数保持为 13 。 */
...
declare interface CreateScheduledActionRequest {
  ...
  Recurrence?: string;
  /** 停用期望数更新。默认值为 False，表示定时任务触发时期望实例数正常更新。该值为 True 时，定时任务触发时不会主动修改期望实例数，但可能会因最大最小值机制修改期望实例数。以下案例的前提都是停用期望数更新为 True：- 定时任务触发时，原期望数为 5，定时任务将最小值改为 10，最大值改为 20，期望数改为 15，由于停用期望数更新，15不生效，但原期望数 5 小于最小值 10，最终新期望数为 10。- 定时任务触发时，原期望数为 25，定时任务将最小值改为 10，最大值改为 20，期望数改为 15，由于停用期望数更新，15不生效，但原期望数 25 大于最大值 20，最终新期望数为 20。- 定时任务触发时，原期望数为 13，定时任务将最小值改为 10，最大值改为 20，期望数改为 15，由于停用期望数更新，15不生效，期望数保持为 13 。 */
  DisableUpdateDesiredCapacity?: boolean;
}
