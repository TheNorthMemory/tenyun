...
declare interface AddNodeList {
  ...
  Role: string;
  /** 节点所对应的可用区。当前支持的可用区，请参见[地域和可用区](https://cloud.tencent.com/document/product/240/3637)。- 单可用区，所有节点在同一可用区。- 多可用区：当前标准规格是三可用区分布，主从节点不在同一可用区，需注意配置新增节点对应的可用区，且新增后必须满足任意2个可用区节点数大于第3个可用区原则。 */
  Zone: string;
}
...
declare interface Auth {
  ...
  Mask: number | null;
  /** 指具有当前账号权限的数据库名。- \* ：表示所有数据库。- db.name：表示特定 name 的数据库。 */
...
declare interface BackupInfo {
  ...
  /** 备份状态。1：备份中。2：备份成功。 */
  Status?: number;
  /** 备份方式。0：逻辑备份。1：物理备份。3：快照备份。说明:通用版实例支持逻辑备份与物理备份。云盘版实例支持物理备份与快照备份，暂不支持逻辑备份。实例开通存储加密，则备份方式不能为物理备份。 */
  BackupMethod?: number;
  /** 备份记录 ID。 */
  BackId?: number;
  /** 备份删除时间。 */
...
declare interface CurrentOp {
  ...
  /** 操作执行语句。 */
  Query?: string;
  /** 操作类型。- none：特殊状态，空闲连接或内部任务等。- update：更新数据。- insert：插入操作。- query：查询操作。- command：命令操作。- getmore：获取更多数据。- remove：删除操作。- killcursors：释放查询游标的操作。 */
  Op?: string;
  /** 操作所在的分片名称。 */
  ...
  /** 操作详细信息。 */
  Operation?: string;
  /** 节点角色。- primary：主节点。- secondary：从节点。 */
  State?: string;
  /** 操作已执行时间（ms）。 */
...
declare interface InstanceChargePrepaid {
  ...
  Period?: number;
  /** 自动续费标识。取值范围：- NOTIFY_AND_AUTO_RENEW：通知过期且自动续费。在账户余额充足的情况下，实例到期后将按月自动续费。- NOTIFY_AND_MANUAL_RENEW：通知过期不自动续费。默认为NOTIFY_AND_MANUAL_RENEW。- DISABLE_NOTIFY_AND_MANUAL_RENEW：不通知过期不自动续费。 */
  RenewFlag?: string;
}
...
declare interface InstanceDetail {
  ...
  /** 私有网络的子网ID。 */
  SubnetId?: string;
  /** 实例状态。- 0：待初始化。- 1：流程处理中，例如：变更规格、参数修改等。- 2：实例正常运行中。- -2：已隔离（包年包月）。- -3：已隔离（按量计费）。 */
  Status?: number;
  /** 实例IP。 */
  Vip?: string;
  /** 端口号。 */
  ...
  /** 实例 CPU 核心数。 */
  CpuNum?: number;
  /** 实例机器类型。- HIO10G：通用高 HIO 万兆型。- HCD：云盘版类型。 */
  MachineType?: string;
  /** 实例从节点数。 */
  SecondaryNum?: number;
  /** 实例分片数。 */
  ReplicationSetNum?: number;
  /** 实例自动续费标志。- 0：手动续费。- 1：自动续费。- 2：确认不续费。 */
  ...
  /** 协议信息：mongodb。 */
  Protocol?: number;
  /** 实例类型。- 0：所有实例。- 1：正式实例。- 2：临时实例- 3：只读实例。- -1：同时包括正式实例、只读实例与灾备实例。 */
  InstanceType?: number;
  /** 实例状态描述。 */
...
declare interface ParamTpl {
  ...
  /** 参数模板描述。 */
  TplDesc?: string;
  /** 模板类型。- DEFAULT：系统默认模板。- CUSTOMIZE：自定义模板。 */
  TplType?: string;
}
...
declare interface RemoveNodeList {
  ...
  Role: string;
  /** 要删除的节点 ID。分片集群须指定一组分片要删除的节点名称即可，其余分片对该组对齐。- 获取方式：登录 [MongoDB控制台](https://console.cloud.tencent.com/mongodb)，在**节点管理**页签，可获取**节点 ID**。- 特别说明：分片集群同一节点上的分片，仅需指定0分片节点 ID 即可。例如：cmgo-6hfk\*\*\*\*\_0-node-primary。 */
  NodeName: string;
  /** 节点所对应的可用区。当前支持可用区信息，请参见[地域和可用区](https://cloud.tencent.com/document/product/240/3637)。- 单可用区，所有节点在同一可用区。- 多可用区：当前标准规格是三可用区分布，主从节点不在同一可用区，需注意配置所删除节点对应的可用区，且删除后必须满足任意2个可用区节点数大于第3个可用区原则。 */
  Zone: string;
}
...
declare interface SecurityGroupBound {
  /** 执行策略。- ACCEPT：允许，放行该端口相应的访问请求。- DROP：拒绝，直接丢弃数据包，不返回任何回应信息。 */
  Action?: string;
  /** 访问数据库的入站 IP 或 IP 段。 */
  CidrIp?: string;
  /** 访问数据库的端口。 */
...
declare interface CreateAccountUserRequest {
  ...
  InstanceId: string;
  /** 新账号名称。其格式要求如下：- 字符范围[1,64]。- 可输入[A,Z]、[a,z]、[1,9]范围的字符以及下划线“\_”与短划线“-”。 */
  UserName: string;
...
declare interface CreateBackupDownloadTaskRequest {
  ...
  BackupName: string;
  /** 指定要下载的副本集节点 ID 或分片集群的分片节点 ID 列表。- 如副本集实例 ID 为 cmgo-p8vnipr5，示例：BackupSets.0=cmgo-p8vnipr5_0，可下载全量数据。- 如分片集群实例 ID 为 cmgo-p8vnipr5，示例：BackupSets.0=cmgo-p8vnipr5_0&BackupSets.1=cmgo-p8vnipr5_1，即下载分片0和分片1的数据。分片集群如需全量下载，请按示例方式传入全部分片名称。 */
...
declare interface CreateDBInstanceParamTplRequest {
  ...
  Params?: ParamType[];
  /** 镜像模板 ID。若指定镜像模板，则以该模板为镜像，克隆出一个新的模板。**注意**：MirrorTplId 不为空值时，MongoVersion 及 ClusterType 将以 MirrorTpl 模板的版本及实例类型为准。 */
  MirrorTplId?: string;
}
...
declare interface DescribeBackupDownloadTaskRequest {
  ...
  /** 指定此次查询返回的页数，默认为0。 */
  Offset?: number;
  /** 排序字段。- createTime：按照备份下载任务的创建时间排序。默认为 createTime。- finishTime：按照备份下载任务的完成时间排序。 */
  OrderBy?: string;
  /** 排序方式。- asc：升序排列。- desc：降序排列。默认为 desc。 */
  OrderByType?: string;
  /** 指定任务状态，用于过滤下载任务。若不配置该参数，则返回所有状态类型的任务。- 0：等待执行。- 1：正在下载。- 2：下载完成。- 3：下载失败。- 4：等待重试。 */
  Status?: number[];
}
...
declare interface DescribeCurrentOpRequest {
  ...
  /** 设置查询筛选条件为操作任务类型。取值包括：- none：特殊状态，空闲连接或内部任务等。- update：更新数据。- insert：插入操作。- query：查询操作。- command：命令操作。- getmore：获取更多数据。- remove：删除操作。- killcursors：释放查询游标的操作。 */
  Op?: string;
  /** 筛选条件，分片名称。 */
  ReplicaSetName?: string;
  ...
  OrderBy?: string;
  /** 返回结果集排序方式。- ASC：升序。默认为 ASC，按照升序排序。- DESC：降序。 */
  OrderByType?: string;
}
...
declare interface DescribeDBInstanceDealRequest {
  /** 订单 ID。- 按量计费实例，请通过 [CreateDBInstanceHour](https://cloud.tencent.com/document/product/240/38570) 接口输出的参数**DealId**获取。。- 包年包月计费实例，请通过 [CreateDBInstance](https://cloud.tencent.com/document/product/240/38571) 接口输出的参数**DealId**获取。 */
  DealId: string;
}
...
declare interface DescribeDBInstanceDealResponse {
  ...
  /** 订单折扣价格。单位：元。 */
  DiscountPrice?: number;
  /** 订单操作行为。- purchase：新购。- renew：续费。- upgrade：升配.- downgrade：降配.- refund：退货退款。 */
  Action?: string;
  /** 当前订单的实例 ID。 */
  InstanceId?: string;
  /** 唯一请求 ID，每次请求都会返回。 */
...
declare interface DescribeDBInstancesRequest {
  /** 实例 ID 列表。请登录 [MongoDB 控制台](https://console.cloud.tencent.com/mongodb)在实例列表复制实例 ID。 */
  InstanceIds?: string[];
  /** 指定查询的实例类型。取值范围如下：- 0：所有实例。- 1：正式实例。- 2：临时实例- 3：只读实例。- -1：查询同时包括正式实例、只读实例与灾备实例。 */
  InstanceType?: number;
  /** 指定所查询实例的集群类型，取值范围如下：- 0：副本集实例。- 1：分片实例。- -1：副本集与分片实例。 */
  ClusterType?: number;
  /** 指定所查询实例的当前状态，取值范围如下所示：- 0：待初始化。- 1：流程处理中，例如：变更规格、参数修改等。- 2：实例正常运行中。- -2：已隔离（包年包月）。- -3：已隔离（按量计费）。 */
  Status?: number[];
  ...
  SubnetId?: string;
  /** 指定所查询实例的付费类型。- 0：查询按量计费实例。- 1：查询包年包月实例。- -1：查询按量计费与包年包月实例。 */
  PayMode?: number;
...
declare interface DescribeDetailedSlowLogsRequest {
  ...
  StartTime: string;
  /** 指定查询慢日志的结束时间。- 格式：yyyy-mm-dd hh:mm:ss，如：2019-06-02 12:00:00。- 查询起止时间间隔不能超过24小时，只允许查询最近7天内慢日志。 */
  EndTime: string;
  ...
  Limit?: number;
  /** 指定慢日志排序条件。- StartTime：按照慢日志生成时间排序。- ExecTime：按照慢日志执行时间排序。 */
  OrderBy?: string;
  /** 指定排序方式。- desc：倒序。- asc：顺序。 */
  OrderByType?: string;
}
...
declare interface InquirePriceModifyDBInstanceSpecRequest {
  ...
  Volume: number;
  /** 实例节点数量。请通过接口 [DescribeSpecInfo](https://cloud.tencent.com/document/product/240/38567) 获取。- 副本集实例，指变更配置后实例的主从节点数量。- 分片集群实例，指变更配置后实例每一个分片的主从节点数。**说明**：切勿同时发起调整节点数、调整分片数、调整节点规格的任务。 */
  NodeNum?: number;
...
declare interface ModifyDBInstanceSecurityGroupRequest {
  /** 实例 ID。请登录 [MongoDB 控制台](https://console.cloud.tencent.com/mongodb)在实例列表复制实例 ID。 */
  InstanceId: string;
  /** 目标安全组 ID。请登录[安全组控制台页面](https://console.cloud.tencent.com/vpc/security-group)复制目标安全组 ID。**注意**：该入参会全量替换存量已有集合，非增量更新。修改需传入预期的全量集合。 */
...
declare interface RenewDBInstancesRequest {
  /** 指定续费的一个或多个待操作的实例ID。- 可通过[DescribeDBInstances](https://cloud.tencent.com/document/product/240/38568)接口返回值中的**InstanceId**获取。- 每次续费请求的实例数量上限为100。 */
  InstanceIds: string[];
...
declare interface ResetDBInstancePasswordRequest {
  ...
  UserName: string;
  /** 指定账户的新密码。密码复杂度要求：- 8-32个字符长度。- 至少包含字母、数字和字符（!@#%^\*()\_）中的两种。 */
  Password: string;
}
...
declare interface RestartNodesRequest {
  /** 实例 ID。请登录 [MongoDB 控制台](https://console.cloud.tencent.com/mongodb)在实例列表复制实例 ID。 */
  InstanceId: string;
  /** 需要重启的节点 ID 列表。- 支持重启的节点类型包含：mongod节点、只读节点、mongos节点。- 节点 ID，请登录 [MongoDB 控制台](https://console.cloud.tencent.com/mongodb)在**节点管理**页面复制，或者通过 [DescribeDBInstanceNodeProperty ](https://cloud.tencent.com/document/product/240/82022)接口获取。 */
  NodeIds: string[];
}
...
declare interface SetBackupRulesRequest {
  ...
  BackupTime?: number;
  /** 指定每日自动备份频率。12: 每日备份2次，间隔约12小时。24: 每日备份1次（默认），间隔约24小时。 */
  BackupFrequency?: number;
  /** 设置自动备份发生错误时，是否发送失败告警。true：发送。false：不发送。 */
  Notify?: boolean;
  /** 指定备份数据保留时长。单位：天，默认为 7 天。取值范围：[7,365]。 */
  ...
  LongTermUnit?: string;
  /** 指定用于长期保留的具体备份日期。此设置仅在 LongTermUnit 被设为weekly 或 monthly 时生效。按周（weekly）保留：请输入 0-6 之间的数字来代表周日至周六。多个日期请用英文逗号分隔。按月（monthly）保留：请输入 1-31 之间的数字来代表月份中的具体日期。多个日期请用英文逗号分隔。 */
  LongTermActiveDays?: string;
  /** 长期备份保留时长。取值范围[30,1075]。 */
  LongTermExpiredDays?: number;
  /** 增量备份保留时长。单位：天。默认值：7天。取值范围：[7,365]。 */
  OplogExpiredDays?: number;
...
declare interface UpgradeDbInstanceVersionRequest {
  /** 实例ID列表，格式如：cmgo-p8vnipr5。与云数据库控制台页面中显示的实例ID相同。 */
  InstanceId: string;
  /** 新升级的数据库版本。当前支持版本如下所示，支持旧版本向高版本升级，不支持跨版本升级。- MONGO_40_WT：4.0 版本。- MONGO_42_WT：4.2 版本。- MONGO_44_WT：4.4 版本- MONGO_50_WT：5.0 版本- MONGO_60_WT：6.0 版本。- MONGO_70_WT：7.0 版本。- MONGO_80_WT：8.0 版本。 */
  MongoVersion: string;
  /** 指定升级操作是否在维护时间内进行。- 0：立即升级。- 1：维护时间窗升级。 */
  InMaintenance?: number;
}
