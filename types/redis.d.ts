...
declare interface Groups {
  ...
  AppId?: number;
  /** 地域ID 。- 1：广州 - 4：上海 - 5：中国香港 - 7：上海金融 - 8：北京 - 9：新加坡- 11：深圳金融- 15：美西（硅谷）- 16：成都 - 17：德国 - 18：韩国 - 19：重庆 - 21：印度 - 22：美东（弗吉尼亚）- 23：泰国 - 25：日本 */
  RegionId?: number;
...
declare interface InstanceSet {
  ...
  /** 项目 ID。 */
  ProjectId?: number;
  /** 地域 ID。1：广州。4：上海。5：中国香港。7：上海金融。 8：北京。 9：新加坡。 11：深圳金融。 15：美西（硅谷）。16：成都。17：法兰克福。18：首尔。19：重庆。21：孟买。22：美东（弗吉尼亚）。23：曼谷。25：东京。 */
  RegionId?: number;
  /** 区域 ID。 */
  ZoneId?: number;
  /** vpc网络 ID，例如75101。 */
...
declare interface ModifyDBInstanceSecurityGroupsRequest {
  /** 数据库引擎名称，本接口取值：redis。 */
  Product: string;
  /** 更换为新的安全组 ID 列表，即一个或者多个安全组 ID 组成的数组。- 若实例第一次配置安全组，请使用接口[AssociateSecurityGroups](https://cloud.tencent.com/document/product/239/41260)先绑定安全组。- 更换安全组，请在[控制台安全组](https://console.cloud.tencent.com/vpc/security-group)页面获取安全组 ID。 */
