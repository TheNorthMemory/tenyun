...
/** 关联实例信息 */
declare interface AttachInsInfo {
...
declare interface ClusterFwPreAccessCheckResult {
  ...
  CurrentStage?: string;
  /** 逐条检查项结果列表，按执行顺序追加 */
  Stages?: ClusterFwPreAccessCheckStage[];
  /** 策略路由配额核算报告，仅在 Status=1（通过）时非空 */
  PolicyRouteReport?: PolicyRoutePreCheckReport | null;
  /** 最近一次更新时间，格式：YYYY-MM-DD HH:MM:SS */
  UpdateTime?: string;
}

/** 单个阶段的实时检查状态 */
declare interface ClusterFwPreAccessCheckStage {
...
/** 集群模式防火墙开关数据详情 */
declare interface ClusterSwitchDetail {
...
declare interface CreateNatRuleItem {
  ...
  Direction: number;
  /** 规则序号。入口按 int64 读取后转换为 uint32，转换结果为 0 时归一化为 1；负值不会被单独拒绝，而会按 uint32 转换。写入中间分区时，序号为 1 或不大于当前同方向最大序号会按该位置插入并后移原规则，超过最大序号时通常归一化为末尾序号。新增且 From 不等于 insert_rule 时，如果本批首条规则转换后的序号为 1，则批内后续规则即使超过最大序号也按各自转换后的序号直接插入。 */
  OrderIndex: number;
  ...
  Port: string;
  /** 协议，大小写不敏感并归一化为大写。四层值为 TCP、UDP、ICMP、ICMPV6、ANY；应用层值为 HTTP、HTTPS、HTTP/HTTPS、TLS/SSL、SMTP、SMTPS、SMTP/SMTPS、FTP、DNS，其中 domain、tls、ssl 也归一化为 TLS/SSL；ANY 同时可按四层协议和应用协议解析。入站仅允许 ANY、TCP、UDP；domain 目的及解析为域名模板的 template 目的接受上述应用层协议及 ANY，但不接受 FTP；dnsparse 和 domainiptwoverify 目的仅允许 TCP 或 UDP；其他目的不接受 FTP 和 ANY 之外的应用层协议。Protocol=DNS 且目的不是域名模板或单独的 * 时，目的列表只能包含域名，不能包含 IP。 */
  Protocol: string;
  /** 流量处理方式，大小写不敏感：accept 表示放行，drop 表示拒绝，log 表示观察；isolateinaccept、isolateoutaccept 归一化为 accept，isolateindrop、isolateoutdrop 归一化为 drop。 */
  RuleAction: string;
  /** 访问源内容，格式由 SourceType 和 Direction 决定。net/ip 使用合法 IP 或 CIDR 的逗号列表，最多 10 项；location 使用地域 code，空字符串归一化为默认全地域掩码；vendor 使用 tencent、aliyun、aws、huawei、azure 或 all，可用逗号分隔；template 使用当前账号可解析的地址模板标识；instance 和 tag 必须引用当前账号已有对象；group 使用资源组标识，入口不校验其是否存在。 */
  SourceContent: string;
  /** 访问源类型，大小写不敏感。入站支持 net、ip、template、location、vendor；出站支持 net、ip、template、instance、group、tag。ip 和 net 均归一化为 IP 类型；template 会解析为模板的实际类型。 */
  SourceType: string;
  /** 访问目的内容，格式由 TargetType 和 Direction 决定。net/ip 使用合法 IP 或 CIDR 的逗号列表；domain 使用合法域名或 IP 的逗号列表，也接受单独的 *，标准泛域名和段内通配域名最多 5 级，段内通配域名还要求对应引擎版本支持；dnsparse 使用单个精确域名、最多 5 级的标准泛域名或相应域名模板，不接受单独的 *、段内通配域名、IP 或逗号列表；domainiptwoverify 使用单个精确域名或不含通配符的相应域名模板，不接受单独的 *、任何通配域名、IP 或逗号列表；location 使用地域 code，空字符串归一化为默认全地域掩码；vendor 使用 tencent、aliyun、aws、huawei、azure 或 all，可用逗号分隔；template 使用当前账号可解析的地址模板标识；instance 和 tag 必须引用当前账号已有对象；group 使用资源组标识，入口不校验其是否存在。解析后的目的内容最长 1023 字节；IP 或 domain 目的最多包含 10 个逗号分隔项。 */
  TargetContent: string;
  /** 访问目的类型，大小写不敏感。入站支持 net、ip、template、instance、group、tag；出站支持 net、ip、template、domain、dnsparse、domainiptwoverify、location、vendor。ip 和 net 均归一化为 IP 类型；template 会解析为模板的实际类型，入站解析为域名模板时被拒绝。dnsparse 和 domainiptwoverify 分别要求对应引擎版本支持；domainiptwoverify 使用域名模板时还要求严格模式域名模板版本支持；domain 目的使用段内通配域名时要求段内通配域名版本支持。 */
  TargetType: string;
  /** 规则描述。省略或传空字符串时保存为空；入口不裁剪内容，也不执行长度归一化或字符数校验。 */
  Description?: string;
  /** 规则状态。字符串 true 表示启用，false 表示禁用，大小写不敏感。省略或传空字符串时读取当前账号的访问控制规则默认状态；配置不存在或无法解析时默认为启用。 */
  Enable?: string;
  /** 规则内部 UUID。新增请求仅在 From=batch_import_cover 时采用正整数值替换自动生成的内部 UUID；其他新增路径和修改请求忽略该字段。 */
  InternalUuid?: number;
  /** 端口协议模板 ID。省略或传空字符串时直接使用请求中的 Protocol 和 Port；非空时必须指向当前账号已有的端口协议模板，模板条目会逐项参与协议与目的类型联动校验。使用模板时，入口仍先校验请求中的 Protocol，并在该协议不是 ICMP 时校验请求中的 Port；请求值不要求固定为 ANY 和 -1/-1。 */
  ...
  Scope?: string;
  /** 规则数据库 ID。普通新增、insert_rule 和 batch_import 忽略该字段；batch_import_cover 新增会采用正整数值作为待写入记录 ID。修改请求使用正整数定位中间分区中的现有规则，先删除该记录再以同一 Uuid 重建并返回该 ID；省略、0 或负值无法定位修改目标。 */
  Uuid?: number;
}
...
/** 离线导出任务 */
declare interface OfflineExportTask {
  /** 任务ID */
  TaskId?: string;
  /** 任务名称 */
  TaskName?: string;
...
/** 策略路由预编排配额报告 */
declare interface PolicyRoutePreCheckReport {
  /** 本次编排生成的下一跳数量 */
  NextHopCount?: number;
  /** 下一跳上限 */
  NextHopLimit?: number;
...
declare interface VpcRuleItem {
  ...
  DestContent: string;
  /** 访问目的类型，不区分大小写。VPC 规则可用 net（ip 为同义值）、domain、template、dnsparse、domainiptwoverify、instance、tag、group、location 或 vendor；url 虽可被通用类型映射识别，但 VPC 出向内容解析不支持。地址模板、dnsparse、domainiptwoverify、严格模式域名模板和段内通配域名分别要求当前 VPC 防火墙引擎支持对应能力，否则返回 UnsupportedOperation。类型及地址模板查询到的实际类型共同决定 DestContent 和 Protocol 的校验。 */
  DestType: string;
  /** 规则生效的 VPC 边范围，不区分大小写。ALL 表示全部 VPC 间，cfws- 前缀表示指定 VPC 边；其它格式返回参数错误。ALL 还要求相关 VPC 防火墙引擎支持全局规则，否则返回 UnsupportedOperation。 */
  EdgeId: string;
  /** 规则状态。字符串 true 表示启用，false 表示禁用，大小写不敏感；省略或传空字符串时使用租户通知配置中的默认规则状态，该配置没有可用值时默认启用；其它字符串返回参数错误。 */
  Enable: string;
  /** 规则在相同 IpVersion 的中间分区内的顺序。请求整数先转换为 uint32，0 归一化为 1，负整数及超出 uint32 范围的整数按该转换结果处理；写入时，顺序为 1 或不大于当前最大顺序的规则在该位置插入并将现有规则后移，通常大于当前最大顺序的值归一化为末尾。例外是 AddVpcAcRule 未使用精确值 insert_rule 且首条规则解析后的顺序为 1 时，本批每条规则都保留其解析后顺序并按该值移动现有规则，因此后续规则即使大于当前最大顺序也不会归一化为末尾。 */
  OrderIndex: number;
  /** 访问控制策略的端口。除 ICMP 和 ICMPV6 外，该字段按逗号拆分；每项可以是正整数单端口，或以斜杠分隔且起点不大于终点的两个整数，-1/-1 表示全部端口。ICMP 和 ICMPV6 忽略请求值并保存为空字符串；其它协议的空值或非法格式返回参数错误。FTP 只接受正整数单端口，不接受逗号组合或斜杠区间。 */
  Port: string;
  /** 协议名称，不区分大小写并归一化。IP、instance、tag、group、location、vendor 及 IP 地址模板目的支持 TCP、UDP、ICMP、ICMPV6、ANY 和 FTP，其中 FTP 只接受单端口；domain 及域名地址模板目的支持 ANY、HTTP、HTTPS、HTTP/HTTPS、TLS/SSL、SMTP、SMTPS、SMTP/SMTPS 和 DNS，不支持 FTP，domain、tls、ssl 归一化为 TLS/SSL；dnsparse 和 domainiptwoverify 仅支持 TCP 或 UDP。template 按查询到的实际模板类型应用上述限制；填写 ParamTemplateId 时，组内每个协议端口项还会应用相同的目的类型限制。 */
  Protocol: string;
  /** 流量通过云防火墙时的处理方式，不区分大小写。accept 表示放行，drop 表示拒绝，log 表示观察；isolateinaccept 和 isolateoutaccept 归一化为放行，isolateindrop 和 isolateoutdrop 归一化为拒绝；其它值返回参数错误。 */
  RuleAction: string;
  /** 访问源内容，由 SourceType 决定格式和校验。net/ip 接受合法 IP 或 CIDR 的逗号分隔列表，最多 10 项；template 接受当前租户的地址模板标识并归一化为模板 UUID，只允许与 IpVersion 一致的 IP 地址模板，域名地址模板不能作为访问源；instance 和 tag 必须在当前租户存在，其中 instance 必须具有 IpVersion 对应的私网地址；group 接受资源组标识。 */
  SourceContent: string;
  /** 访问源类型，不区分大小写。VPC 规则可用 net（ip 为同义值）、template、instance、tag 或 group；url、location 和 vendor 虽可被通用类型映射识别，但 VPC 出向源内容解析不支持。使用 template 要求当前 VPC 防火墙引擎支持地址模板能力。类型及模板查询到的实际类型共同决定 SourceContent 的校验。 */
  SourceType: string;
  ...
  Deleted?: number;
  /** 查询结果中对目的实际类型的补充提示，例如 template 可区分 IP 地址模板和域名地址模板。新增和修改处理程序不读取请求中的该字段；实际目的类型由 DestType、DestContent 和模板查询结果确定。 */
  DestValueType?: string;
  /** 规则命中次数。请求中的该字段不参与新增或修改规则构造，主要用于查询返回。 */
  DetectedTimes?: number;
  /** EdgeId 对应的 VPC 边名称。请求中的该字段不参与新增或修改规则构造，主要用于查询返回和操作记录展示。 */
  EdgeName?: string;
  /** 规则生效的防火墙组或 CCN 范围，格式比较不区分大小写。ALL 归一化为大写 ALL；cfwg- 前缀和 ccn- 前缀的值保留请求原文；省略、空字符串或其它格式归一化为 ALL，处理程序不因格式无效而返回错误。 */
  FwGroupId?: string;
  /** 防火墙组或 CCN 名称。请求中的该字段不参与新增或修改规则构造，主要用于查询返回和操作记录展示。 */
  FwGroupName?: string;
  /** 内部规则标识。AddVpcAcRule 的精确 batch_import_cover 分支会采用正整数 InternalUuid 替换自动生成的内部标识；其它新增分支和 ModifyVpcAcRule 不读取请求中的该字段。 */
  InternalUuid?: number;
  /** 查询结果中的规则有效性标记，0 表示有效，1 表示无效。请求中的该字段不参与新增或修改规则构造。 */
  Invalid?: number;
  /** IP 版本，0 表示 IPv4，1 表示 IPv6。省略或传入 0、1 以外的整数时按 IPv4 处理；instance 必须具有对应版本的私网地址，IP 地址模板必须与解析后的版本一致。处理程序不使用该字段校验直接填写的 net IP/CIDR 版本。 */
  IpVersion?: number;
  /** 端口协议组 ID。省略或传空字符串时直接使用 Protocol 和 Port；非空时查询当前租户的端口协议组并用组内协议端口项执行目的类型和协议类别校验，模板查询失败时请求失败。无论是否填写该字段，请求中的 Protocol 和非 ICMP/ICMPV6 Port 都会先按普通规则解析，格式无效时请求失败。 */
  ParamTemplateId?: string;
  /** 端口协议组名称。请求中的该字段不参与新增或修改规则构造，主要用于查询返回。 */
  ParamTemplateName?: string;
  /** 规则分区展示值，1 表示最前分区，2 表示中间分区，3 表示最后分区。新增和修改处理程序不读取请求中的该字段，写入时固定为中间分区；ModifyVpcAcRule 只能定位并替换中间分区中的现有规则。 */
  RulePartition?: number;
  ...
  UpdateTime?: string;
  /** 规则数据库 ID。ModifyVpcAcRule 必须传当前租户中间分区内既有规则的正整数 Uuid；处理程序按该 ID 删除原记录并以同一 ID 插入替换后的规则。AddVpcAcRule 的普通新增、insert_rule 和 batch_import 分支忽略该字段；精确 batch_import_cover 分支会采用正整数 Uuid 作为新记录 ID，非正数值仍由数据库生成 ID。 */
  Uuid?: number;
}
...
declare interface CreateAlertCenterOmitRequest {
  /** 直接处置的记录 ID 列表，公共请求结构要求提供。TableType=AlertTable 时元素为告警日志 logid；TableType=InterceptionTable 时元素为拦截记录 unique_id。处理时会与 HandleEventIdList 解析出的日志 ID 合并，再删除空字符串并去重；因此仅按聚合事件处置时可传 [""] 作为空占位。HandleIdList 与 HandleEventIdList 不能同时为空。 */
  HandleIdList: string[];
  /** 必填的忽略数据来源，只接受 AlertTable 或 InterceptionTable。AlertTable 更新租户告警表中 logid 命中的记录；InterceptionTable 更新租户拦截表中 unique_id 命中的记录。字段没有默认值，缺失、空字符串或其它值均返回参数错误。 */
  TableType: string;
  /** AI操作来源枚举值：console： 控制台来源值wechat： 微信。 */
  CfwAiAgentOperationSource?: string;
  /** 可选的告警聚合事件 ID 列表。处理时逐个事件 ID 查询其对应的告警日志 logid，并将查询结果并入 HandleIdList；无法解析的事件 ID 不会产生目标 ID。该解析不会改写 TableType：若 TableType=InterceptionTable，解析出的 logid 仍会作为 unique_id 查询拦截表。字段可省略或传空数组，但 HandleIdList 与本字段不能同时为空；若最终合并、去空和去重后没有目标 ID，业务状态为失败。 */
  HandleEventIdList?: string[];
}

declare interface CreateAlertCenterOmitResponse {
  /** Cloud API 处理返回码。0 表示 Action 处理函数未返回顶层错误，-1 表示入口参数校验或路由处理失败。忽略记录的数据库处理结果由 Status 表示；ReturnCode=0 不代表一定有记录被更新。 */
  ReturnCode?: number;
  /** Cloud API 处理信息。Action 处理函数未返回顶层错误时为 success；入口参数校验或路由处理失败时为 failed，并同时返回 Error。 */
  ReturnMsg?: string;
  /** 忽略处理状态。0 表示目标表更新语句执行时未返回数据库错误，但接口不检查受影响行数，因此不保证有记录命中；-1 表示参数归一化后无有效目标或下游处理失败；-3 表示下游报告重复记录错误。应结合 ReturnCode 判断入口校验是否通过。 */
  Status?: number;
  /** 唯一请求 ID，每次请求都会返回。 */
  RequestId?: string;
}
...
declare interface DescribeCcnVpcFwPolicyLimitRequest {
}

declare interface DescribeCcnVpcFwPolicyLimitResponse {
  /** 支持的引流策略数量（最外层总条数） */
...
declare interface DescribeClusterVpcFwSwitchsResponse {
  /** 总条数 */
  Total?: number;
  /** 防火墙开关列表 */
...
declare interface DescribeLogStorageStatisticRequest {
  /** 开始时间参数格式：2026-07-01 15:02:01 */
  StartTime?: string;
  /** 结束时间参数格式：2026-07-01 15:02:01 */
  EndTime?: string;
}
...
declare interface DescribeOfflineExportTaskResponse {
  /** 返回状态码 0 成功 非0不成功 */
  ReturnCode?: number;
  /** 返回信息 success 成功 其他 不成功 */
  ReturnMsg?: string;
  /** 离线导出任务列表 */
  Data?: OfflineExportTask[];
  /** 任务数量 */
  Total?: number;
  /** 剩余导出文件配额，单位B */
...
declare interface DescribeOfflineExportTemporaryCredentialsRequest {
  /** 任务ID */
  TaskId: string;
}

declare interface DescribeOfflineExportTemporaryCredentialsResponse {
  /** 返回状态码 0 成功 非0不成功 */
  ReturnCode?: number;
  /** 返回信息 success 成功 其他 不成功 */
  ReturnMsg?: string;
...
declare interface ExportLogsOfflineRequest {
  /** 日志类型标识枚举值：cfw_netflow_border： 流量日志-互联网边界cfw_netflow_vpc： 流量日志-VPC边界cfw_netflow_nat： 流量日志-NAT边界cfw_rule_acl： 访问控制-互联网边界cfw_rule_vpc_acl： 访问控制-VPC边界cfw_rule_nat_acl： 访问控制-NAT边界cfw_rule_threatinfo： 入侵防御-入侵防御cfw_netflow_nta： 全流量检测与响应-流量分析cfw_ndr_subject_risk： 全流量检测与响应-流量风险cfw_ndr_ai_audit： 全流量检测与响应-AI流量风险operate_log_all： 操作日志 */
  Index: string;
  /** 筛选开始时间 */
  StartTime: string;
  /** 筛选结束时间 */
  ...
  /** 文件过期时长，1一天，7七天，-1永久 */
  StorageDays?: number;
  /** 日志分析查询语句，Query和Filters不能同时使用，在一次请求中有且只能选择一个 */
  Query?: string;
  /** 导出任务类型，LogAnalysis日志分析导出，LogSearch日志审计导出 */
  TaskType?: string;
}

declare interface ExportLogsOfflineResponse {
  /** 返回状态码 0 成功 非0不成功 */
  ReturnCode?: number;
  /** 返回信息 success 成功 其他 不成功 */
  ReturnMsg?: string;
...
declare interface RemoveOfflineExportTaskRequest {
  /** 任务ID */
  TaskId: string;
  /** 是否保留文件，1保留，非1删除 */
  KeepFile?: number;
...
declare interface RemoveOfflineExportTaskResponse {
  /** 返回状态码 0 成功 非0不成功 */
  ReturnCode?: number;
  /** 返回信息 success 成功 其他 不成功 */
  ReturnMsg?: string;
...
declare interface SearchLogRequest {
  ...
  Query: string;
  /** 检索语法规则，默认值为0，推荐使用1 。0：Lucene语法1：CQL语法（日志服务专用检索语法，控制台默认也使用该语法规则）。详细说明参见检索条件语法规则 */
  SyntaxRule?: number;
...
declare interface Cfw {
  ...
  /** 查询CCN中VPC防火墙接入策略配置时的规则条数限制 {@link DescribeCcnVpcFwPolicyLimitRequest} {@link DescribeCcnVpcFwPolicyLimitResponse} */
  ...
  DescribeOfflineExportTemporaryCredentials(data: DescribeOfflineExportTemporaryCredentialsRequest, config?: AxiosRequestConfig): AxiosPromise<DescribeOfflineExportTemporaryCredentialsResponse>;
  ...
  /** 日志审计日志离线导出 {@link ExportLogsOfflineRequest} {@link ExportLogsOfflineResponse} */
  ExportLogsOffline(data: ExportLogsOfflineRequest, config?: AxiosRequestConfig): AxiosPromise<ExportLogsOfflineResponse>;
  ...
  /** 删除日志离线导出任务 {@link RemoveOfflineExportTaskRequest} {@link RemoveOfflineExportTaskResponse} */
  RemoveOfflineExportTask(data: RemoveOfflineExportTaskRequest, config?: AxiosRequestConfig): AxiosPromise<RemoveOfflineExportTaskResponse>;
