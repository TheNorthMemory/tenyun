...
declare interface ApproverInfo {
  ...
  Deadline?: number;
  /** 【在用文件发起合同场景下才有效，模板发起场景下需要在模板中配置】签署人在合同中的填写控件列表，列表中可支持下列多种填写控件，控件的详细定义参考开发者中心的Component结构体单行文本控件多行文本控件勾选框控件数字控件图片控件具体使用说明可参考为签署方指定填写控件注：此参数仅在通过文件发起合同或者合同组时生效 */
  ...
  /** 是否不保存联系人默认 false 保存联系人 true 不保存联系人设置这个参数为保存联系人的时候,他方企业签署人会被保存进发起人的联系人中。联系人查看可登录电子签控制台 进行查看。如下图位置： */
...
declare interface AuthRecord {
  ...
  AuditStatus?: number;
  /** 审核失败原因，当 AuditStatus 返回2时，则会返回具体的原因。 */
  Reason?: string;
}
...
declare interface FlowBrief {
  ...
  FlowType?: string;
  /** 合同流程当前的签署状态, 会存在下列的状态值 **0** : 未开启流程(合同中不存在填写环节) **1** : 待签署 **2** : 部分签署 **3** : 已拒签 **4** : 已签署 **5** : 已过期 **6** : 已撤销 **7** : 未开启流程(合同中存在填写环节) **8** : 等待填写 **9** : 部分填写 **10** : 已拒填 **16** : 已失效（签署期间有签署人改名等原因导致） **21** : 已解除 */
  FlowStatus?: number;
...
declare interface FlowCreateApprover {
  ...
  SignComponents?: Component[];
  /** 签署人填写控件 此参数仅针对文件发起（CreateFlowByFiles）生效合同中的填写控件列表，列表中可支持下列多种填写控件，控件的详细定义参考开发者中心的Component结构体单行文本控件多行文本控件勾选框控件数字控件图片控件动态表格等填写控件此参数仅针对文件发起设置生效,模板发起合同签署流程, 请以模板配置为主 */
  Components?: Component[];
  ...
  /** 指定个人签署方查看合同的校验方式,可以传值如下: **1** : （默认）人脸识别,人脸识别后才能合同内容 **2** : 手机号验证, 用户手机号和参与方手机号(ApproverMobile)相同即可查看合同内容（当手写签名方式为OCR_ESIGN时，该校验方式无效，因为这种签名方式依赖实名认证）注: 如果合同流程设置ApproverVerifyType查看合同的校验方式, 则忽略此签署人的查看合同的校验方式此字段可传多个校验方式此参数仅针对文件发起设置生效,模板发起合同签署流程, 请以模板配置为主. */
  ApproverVerifyTypes?: number[];
  ...
  Deadline?: number;
  /** 只有在生成H5签署链接的情形下（ 如调用获取H5签署链接、获取H5批量签署链接等接口），该配置才会生效。您可以指定H5签署视频核身的意图配置，选择问答模式或点头模式的语音文本。注意：视频认证为白名单功能，使用前请联系对接的客户经理沟通。使用视频认证时，生成H5签署链接必须将签署认证方式指定为人脸（即ApproverSignTypes设置成人脸签署）。签署完成后，可以通过查询签署认证人脸视频获取到当时的视频。 */
  Intention?: Intention;
  /** 进入签署流程的限制，目前支持以下选项： 空值（默认） :无限制，可在任何场景进入签署流程。 link :选择此选项后，将无法通过控制台或电子签小程序列表进入填写或签署操作，仅可预览合同。填写或签署流程只能通过短信或发起方提供的专用链接进行。 */
  SignEndpoints?: string[];
  /** 是否不保存联系人默认 false 保存联系人 true 不保存联系人设置这个参数为保存联系人的时候,他方企业签署人会被保存进发起人的联系人中。联系人查看可登录电子签控制台 进行查看。如下图位置： */
...
/** 合同备注 */
declare interface FlowRemarkItem {
  /** 合同备注下标，对应最多5个备注位取值范围：[0, 4] */
  RemarkId?: number;
...
declare interface JumpEvent {
  /** 跳转事件枚举枚举值：1： 企业收录2： 超管授权书审核3： 企业认证完成 */
...
declare interface PresetApproverInfo {
  ...
  IdCardType?: string;
  /** 企业用户动态签署方场景指定预设企业名称。注意：1. 若为企业动态签署方场景，此参数必须要指定。2. 企业动态签署方场景暂不支持指定姓名证件手机号等参数，仅支持指定企业名称。 */
  OrganizationName?: string;
}
...
/** 催办邮件结构体 */
declare interface RemindEmailInfo {
  /** 签署编号 */
  SignId?: string;
...
declare interface RemindFlowGroupDetail {
  ...
  /** 签署人对应的签署id，签署方唯一编号，一个全局唯一的标识符，不同的流程不会出现冲突。在CreateFlowSignUrl、CreateBatchQuickSignUrl等接口生成签署链接时，可以通过查询合同流程的详情信息接口 DescribeFlowInfo 获取签署人的Signld，然后可以将此值传入，为该签署人创建签署链接。这样可以避免重复传输姓名、手机号、证件号等其他信息。 */
  SignId?: string;
...
/** 根据合同对印章授权 */
declare interface SealPolicyAuthorizationFlows {
  /** 合同id列表，最大支持50个 */
  FlowIds?: string[];
  /** 合同组id列表， 最大支持10个FlowGroupIds(合同组)与FlowIds(合同列表) 两个参数只能选择其中一个，两者同时传会提示参数错误。 */
  FlowGroupIds?: string[];
}
...
declare interface CancelOrganizationFlowsResponse {
  /** 全量撤销企业合同任务编号，为32位字符串，可用于查询批量撤销合同结果 或关联全量撤销企业合同任务结果回调 */
  TaskId?: string;
  /** 唯一请求 ID，每次请求都会返回。 */
  RequestId?: string;
}
...
declare interface CreateBatchContractReviewTaskRequest {
  ...
  Operator: UserInfo;
  /** 合同审查的PDF、WORD文件资源编号列表，通过[UploadFiles](https://qian.tencent.com/developers/companyApis/templatesAndFiles/UploadFiles)接口获取PDF、WORD文件资源编号。 注: `目前，此接口仅支持5个文件发起。每个文件限制在10M以下，文件必须是PDF、WORD格式` */
  ResourceIds: string[];
  ...
  PolicyType?: number;
  /** 该字段已不再使用！合同审查中的角色信息，通过明确入参角色的名称和描述，可以提高合同审查的效率和准确性。用户不做配置时大模型会根据合同内容推荐出风险识别角色的名称和描述信息。 */
  Role?: RiskIdentificationRoleInfo;
  /** 合同审查中的角色信息，通过明确入参角色的名称和描述，可以提高合同审查的效率和准确性。用户不做配置时大模型会根据合同内容推荐出风险识别角色的名称和描述信息。 */
  Roles?: RiskIdentificationRoleInfo[];
  /** 该字段已不再使用！用户配置的审查清单ID，基于此清单ID批量创建合同审查任务，为32位字符串。[点击查看审查清单ID在控制台上的位置](https://qcloudimg.tencent-cloud.cn/raw/2c6588549e28ca49bd8bb7f4a072b19e.png)。如果用户不做此配置大模型会根据合同内容在当前企业下的审查清单和系统默认的清单中选择一个清单进行审查。 */
  ChecklistId?: string;
...
declare interface CreateBatchInformationExtractionTaskRequest {
  ...
  Operator: UserInfo;
  /** 合同智能提取的文件资源编号列表，通过UploadFiles接口获取PDF、WORD、PNG、JPEG文件资源编号。注意：此接口仅支持5个文件发起PDF、WORD文件限制在10M以内PNG、JPEG文件限制在5M以内 */
  ResourceIds: string[];
  /** 代理企业和员工的信息。在集团企业代理子企业操作的场景中，需设置此参数。在此情境下，ProxyOrganizationId（子企业的组织ID）为必填项。 */
  Agent?: Agent;
  /** 用户配置的合同智能提取字段模板ID，会基于此模板批量创建合同智能提取任务，为32位字符串。点击查看模板Id在控制台上的位置注: 此配置优先级最高，设置了模板ID后Fields配置就会无效 */
...
declare interface CreateBatchQuickSignUrlRequest {
  ...
  /** 预设的动态签署方的补充信息，仅匹配对应信息的签署方才能领取合同。暂时仅对个人参与方生效。若为个人动态签署方，支持预设姓名、证件、手机号。若为企业员工动态签署方，仅支持预设企业名称。 */
...
declare interface CreateFlowByFilesRequest {
  ...
  FlowDescription?: string;
  /** 合同流程的类别分类（可自定义名称，如销售合同/入职合同等），最大长度为200个字符，仅限中文、字母、数字和下划线组成。如果用户已经在控制台创建了自定义合同类型，可以将这里的类型名称传入。 如果没有创建，我们会自动给发起方公司创建此自定义合同类型。 */
  FlowType?: string;
  /** 发起的时候合成到PDF上的填写控件（需要提供填写控件的内容） 单行文本控件 多行文本控件 勾选框控件 数字控件 图片控件 水印控件等 */
  Components?: Component[];
  ...
  RemindedOn?: number;
  /** 指定个人签署方查看合同的校验方式 VerifyCheck :（默认）人脸识别,人脸识别后才能查看合同内容 MobileCheck : 手机号验证, 用户手机号和参与方手机号（ApproverMobile）相同即可查看合同内容（当手写签名方式为OCR_ESIGN时，该校验方式无效，因为这种签名方式依赖实名认证） */
  ApproverVerifyType?: string;
...
declare interface CreateFlowByFilesResponse {
  /** 合同流程ID，为32位字符串。建议开发者妥善保存此流程ID，以便于顺利进行后续操作。注: 如果是预览模式(即NeedPreview设置为true)时, 此处不会有值返回。点击查看FlowId在控制台中的位置 */
  FlowId?: string;
...
declare interface CreateFlowRemindsRequest {
  ...
  /** 代理企业和员工的信息。在集团企业代理子企业操作的场景中，需设置此参数。在此情境下，ProxyOrganizationId（子企业的组织ID）为必填项。 */
  Agent?: Agent;
  /** 指定通知方式，枚举值：SMS / EMAIL；为空则按签署人原始 NotifyType 处理 */
  RemindTypes?: string[];
  /** 邮箱覆盖列表，为指定签署人使用特定邮箱发送邮件催办 */
  RemindEmailInfos?: RemindEmailInfo[];
}
...
declare interface CreateFlowRequest {
  ...
  /** 自定义的合同流程的名称，长度不能超过200个字符，只能由中文汉字、中文标点、英文字母、阿拉伯数字、空格、小括号、中括号、中划线、下划线以及（,）、（;）、（.）、(&amp;)、（+）组成。该名称还将用于合同签署完成后文件下载的默认文件名称。 */
  FlowName: string;
  /** 合同流程的参与方列表，最多可支持50个参与方，可在列表中指定企业B端签署方和个人C端签署方的联系和认证方式等信息，具体定义可以参考开发者中心的ApproverInfo结构体。注: 在发起流程时，需要保证 approver 中的顺序与模板定义顺序一致，否则会发起失败。例如，如果模板中定义的第一个参与人是个人用户，第二个参与人是企业员工，则在 approver 中传参时，第一个也必须是个人用户，第二个参与人必须是企业员工。点击查看模板参与人顺序定义位置 */
  ...
  /** 合同到期提醒时间，为Unix标准时间戳（秒）格式，支持的范围是从发起时间开始到后10年内。到达提醒时间后，腾讯电子签会短信通知发起方企业合同提醒，可用于处理合同到期事务，如合同续签等事宜。 */
  RemindedOn?: number;
  /** 调用方自定义的个性化字段(可自定义此名称)，并以base64格式编码，支持的最大数据大小为 20480长度。在合同状态变更的回调信息等场景中，该字段的信息将原封不动地透传给贵方。回调的相关说明可参考开发者中心的回调通知模块。 */
  UserData?: string;
...
declare interface CreateFlowResponse {
  /** 合同流程ID，为32位字符串。建议开发者妥善保存此流程ID，以便于顺利进行后续操作。注:此返回的合同流程ID，需再次调用创建电子文档和发起签署流程接口将合同开始后，合同才能进入签署环节，点击查看FlowId在控制台中的位置（只在进入签署环节后有效） */
  FlowId?: string;
  /** 唯一请求 ID，每次请求都会返回。 */
  RequestId?: string;
}
...
declare interface CreateFlowSignUrlRequest {
  ...
  JumpUrl?: string;
  /** 链接类型支持以下指定类型：0: 签署链接（默认值），进入后可以填写或签署合同。1 : 预览链接，进入后可以预览合同当前的样子。注：当指定链接类型为1时，链接为预览链接，打开链接后无法进行签署操作，仅支持预览和查看当前合同状态。如需生成发起方预览链接，则签署方信息应传空，即FlowApproverInfos传空或者不传。 */
  UrlType?: number;
...
declare interface CreateSealPolicyRequest {
  /** 执行本接口操作的员工信息。注: 在调用此接口时，请确保指定的员工已获得所需的接口调用权限，并具备接口传入的相应资源的数据权限。 */
  Operator: UserInfo;
  /** 用户在电子文件签署平台标识信息，具体参考UserInfo结构体。可跟下面的UserIds可叠加起作用,同时也支持使用主企业的User进行授权， 当使用主企业的User时， 需要有集团角色。 */
  Users: UserInfo[];
  ...
  Expired: number;
  /** 需要授权的用户UserId集合。跟上面的SealId参数配合使用。选填，跟上面的Users同时起作用。支持使用主企业的UserId进行授权， 当使用主企业的UserId时，该UserId 需要有集团角色。 */
  UserIds?: string[];
  /** 印章授权内容，最多300 个字符。 */
  Policy?: string;
  /** 代理企业和员工的信息。在集团企业代理子企业操作的场景中，需设置此参数。在此情境下，ProxyOrganizationId（子企业的组织ID）为必填项。 */
  ...
  /** 个性化配置字段，默认不传。 */
  Options?: Option[];
  /** 针对用印的合同/合同组授权。当单次用印授权时，可以仅在指定合同获得印章授权。此场景下不会触发印章授权回调。 */
...
declare interface DescribeCancelFlowsTaskRequest {
  /** 执行本接口操作的员工信息。注: `在调用此接口时，请确保指定的员工已获得所需的接口调用权限，并具备接口传入的相应资源的数据权限。 */
  Operator: UserInfo;
  /** 批量撤销任务编号，为32位字符串，通过接口获取批量撤销签署流程腾讯电子签小程序链接和接口全量撤销企业合同获得。 */
  TaskId: string;
  /** 代理企业和员工的信息。在集团企业代理子企业操作的场景中，需设置此参数。在此情境下，ProxyOrganizationId（子企业的组织ID）为必填项。 */
  Agent?: Agent;
  /** 撤销任务类型枚举值：0： 默认类型，获取批量撤销合同小程序链接的批量撤销任务1： 全量撤销任务枚举值：0： 默认类型，批量撤销合同任务或者获取批量撤销合同小程序链接的批量撤销任务1： 全量撤销任务枚举值：0： 默认类型，获取批量撤销合同小程序链接的批量撤销任务1： 全量撤销任务 */
  CancelType?: number;
}

declare interface DescribeCancelFlowsTaskResponse {
  /** 批量撤销任务编号，为32位字符串，通过接口获取批量撤销签署流程腾讯电子签小程序链接获得。 */
  TaskId?: string;
...
declare interface DescribeContractReviewTaskResponse {
  /** 该字段已不再使用！用于审查任务的审查清单ID。注意：如果用户没有配置清单时此值可能为空，需要等大模型根据合同内容推荐出可以使用的审查清单。 */
  ChecklistId?: string;
  ...
  Risks?: OutputRisk[];
  /** 该字段已不再使用！合同审查中的角色信息。注意： `如果用户没有配置审查角色时此值可能为null，需要等大模型根据合同内容推荐出审查角色信息。` */
...
declare interface DescribeEnterpriseContractReviewChecklistsResponse {
  /** 查询的总条数 */
  Total?: number;
  /** 清单列表 */
...
declare interface DescribeIntegrationEmployeesRequest {
  /** 执行本接口操作的员工信息。使用此接口时，必须填写UserId。注: `在调用此接口时，请确保指定的员工已获得所需的接口调用权限，并具备接口传入的相应资源的数据权限。` */
  Operator: UserInfo;
  /** 指定分页每页返回的数据条数，单页最大支持 20。如果没有传递， 则为默认值20。 */
  Limit?: number;
  /** 代理企业和员工的信息。在集团企业代理子企业操作的场景中，需设置此参数。在此情境下，ProxyOrganizationId（子企业的组织ID）为必填项。 */
  Agent?: Agent;
...
declare interface DescribeOrganizationAuthStatusRequest {
  ...
  /** 法人姓名 */
  LegalName?: string;
  /** 认证流 Id 是指在企业认证过程中，当前操作人的认证流程的唯一标识。每个企业在认证过程中只能有一条认证流认证成功。这意味着在同一认证过程内，一个企业只能有一个认证流程处于成功状态，以确保认证的唯一性和有效性。认证流 Id可以通过回调[授权书认证审核结果回调](https://qian.tencent.com/developers/company/callback_types_staffs/#%E5%8D%81%E5%85%AD-%E6%8E%88%E6%9D%83%E4%B9%A6%E8%AE%A4%E8%AF%81%E5%AE%A1%E6%A0%B8%E7%BB%93%E6%9E%9C%E5%9B%9E%E8%B0%83)获取注意：如果传递了认证流Id，则下面的参数 超管二要素不会生效示例值：yDCHHUUckpbdaiqbUxJVsHWy99WG6kTY */
  AuthorizationInfoId?: string;
}
...
declare interface ExportContractReviewResultRequest {
  ...
  /** 合同审查任务ID */
  TaskId: string;
  /** 导出文件类型枚举值：1： WORD、PDF当前带风险批注文件2： 审查结果＆摘要（.xIsx）3： WORD、PDF审查合同内容时的文件（最原始文件）4： WORD、PDF当前无风险批注文件 */
  FileType: number;
  /** 代理企业和员工的信息。在集团企业代理子企业操作的场景中，需设置此参数。在此情境下，ProxyOrganizationId（子企业的组织ID）为必填项。 */
  Agent?: Agent;
}
...
declare interface OperateFlowRemarksRequest {
  /** 执行本接口操作的员工信息。注: 在调用此接口时，请确保指定的员工已获得所需的接口调用权限，并具备接口传入的相应资源的数据权限。 */
  Operator: UserInfo;
  /** 指定对合同备注的操作枚举值：CREATE： 创建合同备注UPDATE： 更新合同备注DELETE： 删除合同备注 */
  OperateType: string;
  /** 对应的合同流程id目标合同为合同组时此参数不填 */
  FlowId?: string;
  /** 代理企业和员工的信息。在集团企业代理子企业操作的场景中，需设置此参数。在此情境下，ProxyOrganizationId（子企业的组织ID）为必填项。 */
  Agent?: Agent;
  /** 对应合同组id目标合同为单份合同时此参数不填 */
  FlowGroupId?: string;
  /** 合同备注信息。入参限制：当OperateType为UPDATE和DELETE时，通过该结构体进行对应备注操作，合同备注数量范围为 1 - 5个。 */
  FlowItem?: FlowRemarkItem;
  /** 合同备注列表。入参限制：当OperateType为CREATE时，通过该参数进行全量的合同备注创建。 */
  FlowItems?: string[];
}

declare interface OperateFlowRemarksResponse {
  /** 唯一请求 ID，每次请求都会返回。 */
  RequestId?: string;
}
...
declare interface Ess {
  ...
  /** 合同备注管理 {@link OperateFlowRemarksRequest} {@link OperateFlowRemarksResponse} */
  OperateFlowRemarks(data: OperateFlowRemarksRequest, config?: AxiosRequestConfig): AxiosPromise<OperateFlowRemarksResponse>;
