...
declare interface ApproverInfo {
  ...
  /** 组织机构名称。如果签署方是企业签署方(approverType = 1 或者 approverType = 3)， 则企业名称必填。注: 请确认该名称与企业营业执照中注册的名称一致 ; 如果名称中包含英文括号()，请使用中文括号（）代替。 */
  OrganizationName?: string;
  /** 合同中的签署控件列表，列表中可支持下列多种签署控件,控件的详细定义参考开发者中心的Component结构体- 个人签名/印章- 企业印章- 骑缝章等签署控件 */
  SignComponents?: Component[];
  /** 签署方经办人的证件类型，支持以下类型- ID_CARD 居民身份证 (默认值)- HONGKONG_AND_MACAO 港澳居民来往内地通行证- HONGKONG_MACAO_AND_TAIWAN 港澳台居民居住证(格式同居民身份证)- OTHER_CARD_TYPE 其他证件注: 其他证件类型为白名单功能，使用前请联系对接的客户经理沟通。 */
  ApproverIdCardType?: string;
  /** 签署方经办人的证件号码，应符合以下规则- 居民身份证号码应为18位字符串，由数字和大写字母X组成（如存在X，请大写）。- 港澳居民来往内地通行证号码应为9位字符串，第1位为“C”，第2位为英文字母（但“I”、“O”除外），后7位为阿拉伯数字。- 港澳台居民居住证号码编码规则与中国大陆身份证相同，应为18位字符串。 */
  ...
  /** 指定个人签署方查看合同的校验方式,可以传值如下:- 1 : （默认）人脸识别,人脸识别后才能合同内容- 2 : 手机号验证, 用户手机号和参与方手机号(ApproverMobile)相同即可查看合同内容注: - 如果合同流程设置ApproverVerifyType查看合同的校验方式, 则忽略此签署人的查看合同的校验方式- 此字段不可传多个校验方式 */
...
declare interface AutoSignConfig {
  ...
  /** 接受回调URL地址。支持http://或者https://协议Post数据到此地址后返回httpcode200表示接受回调成功, 返回其他httpcode表示接受回调失败 */
  CallbackUrl: string;
  /** 是否回调证书信息false-不需要 (默认值)true-需要 */
  CertInfoCallback?: boolean;
  /** 是否支持用户自定义签名印章false-不需要(默认)true-需要 */
  UserDefineSeal?: boolean;
  /** 是否需要回调的时候返回印章(签名) 图片的 base64false-不需要(默认)true-需要 */
  SealImgCallback?: boolean;
  /** 开通时候的验证方式, 分布为WEIXINAPP : 微信人脸识别INSIGHT : 慧眼人脸认别TELECOM : 运营商三要素验证如果是小程序开通链接，支持传 WEIXINAPP / TELECOM。如果是 H5 开通链接，支持传 INSIGHT / TELECOM。默认值 WEIXINAPP / INSIGHT。 */
...
declare interface FlowCreateApprover {
  ...
  ApproverName?: string;
  /** 签署方经办人手机号码在未指定签署人电子签UserId情况下，为必填参数 */
  ApproverMobile?: string;
  /** 签署人的证件类型ID_CARD 身份证HONGKONG_AND_MACAO 港澳居民来往内地通行证HONGKONG_MACAO_AND_TAIWAN 港澳台居民居住证(格式同居民身份证)OTHER_CARD_TYPE 其他（需要使用该类型请先联系运营经理） */
  ApproverIdCardType?: string;
  /** 签署人证件号（长度不超过18位） */
  ApproverIdCardNumber?: string;
  ...
  VerifyChannel?: string[];
  /** 是否发送短信sms--短信通知none--不通知默认为sms发起方=签署方时不发送短信 */
  NotifyType?: string;
  /** 合同强制需要阅读全文，无需传此参数 */
  IsFullText?: boolean;
...
declare interface CreateFlowByFilesRequest {
  /** 执行本接口操作的员工信息。使用此接口时，必须填写userId。在调用此接口时，请确保指定的员工已获得所需的接口调用权限，并具备接口传入的相应资源的数据权限。支持填入集团子公司经办人 userId 代发合同。 */
  Operator: UserInfo;
  ...
  NeedPreview?: boolean;
  /** 预览模式下产生的预览链接类型 - 0 :(默认) 文件流 ,点开后后下载预览的合同PDF文件 - 1 :H5链接 ,点开后在浏览器中展示合同的样子注意: 此参数在NeedPreview 为true时有效 */
  PreviewType?: number;
  ...
  Unordered?: boolean;
  /** 您可以自定义腾讯电子签小程序合同列表页展示的合同内容模板，模板中支持以下变量：- {合同名称}- {发起方企业}- {发起方姓名}- {签署方N企业}- {签署方N姓名}其中，N表示签署方的编号，从1开始，不能超过签署人的数量。例如，如果是腾讯公司张三发给李四名称为“租房合同”的合同，您可以将此字段设置为：合同名称:`合同名称:{合同名称};发起方: {发起方企业}({发起方姓名});签署方:{签署方1姓名}`则小程序中列表页展示此合同为以下样子```合同名称：租房合同 发起方：腾讯公司(张三) 签署方：李四``` */
  CustomShowMap?: string;
  /** 发起方企业的签署人进行签署操作前，是否需要企业内部走审批流程，取值如下：- false：（默认）不需要审批，直接签署。- true：需要走审批流程。当到对应参与人签署时，会阻塞其签署操作，等待企业内部审批完成。企业可以通过CreateFlowSignReview审批接口通知腾讯电子签平台企业内部审批结果- 如果企业通知腾讯电子签平台审核通过，签署方可继续签署动作。- 如果企业通知腾讯电子签平台审核未通过，平台将继续阻塞签署方的签署动作，直到企业通知平台审核通过。注：此功能可用于与企业内部的审批流程进行关联，支持手动、静默签署合同 */
  NeedSignReview?: boolean;
  /** 调用方自定义的个性化字段(可自定义此名称)，并以base64方式编码，支持的最大数据大小为 20480长度。在合同状态变更的回调信息等场景中，该字段的信息将原封不动地透传给贵方。回调的相关说明可参考开发者中心的"回调通知"模块。 */
  UserData?: string;
  /** 指定个人签署方查看合同的校验方式- VerifyCheck :（默认）人脸识别,人脸识别后才能合同内容- MobileCheck: 手机号验证, 用户手机号和参与方手机号(ApproverMobile)相同即可查看合同内容 */
  ApproverVerifyType?: string;
...
declare interface CreateFlowSignUrlRequest {
  /** 流程编号 */
  FlowId: string;
  /** 流程签署人列表，其中结构体的ApproverName，ApproverMobile和ApproverType必传，其他可不传，ApproverType目前只支持个人类型的签署人。签署人只能有手写签名和时间类型的签署控件，其他类型的填写控件和签署控件暂时都未支持。 */
...
declare interface CreateUserAutoSignEnableUrlRequest {
  ...
  /** 链接类型，空-默认小程序端链接H5SIGN-h5端链接 */
  UrlType?: string;
  /** 通知类型默认不设置为不通知开通方，SMS 为短信通知 , 此种方式需要NotifyAddress填写手机号。 */
  NotifyType?: string;
...
declare interface DescribeFlowEvidenceReportResponse {
  /** 出证报告PDF的下载 URL */
  ReportUrl?: string | null;
  /** 出证任务执行的状态, 分布表示下面的含义EvidenceStatusExecuting 出证任务在执行中EvidenceStatusSuccess 出证任务执行成功EvidenceStatusFailed 出证任务执行失败 */
  Status?: string;
  /** 唯一请求 ID，每次请求都会返回。 */
  RequestId?: string;
}
...
declare interface DescribeUserAutoSignStatusResponse {
  /** 查询用户是否已开通自动签 */
  IsOpen?: boolean;
  /** 自动签许可生效时间。当且仅当已开通自动签时有值。值为unix时间戳,单位为秒。 */
  LicenseFrom?: number;
  /** 自动签许可到期时间。当且仅当已开通自动签时有值。值为unix时间戳,单位为秒。 */
  LicenseTo?: number;
  /** 唯一请求 ID，每次请求都会返回。 */
  RequestId?: string;
}
