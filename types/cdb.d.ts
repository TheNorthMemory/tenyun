...
declare interface DescribeDBPriceRequest {
  ...
  GoodsNum?: number;
  /** 实例内存大小，单位：MB。InstanceId 为空时该参数为必填项。为保证传入值有效，请使用 获取云数据库可售卖规格 接口获取可售卖的实例内存大小范围。 */
  Memory?: number;
  /** 实例硬盘大小，单位：GB。InstanceId 为空时该参数为必填项。为保证传入值有效，请使用 获取云数据库可售卖规格 接口获取可售卖的硬盘大小范围。 */
  Volume?: number;
  ...
  /** 数据复制方式，默认为 0，支持值包括：0 - 表示异步复制，1 - 表示半同步复制，2 - 表示强同步复制。 */
  ProtectMode?: number;
  /** 实例隔离类型。枚举值：UNIVERSAL： 通用型实例EXCLUSIVE： 独享型实例CLOUD_NATIVE_CLUSTER： 云盘版标准型CLOUD_NATIVE_CLUSTER_EXCLUSIVE： 云盘版加强型CLOUD_NATIVE_CLUSTER_ULTRA： 云盘版旗舰型默认值：UNIVERSAL如需查询单节点云盘版实例的价格，请设置此参数为 CLOUD_NATIVE_CLUSTER，并且指定参数 InstanceNodes 为1。 */
  DeviceType?: string;
  /** 实例节点数。1.查询 RO 实例或者单节点实例价格时，此字段值为1。2.查询双节点实例价格时，此字段值为2。3.查询三节点实例价格时，此字段值为3。4.查询云盘版实例价格时，此字段值范围可输入2 - 6，取值为2表示云盘版实例下对应有1个读写节点 + 1个只读节点；取值为6表示云盘版实例下对应有1个读写节点 + 5个只读节点；其余取值（3 - 5）按1个读写节点 +（取值数 - 1）个只读节点规则类推。 */
  InstanceNodes?: number;
  /** 询价实例的CPU核心数目，单位：核，为保证传入 CPU 值有效，请使用 获取云数据库可售卖规格 接口获取可售卖的核心数目，当未指定该值时，将按照 Memory 大小补全一个默认值。 */
  Cpu?: number;
...
declare interface Cdb {
  ...
  ModifyDBInstanceModes(data: ModifyDBInstanceModesRequest, config?: AxiosRequestConfig): AxiosPromise<ModifyDBInstanceModesResponse>;
