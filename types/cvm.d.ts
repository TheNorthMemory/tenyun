...
declare interface DisasterRecoverGroup {
  ...
  /** 置放群组亲和度 */
  Affinity?: number;
  /** 置放群组类型，当前支持两种；分散置放群组和分区置放群组(功能灰度中)，取值范围：PARTITION：分区置放群组，SPREAD：分散置放群组 */
  Strategy?: string;
  /** 分区置放群组的分区数量，如果是分区置放群组会返回(功能灰度中) */
...
declare interface Instance {
  ...
  /** 分区置放群组的分区号，具体取决于所选置放群组的分区数量(功能灰度中) */
...
declare interface ZoneInfo {
  /** 可用区名称，例如，ap-guangzhou-3全网可用区名称如下： ap-chongqing-1 ap-seoul-1 ap-seoul-2 ap-chengdu-1 ap-chengdu-2 ap-hongkong-1（售罄） ap-hongkong-2 ap-hongkong-3 ap-shenzhen-fsi-1 ap-shenzhen-fsi-2 ap-shenzhen-fsi-3（售罄） ap-guangzhou-1（售罄） ap-guangzhou-2（售罄） ap-guangzhou-3 ap-guangzhou-4 ap-guangzhou-6 ap-guangzhou-7 ap-tokyo-1 ap-tokyo-2 ap-singapore-1 ap-singapore-2 ap-singapore-3 ap-singapore-4 ap-shanghai-fsi-2 ap-shanghai-fsi-3 ap-shanghai-fsi-4 ap-bangkok-1 ap-bangkok-2 ap-shanghai-2 ap-shanghai-3 ap-shanghai-4 ap-shanghai-5 ap-shanghai-8 ap-mumbai-1 ap-mumbai-2 ap-beijing-1（售罄） ap-beijing-3 ap-beijing-4 ap-beijing-5 ap-beijing-6 ap-beijing-7 na-siliconvalley-1 na-siliconvalley-2 eu-frankfurt-1 eu-frankfurt-2 na-ashburn-1 na-ashburn-2 ap-nanjing-1 ap-nanjing-2 ap-nanjing-3 sa-saopaulo-1 ap-jakarta-1 ap-jakarta-2 */
  Zone?: string;
  /** 可用区描述，例如，广州三区 */
  ZoneName?: string;
  /** 可用区ID */
...
declare interface CreateLaunchTemplateRequest {
  ...
  Placement: Placement;
  /** 指定有效的镜像ID，格式形如img-xxx。镜像类型分为四种：公共镜像自定义镜像共享镜像服务市场镜像可通过以下方式获取可用的镜像ID：公共镜像、自定义镜像、共享镜像的镜像ID可通过登录控制台查询；服务镜像市场的镜像ID可通过云市场查询。通过调用接口 DescribeImages ，传入InstanceType获取当前机型支持的镜像列表，取返回信息中的ImageId字段。 */
  ImageId: string;
  /** 实例启动模板版本描述。长度为2~256个英文或中文字符，默认为空字符。 */
  LaunchTemplateVersionDescription?: string;
  /** 实例机型。不同实例机型指定了不同的资源规格。对于付费模式为PREPAID或POSTPAID_BY_HOUR的实例创建，具体取值可通过调用接口DescribeInstanceTypeConfigs来获得最新的规格表或参见实例规格描述。若不指定该参数，则系统将根据当前地域的资源售卖情况动态指定默认机型。对于付费模式为CDHPAID的实例创建，该参数以"CDH_"为前缀，根据CPU和内存配置生成，具体形式为：CDH_XCXG，例如对于创建CPU为1核，内存为1G大小的专用宿主机的实例，该参数应该为CDH_1C1G。 */
  InstanceType?: string;
  /** 实例系统盘配置信息。若不指定该参数，则按照系统默认值进行分配。 */
  SystemDisk?: SystemDisk;
  /** 实例数据盘配置信息。若不指定该参数，则默认不购买数据盘。支持购买的时候指定21块数据盘，其中最多包含1块LOCAL_BASIC数据盘或者LOCAL_SSD数据盘，最多包含20块CLOUD_BASIC数据盘、CLOUD_PREMIUM数据盘或者CLOUD_SSD数据盘，详情请参考 云硬盘使用限制。 */
  DataDisks?: DataDisk[];
  ...
  InternetAccessible?: InternetAccessible;
  /** 购买实例数量。默认取值：1。指定购买实例的数量不能超过用户所能购买的剩余配额数量，具体配额相关限制详见CVM实例购买限制。 */
  InstanceCount?: number;
  /** 实例显示名称。不指定实例显示名称则默认显示‘未命名’。购买多台实例，如果指定模式串{R:x}，表示生成数字[x, x+n-1]，其中n表示购买实例的数量，例如server_{R:3}，购买1台时，实例显示名称为server_3；购买2台时，实例显示名称分别为server_3，server_4。支持指定多个模式串{R:x}。购买多台实例，如果不指定模式串，则在实例显示名称添加后缀1、2...n，其中n表示购买实例的数量，例如server_，购买2台时，实例显示名称分别为server_1，server_2。最多支持128个字符（包含模式串）。 */
  InstanceName?: string;
  /** 实例登录设置。通过该参数可以设置实例的登录方式为密钥或保持镜像的原始登录设置。 */
  LoginSettings?: LoginSettings;
  /** 实例所属安全组。该参数可以通过调用 DescribeSecurityGroups 的返回值中的 SecurityGroupId 字段来获取。若不指定该参数，则绑定指定项目下的默认安全组，如默认安全组不存在则将自动创建。 */
  SecurityGroupIds?: string[];
  ...
  /** 定时任务。通过该参数可以为实例指定定时任务，目前仅支持定时销毁。 */
  ActionTimer?: ActionTimer;
  /** 置放群组id，仅支持指定一个。该参数可以通过调用 DescribeDisasterRecoverGroups 的返回值中的 DisasterRecoverGroupId 字段来获取。 */
  DisasterRecoverGroupIds?: string[];
  ...
  DryRun?: boolean;
  /** CAM角色名称。可通过 DescribeRoleList 接口返回值中的RoleName获取。 */
  CamRoleName?: string;
  /** 高性能计算集群ID。若创建的实例为高性能计算实例，需指定实例放置的集群，否则不可指定。该参数可以通过调用 DescribeHpcClusters 的返回值中的 HpcClusterId 字段来获取。 */
  HpcClusterId?: string;
...
declare interface CreateLaunchTemplateVersionRequest {
  /** 实例所在的位置。通过该参数可以指定实例所属可用区，所属项目，所属宿主机（在专用宿主机上创建子机时指定）等属性。 */
  Placement: Placement;
  /** 启动模板ID，新版本将基于该实例启动模板ID创建。可通过 DescribeLaunchTemplates 接口返回值中的LaunchTemplateId获取。 */
  LaunchTemplateId: string;
  /** 若给定，新实例启动模板将基于给定的版本号创建。若未指定则使用默认版本,可以通过 DescribeLaunchTemplateVersions查询默认版本。 */
  LaunchTemplateVersion?: number;
  /** 实例启动模板版本描述。长度为2~256个英文或中文字符，不指定该参数时默认为空字符。 */
  LaunchTemplateVersionDescription?: string;
  /** 实例机型。不同实例机型指定了不同的资源规格。对于付费模式为PREPAID或POSTPAID_BY_HOUR的实例创建，具体取值可通过调用接口DescribeInstanceTypeConfigs来获得最新的规格表或参见实例规格描述。若不指定该参数，则系统将根据当前地域的资源售卖情况动态指定默认机型。对于付费模式为CDHPAID的实例创建，该参数以"CDH_"为前缀，根据CPU和内存配置生成，具体形式为：CDH_XCXG，例如对于创建CPU为1核，内存为1G大小的专用宿主机的实例，该参数应该为CDH_1C1G。 */
  InstanceType?: string;
  /** 指定有效的镜像ID，格式形如img-xxx。镜像类型分为四种：公共镜像自定义镜像共享镜像云镜像市场可通过以下方式获取可用的镜像ID：公共镜像、自定义镜像、共享镜像的镜像ID可通过登录控制台查询；云镜像市场的镜像ID可通过云市场查询。通过调用接口 DescribeImages ，传入InstanceType获取当前机型支持的镜像列表，取返回信息中的ImageId字段。 */
  ImageId?: string;
  /** 实例系统盘配置信息。若不指定该参数，则按照系统默认值进行分配。 */
  SystemDisk?: SystemDisk;
  ...
  /** 定时任务。通过该参数可以为实例指定定时任务，目前仅支持定时销毁。 */
  ActionTimer?: ActionTimer;
  /** 置放群组id，仅支持指定一个。可使用DescribeDisasterRecoverGroups接口获取。 */
  DisasterRecoverGroupIds?: string[];
  /** 标签描述列表。通过指定该参数可以同时绑定标签到相应的资源实例，当前仅支持绑定标签到云服务器实例。 */
  TagSpecification?: TagSpecification[];
  /** 实例的市场相关选项，如竞价实例相关参数，若指定实例的付费模式为竞价付费则该参数必传。 */
...
declare interface DeleteImagesRequest {
  /** 删除的镜像 ID 列表。可通过 [DescribeImages](https://cloud.tencent.com/document/api/213/15715) 接口返回值中的`ImageId`获取。 */
  ImageIds: string[];
  /** 是否删除镜像关联的快照。默认值：false */
  DeleteBindedSnap?: boolean;
  /** 检测是否支持删除镜像。默认值：false */
  DryRun?: boolean;
}
...
declare interface DescribeInstancesModificationRequest {
  ...
  InstanceIds: string[];
  /** status按照【配置规格状态】进行过滤。配置规格状态形如：SELL(表示实例可售卖)、UNAVAILABLE(表示实例不可用)。类型：String必选：否每次请求的`Filters`的上限为10，`Filter.Values`的上限为2。 */
  Filters?: Filter[];
}
...
declare interface DescribeInstancesOperationLimitRequest {
  ...
  InstanceIds: string[];
  /** 实例操作。 INSTANCE_DEGRADE：实例降配操作 INTERNET_CHARGE_TYPE_CHANGE：实例调整带宽付费模式操作 */
  Operation: string;
}
...
declare interface ExportImagesRequest {
  /** COS存储桶名称。可通过 [List Buckets](https://cloud.tencent.com/document/product/436/8291) 接口查询请求者名下的所有存储桶列表或特定地域下的存储桶列表。 */
  BucketName: string;
  /** 镜像ID列表。调用 ExportImages 接口时，参数 ImageIds 和 SnapshotIds 为二选一必填参数，目前参数 SnapshotIds 暂未对外开放。可通过 [DescribeImages](https://cloud.tencent.com/document/api/213/15715) 接口返回值中的`ImageId`获取。 */
  ImageIds?: string[];
...
declare interface ImportImageRequest {
  /** 导入镜像的操作系统架构。取值范围包括：`x86_64` 、`i386`、`arm_64` */
  Architecture: string;
  /** 导入镜像的操作系统类型 。可通过 [DescribeImportImageOs](https://cloud.tencent.com/document/api/213/15718) 接口返回值中的`ImportImageOsListSupported`获取。 */
  OsType: string;
  /** 导入镜像的操作系统版本。可通过 [DescribeImportImageOs](https://cloud.tencent.com/document/api/213/15718) 接口返回值中的`ImportImageOsVersionSet`获取。 */
  OsVersion: string;
  /** 导入镜像存放的cos地址 */
  ImageUrl: string;
  /** 镜像名称。最多支持 60 个字符。 */
  ...
  /** 镜像描述。最多支持 256 个字符。 */
  ImageDescription?: string;
  /** 只检查参数，不执行任务。默认值：false */
  DryRun?: boolean;
  /** 是否强制导入，参考[强制导入镜像](https://cloud.tencent.com/document/product/213/12849)默认值：false */
  Force?: boolean;
  /** 标签描述列表。通过指定该参数可以同时绑定标签到自定义镜像。 */
  TagSpecification?: TagSpecification[];
  /** 导入镜像后，激活操作系统采用的许可证类型。默认值：TencentCloud取值范围：TencentCloud: 腾讯云官方许可BYOL: 自带许可（Bring Your Own License） */
  LicenseType?: string;
...
declare interface InquiryPriceRunInstancesRequest {
  ...
  Placement: Placement;
  /** 指定有效的镜像ID，格式形如img-xxx。镜像类型分为四种：公共镜像自定义镜像共享镜像服务市场镜像可通过以下方式获取可用的镜像ID：公共镜像、自定义镜像、共享镜像的镜像ID可通过登录控制台查询；服务镜像市场的镜像ID可通过云市场查询。通过调用接口 DescribeImages ，取返回信息中的ImageId字段。 注：如果您不指定LaunchTemplate参数，则ImageId为必选参数。若同时传递ImageId和LaunchTemplate，则默认覆盖LaunchTemplate中对应的ImageId的值。 */
  ImageId: string;
  /** 实例计费类型。PREPAID：预付费，即包年包月POSTPAID_BY_HOUR：按小时后付费SPOTPAID：竞价付费默认值：POSTPAID_BY_HOUR。 */
  InstanceChargeType?: string;
  /** 预付费模式，即包年包月相关参数设置。通过该参数可以指定包年包月实例的购买时长、是否设置自动续费等属性。若指定实例的付费模式为预付费则该参数必传。 */
  InstanceChargePrepaid?: InstanceChargePrepaid;
  /** 实例机型。不同实例机型指定了不同的资源规格。对于付费模式为PREPAID或POSTPAID_BY_HOUR的实例创建，具体取值可通过调用接口DescribeInstanceTypeConfigs来获得最新的规格表或参见实例规格描述。若不指定该参数，则系统将根据当前地域的资源售卖情况动态指定默认机型。对于付费模式为CDHPAID的实例创建，该参数以"CDH_"为前缀，根据CPU和内存配置生成，具体形式为：CDH_XCXG，例如对于创建CPU为1核，内存为1G大小的专用宿主机的实例，该参数应该为CDH_1C1G。 */
...
declare interface ModifyDisasterRecoverGroupAttributeRequest {
  ...
  /** 分散置放群组名称，长度1-60个字符，支持中、英文。 */
  Name: string;
  /** 分散置放群组的亲和度，在置放群组的实例会按该亲和度分布，亲和度的取值范围是：1-10。取值范围：[1, 10]调整只能从小调到大，不能从大调到小 */
  Affinity?: number;
}
...
declare interface ModifyInstancesAttributeRequest {
  ...
  InstanceName?: string;
  /** 提供给实例使用的用户数据，需要以 base64 方式编码，支持的最大数据大小为 16 KB。关于获取此参数的详细介绍，请参阅 Windows 和 Linux 启动时运行命令。 */
  UserData?: string;
...
declare interface ModifyKeyPairAttributeRequest {
  /** 密钥对ID。可以通过以下方式获取可用的密钥 ID：通过登录[控制台](https://console.cloud.tencent.com/cvm/sshkey)查询密钥 ID。通过调用接口 [DescribeKeyPairs](https://cloud.tencent.com/document/api/213/9403) ，取返回信息中的 `KeyId` 获取密钥对 ID。 */
  KeyId: string;
...
declare interface RunInstancesRequest {
  /** 实例计费类型。PREPAID：预付费，即包年包月POSTPAID_BY_HOUR：按小时后付费CDHPAID：独享子机（基于专用宿主机创建，宿主机部分的资源不收费）SPOTPAID：竞价付费CDCPAID：专用集群付费默认值：POSTPAID_BY_HOUR。 */
  InstanceChargeType?: string;
  /** 预付费模式，即包年包月相关参数设置。通过该参数可以指定包年包月实例的购买时长、是否设置自动续费等属性。若指定实例的付费模式为预付费则该参数必传。 */
  InstanceChargePrepaid?: InstanceChargePrepaid;
  ...
  /** 实例机型。不同实例机型指定了不同的资源规格。对于付费模式为PREPAID或POSTPAID_BY_HOUR的实例创建，具体取值可通过调用接口DescribeInstanceTypeConfigs来获得最新的规格表或参见实例规格描述。若不指定该参数，则系统将根据当前地域的资源售卖情况动态指定默认机型。对于付费模式为CDHPAID的实例创建，该参数以"CDH_"为前缀，根据CPU和内存配置生成，具体形式为：CDH_XCXG，例如对于创建CPU为1核，内存为1G大小的专用宿主机的实例，该参数应该为CDH_1C1G。 */
  InstanceType?: string;
  /** 指定有效的镜像ID，格式形如img-xxx。镜像类型分为四种：公共镜像自定义镜像共享镜像服务市场镜像可通过以下方式获取可用的镜像ID：公共镜像、自定义镜像、共享镜像的镜像ID可通过登录控制台查询；服务镜像市场的镜像ID可通过云市场查询。通过调用接口 DescribeImages ，传入InstanceType获取当前机型支持的镜像列表，取返回信息中的ImageId字段。 注：如果您不指定LaunchTemplate参数，则ImageId为必选参数。若同时传递ImageId和LaunchTemplate，则默认覆盖LaunchTemplate中对应的ImageId的值。 */
  ImageId?: string;
  /** 实例系统盘配置信息。若不指定该参数，则按照系统默认值进行分配。 */
  SystemDisk?: SystemDisk;
  /** 实例数据盘配置信息。若不指定该参数，则默认不购买数据盘。支持购买的时候指定21块数据盘，其中最多包含1块LOCAL_BASIC数据盘或者LOCAL_SSD数据盘，最多包含20块CLOUD_BASIC数据盘、CLOUD_PREMIUM数据盘或者CLOUD_SSD数据盘。 */
  ...
  /** 公网带宽相关信息设置。若不指定该参数，则默认公网带宽为0Mbps。 */
  InternetAccessible?: InternetAccessible;
  /** 购买实例数量。包年包月实例取值范围：[1，500]，按量计费实例取值范围：[1，500]。默认取值：1。指定购买实例的数量不能超过用户所能购买的剩余配额数量，具体配额相关限制详见CVM实例购买限制。 */
  InstanceCount?: number;
  ...
  MinCount?: number;
  /** 实例显示名称。不指定实例显示名称则默认显示‘未命名’。最多支持128个字符（含模式串）。购买多台实例时：指定模式串 {R:x}：表示生成数字序列 [x, x+n-1]，其中 n为购买实例的数量。例如：输入 server_{R:3}，购买1台时，实例显示名称为 server_3；购买2台时，实例显示名称分别为 server_3、server_4。指定模式串 {R:x,F:y}：y表示固定位数（可选），取值范围为 [0,8]，默认值 0表示不固定位数（等效于 {R:x}）。不足位时自动补零，例如：输入server_{R:3,F:3}，购买2台时，实例显示名称为 server_003、server_004。；若数字位数超过 y（如 {R:99,F:2}），以实际位数为准，例如：app_{R:99,F:2}，购买2台时，实例显示名称为 app_99、app_100。模式串需严格遵循 {R:x,F:y}或 {R:x}格式，无效格式（如 {}）视为普通文本。支持多个模式串。未指定模式串：实例显示名称添加后缀 1、2...n，其中n表示购买实例的数量，例如 server_购买2台时生成 server_1、server_2。 */
  InstanceName?: string;
  /** 实例登录设置。通过该参数可以设置实例的登录方式密码、密钥或保持镜像的原始登录设置。默认情况下会随机生成密码，并以站内信方式知会到用户。 */
  LoginSettings?: LoginSettings;
  /** 实例所属安全组。该参数可以通过调用 DescribeSecurityGroups 的返回值中的 SecurityGroupId 字段来获取。若不指定该参数，则绑定指定项目下的默认安全组，如默认安全组不存在则将自动创建。 */
  ...
  /** 用于保证请求幂等性的字符串。该字符串由客户生成，需保证不同请求之间唯一，最大值不超过64个ASCII字符。若不指定该参数，则无法保证请求的幂等性。 */
  ClientToken?: string;
  /** 实例主机名。点号（.）和短横线（-）不能作为 HostName 的首尾字符，不能连续使用。Windows 实例：主机名名字符长度为[2, 15]，允许字母（不限制大小写）、数字和短横线（-）组成，不支持点号（.），不能全是数字。其他类型（Linux 等）实例：主机名字符长度为[2, 60]，允许支持多个点号，点之间为一段，每段允许字母（不限制大小写）、数字和短横线（-）组成。购买多台实例时：指定模式串 {R:x}：表示生成数字序列 [x, x+n-1]，其中 n为购买实例的数量。例如：输入 server_{R:3}，购买1台时，实例主机名为 server_3；购买2台时，实例主机名分别为 server_3、server_4。指定模式串 {R:x,F:y}：y表示固定位数（可选），取值范围为 [0,8]，默认值 0表示不固定位数（等效于 {R:x}）。不足位时自动补零，例如：输入server_{R:3,F:3}，购买2台时，实例主机名为 server_003、server_004。若数字位数超过 y（如 {R:99,F:2}），以实际位数为准，例如：app_{R:99,F:2}，购买2台时，实例主机名为 app_99、app_100。指定模式串 {IP}：自动替换为实例的内网IP地址。例如：输入 node-{IP}，实例主机名为 node-10.0.12.8；支持与序号模式串混合使用，例如：输入 web-{IP}-{R:1}，购买2台时，实例主机名分别为 web-10.0.12.8-1、web-10.0.12.9-2。模式串需严格遵循 {R:x,F:y}、{R:x} 或 {IP} 格式，无效格式（如 {}）视为普通文本。支持多个模式串。未指定模式串：实例主机名添加后缀 1、2...n，其中n表示购买实例的数量，例如 server_购买2台时生成 server_1、server_2。 */
  HostName?: string;
  /** 定时任务。通过该参数可以为实例指定定时任务，目前仅支持定时销毁。 */
  ActionTimer?: ActionTimer;
  /** 置放群组id，仅支持指定一个。 */
  ...
  /** 指定CHC物理服务器来创建CHC云主机。 */
  ChcIds?: string[];
  /** 分区置放群组的分区序号，具体取决于所选置放群组的分区数量，如果选中的置放群组是分区置放群组，该值不传默认随机。(功能灰度中) */
  PartitionNumber?: number;
  /** 实例销毁保护标志，表示是否允许通过api接口删除实例。取值范围：true：表示开启实例保护，不允许通过api接口删除实例false：表示关闭实例保护，允许通过api接口删除实例默认取值：false。 */
  DisableApiTermination?: boolean;
  /** 实例是否开启巨型帧，取值范围：<li/> true：表示实例开启巨型帧，只有支持巨型帧的机型可设置为true。<li/>false：表示实例关闭巨型帧，只有支持巨型帧的机型可设置为false。 支持巨型帧的实例规格： 实例规格 */
...
declare interface RunInstancesResponse {
  /** 当通过本接口来创建实例时会返回该参数，表示一个或多个实例ID。返回实例ID列表并不代表实例创建成功，可根据 DescribeInstances 接口查询返回的InstancesSet中对应实例的ID的状态来判断创建是否完成；如果实例状态由“PENDING(创建中)”变为“RUNNING(运行中)”，则为创建成功。 */
  InstanceIdSet?: string[];
  /** 唯一请求 ID，每次请求都会返回。 */
  RequestId?: string;
}
...
declare interface StopInstancesRequest {
  ...
  /** 实例的关闭模式。取值范围：SOFT_FIRST：表示在正常关闭失败后进行强制关闭HARD：直接强制关闭SOFT：仅软关机默认取值：SOFT。 */
  StopType?: string;
  /** 按量计费实例关机收费模式。取值范围：KEEP_CHARGING：关机继续收费STOP_CHARGING：关机停止收费默认取值：KEEP_CHARGING。该参数只针对部分按量计费实例生效，详情参考[按量计费实例关机不收费说明](https://cloud.tencent.com/document/product/213/19918) */
  StoppedMode?: string;
}
...
declare interface SyncImagesRequest {
  /** 镜像ID列表 ，镜像ID可以通过如下方式获取：通过[DescribeImages](https://cloud.tencent.com/document/api/213/15715)接口返回的`ImageId`获取，镜像ID对应的镜像状态必须为`NORMAL`。通过[镜像控制台](https://console.cloud.tencent.com/cvm/image)获取。 */
  ImageIds: string[];
  /** 目的同步地域列表，必须满足如下限制：必须是一个合法的Region。如果是自定义镜像，则目标同步地域不能为源地域。如果是共享镜像，则目的同步地域仅支持源地域，表示将共享镜像复制为源地域的自定义镜像。如果是自定义镜像复制为加密自定义镜像，则目的同步地域仅支持源地域，表示将自定义镜像复制为源地域的加密自定义镜像。暂不支持部分地域同步，请参考[复制镜像](https://cloud.tencent.com/document/product/213/4943)。具体地域参数请参考[Region](https://cloud.tencent.com/document/product/213/6091)。 */
  DestinationRegions: string[];
  /** 检测是否支持发起同步镜像。默认值: false */
  DryRun?: boolean;
  /** 目标镜像名称。默认使用源镜像名称。最多支持 60 个字符。 */
  ImageName?: string;
  /** 是否需要返回目的地域的镜像ID。默认值: false */
  ImageSetRequired?: boolean;
  /** 是否复制为加密自定义镜像。默认值为 false。复制加密自定义镜像仅支持同地域， 即 DestinationRegions 仅支持填写指定镜像所在地域。 */
  Encrypt?: boolean;
  /** 加密自定义镜像使用的 KMS 密钥 ID。仅当复制加密镜像时，即 Encrypt 为 true 时，此参数有效；不指定 KmsKeyId，默认使用 CBS 云产品 KMS 密钥。KMS 密钥 ID 通过[KMS 控制台](https://console.cloud.tencent.com/kms2)获取。 */
  KmsKeyId?: string;
}
