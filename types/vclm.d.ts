...
declare interface SubmitImageToVideoViduJobRequest {
  ...
  Images: string[];
  /** 模型名称，可选值，默认值viduq2-turboviduq2-pro：新模型，效果好，细节丰富viduq2-turbo：新模型，效果好，生成快viduq3-turbo：对比viduq3-pro，生成速度更快viduq3-pro：高效生成优质音视频内容，让视频内容更生动、更形象、更立体，效果更好 */
  Model?: string;
  /** 文本提示词生成视频的文本描述。注1：字符长度不能超过 2000 个字符注2：若使用is_rec推荐提示词参数，模型将不考虑此参数所输入的提示词 */
  Prompt?: string;
  /** 是否使用推荐提示词，默认关闭-true：是，由系统自动推荐提示词，并使用提示词内容生成视频，推荐提示词数量=1-false：否，根据输入的prompt生成视频注意：启用推荐提示词后，每个任务多消耗1积分 */
  IsRec?: boolean;
  /** 【首帧生视频】是否使用音视频直出能力，默认关闭，枚举值为：false：不需要音视频直出，输出静音视频true：需要音视频直出，输出带台词以及背景音的视频注1：该参数为true时，voice_id参数才生效注2：该参数为true时，仅q3模型支持错峰注3：当model 为q3-pro或q3-turbo 时，该参数默认值为true【首尾帧生视频】false：不需要音视频直出，输出静音视频true：需要音画同步，输出声音的视频（包括台词和音效）注1：仅q3系列模型支持该参数 */
  Audio?: boolean;
  /** 音色id用来决定视频中的声音音色，为空时系统会自动推荐，可选枚举值参考列表：新音色列表暂不支持声音复刻功能 */
  VoiceId?: string;
  /** 视频时长参数：【首帧生视频】：viduq3-pro、viduq3-turboxa0默认为 5，可选：1 - 16viduq2-pro、viduq2-turboxa0默认为 5，可选：1 - 10【首尾帧生视频】：viduq3-proxa0、viduq3-turbo默认为 5，可选：1 - 16viduq2-pro、viduq2-turboxa0默认为 5，可选：1 - 8 */
  Duration?: number;
  /** 分辨率参数【首帧生视频】：viduq3-pro 、viduq3-turbo 1-16秒：默认 720p，可选：540p、720p、1080pviduq2-pro、viduq2-turbo 1-10秒：默认 720p，可选：720p、1080p示例值：720p【首尾帧生视频】：-xa0viduq3-proxa0、viduq3-turbo1-16秒：默认 720p，可选：540p、720p、1080p-xa0viduq2-proxa0、xa0viduq2-turbo1-8秒：默认 720p，可选：540p、720p、1080p */
  Resolution?: string;
  /** 运动幅度默认 auto，可选值：auto、small、medium、large注：q2、q3系列模型改参数不生效 */
...
declare interface SubmitImageToVideoViduJobResponse {
  /** 任务ID。 */
  JobId?: string;
  /** 唯一请求 ID，每次请求都会返回。 */
  RequestId?: string;
...
declare interface SubmitVideoEditJobRequest {
  ...
  VideoUrl: string;
  /** 视频内容的描述，中文正向提示词。支持视频内容增加、删除、修改等能力最多支持200个 utf-8 字符（首尾空格不计入字符数）不传prompt的时候，Images.N参考图列表必须要传图，且传的图片是经过图片编辑之后的结果图 */
  Prompt?: string;
  /** 参考图列表。用于对视频内容做风格迁移、内容替换、内容删减、内容增加做参考。支持传入图片Base64编码或图片URL图片格式：支持jpg，png，jpeg，webp，bmp，tiff 格式图片文件：大小不能超过10MB（base64后）。单边分辨率不超过5000px，不小于50px，图片长宽限制1:4 ~ 4:1。示例值：[{ &quot;Url&quot;: &quot;https://console.cloud.tencent.com/cos/image.png&quot;}] */
  Images?: Image[];
  /** 图片base64或者图片urlBase64 和 Url 必须提供一个，如果都提供以Url为准。上传图url大小不超过 8M支持jpg，png，jpeg，webp，bmp，tiff 格式单边分辨率不超过5000，不小于50，长宽限制1:4 ~ 4:1 */
  Image?: Image;
  /** 扩展字段。 */
