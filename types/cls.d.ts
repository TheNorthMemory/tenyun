...
declare interface DashboardNoticeMode {
  /** 仪表盘通知方式。- Uin：腾讯云用户- Group：腾讯云用户组- WeCom：企业微信回调- Email：自定义邮件- DingTalk：钉钉- Lark：飞书 */
  ReceiverType: string;
  /** 知方式对应的值。- 当ReceiverType为：`WeCom`、`DingTalk`、`Lark` 时，Values必须为空，且Url字段必填。- 当ReceiverType为：`Uin`、`Group`、`Email` 时，Values必填，且Url字段必须为空。- 当ReceiverType为：`Uin ` 时，Values为用户id，通过 [拉取子用户](https://cloud.tencent.com/document/product/598/34587) 获取子用户 UID 。- 当ReceiverType为：`Group` 时，Values为用户组id，通过 [查询用户组列表](https://cloud.tencent.com/document/product/598/34589) 获取用户组 ID 。- 当ReceiverType为：`Email` 时，Values为用户邮箱信息。 */
  Values?: string[];
  /** 仪表盘通知渠道。- 支持：["Email","Sms","WeChat","Phone"]。- 当ReceiverType为 `Email` 或 `WeCom` 时，ReceiverChannels无效。 */
  ReceiverChannels?: string[] | null;
  /** 订阅方式	- 回调地址。- 当ReceiverType为：`WeCom`、`DingTalk`、`Lark` 时，Url字段必填为各渠道的回调地址。 - 为：`WeCom` 时，Url为 企业微信回调地址。 - 为：`DingTalk` 时，Url为 钉钉机器人Webhook地址。 - 为：`Lark` 时，Url为 飞书机器人Webhook地址。- 当ReceiverType为：`Uin`、`Group`、`Email` 时，Url字段必须为空。 */
  Url?: string;
}
...
/** 仪表盘订阅信息 */
declare interface DashboardSubscribeInfo {
  /** 仪表盘订阅id。 */
  Id?: number;
  /** 仪表盘订阅名称。 */
  Name?: string;
...
declare interface DataTransformTaskInfo {
  ...
  UpdateTime?: string;
  /** 最后启用时间，如果需要重建集群，修改该时间时间格式：yyyy-MM-dd HH:mm:ss */
...
declare interface DeliverConfig {
  /** 地域信息。示例： ap-guangzhou 广州地域；ap-nanjing 南京地域。详细信息请查看官网[地域和访问域名](https://cloud.tencent.com/document/product/614/18940) */
  Region: string;
  /** 日志主题ID。-通过[获取日志主题列表](https://cloud.tencent.com/document/product/614/56454)获取日志主题ID */
  TopicId: string;
...
/** 跨账户投递外部角色 */
declare interface ExternalRole {
  /** 跨账户投递-用户角色RoleArn */
  RoleArn: string;
  /** 跨账户投递-用户角色名称 */
  ExternalId: string;
...
declare interface MonitorTime {
  /** 执行周期， 可选值：Period、Fixed、Cron。Period：固定频率Fixed：固定时间Cron：Cron表达式 */
  Type: string;
  /** 执行的周期，或者定制执行的时间节点。单位为分钟，取值范围为1~1440。当type为Period,Fixed时，time字段生效。 */
  Time?: number;
  /** 执行的周期cron表达式。示例：0/1 * * * * 从左到右每个field的含义 Minutes field(分钟), Hours field(小时),Day of month field(日期),Month field(月份),Day of week field(星期)， 不支持秒级别。当type为Cron时，CronExpression字段生效。 */
...
declare interface PreviewLogStatistic {
  ...
  /** 行号。从0开始 */
  LineNum: number;
  /** 目标日志主题ID- 通过[获取日志主题列表](https://cloud.tencent.com/document/product/614/56454)获取日志主题Id。 */
  DstTopicId?: string;
  /** 失败错误信息， 空字符串""表示正常 */
  FailReason?: string;
...
declare interface ShipperInfo {
  ...
  Progress?: number;
  /** 历史数据全部投递完成剩余的时间（仅当用户选择的数据中有历史数据时才有效）单位：秒 */
  RemainTime?: number;
...
declare interface WebCallback {
  /** 回调的类型。可选值：- Http(自定义接口回调)- WeCom(企业微信)- DingTalk(钉钉)- Lark(飞书) */
  CallbackType: string;
...
declare interface CreateCloudProductLogCollectionResponse {
  /** 日志主题ID */
  TopicId?: string;
  /** 日志主题名称 */
  TopicName?: string;
...
declare interface CreateConfigExtraRequest {
  ...
  TopicName: string;
  /** 自建k8s-节点文件配置信息,包括文件路径、名称及元数据相关信息。- 详细参考 [HostFileInfo](https://cloud.tencent.com/document/api/614/56471) 信息。 */
  HostFile?: HostFileInfo;
  /** 容器文件路径类型配置。 */
  ContainerFile?: ContainerFileInfo;
  /** 自建k8s-容器标准输出信息，包括容器、命名空间等。- 详细参考 [ContainerStdoutInfo](https://cloud.tencent.com/document/api/614/56471) 信息。 */
  ContainerStdout?: ContainerStdoutInfo;
  ...
  UserDefineRule?: string;
  /** 绑定的机器组ID- 通过[获取机器组列表](https://cloud.tencent.com/document/api/614/56438)获取机器组Id。- GroupId 与 GroupIds 选择其一即可，不可同时为空。 */
  GroupId?: string;
  /** 绑定的机器组ID列表- 通过[获取机器组列表](https://cloud.tencent.com/document/api/614/56438)获取机器组Id信息。- GroupId 与 GroupIds 选择其一即可，不可同时为空。 */
  GroupIds?: string[];
  /** 采集相关配置信息。详细参考 [CollectInfo](https://cloud.tencent.com/document/api/614/56471) 信息。 */
  CollectInfos?: CollectInfo[];
...
declare interface CreateMachineGroupRequest {
  /** 机器组名字。输入限制：- 最大支持255个字符，不能为空字符串- 不能包含字符'|' */
  GroupName: string;
...
declare interface CreateSplunkDeliverRequest {
  ...
  /** 日志预过滤-数据写入 Splunk 的原始数据进行预过滤处理 */
  DSLFilter?: string;
  /** 高级配置-跨账户投递用户角色授权信息取值参考：新建自定义角色 */
  ExternalRole?: ExternalRole;
}
...
declare interface DeleteCloudProductLogCollectionRequest {
  ...
  /** 是否删除关联的logset。如果logset下还有topic的话，不会进行删除 */
  IsDeleteLogset?: boolean;
}

declare interface DeleteCloudProductLogCollectionResponse {
...
declare interface DeleteCosRechargeRequest {
  /** COS导入配置Id。- 通过 [获取cos导入配置](https://cloud.tencent.com/document/product/614/88099) 获取COS导入配置Id。 */
  Id: string;
...
declare interface DeleteLogRequest {
  /** 日志主题id仅在创建日志主题时，开启了日志修改/删除开关的主题，支持日志修改/删除。该功能暂时仅面向白名单内客户使用。 */
  TopicId: string;
  /** 检索时间范围-开始时间单位：ms */
  From: number;
  /** 检索时间范围-结束时间单位：ms */
...
declare interface DescribeAlarmsRequest {
  /** name- 按照【告警策略名称】进行过滤。- 类型：String- 必选：否- 示例：test-alarmalarmId- 按照【告警策略ID】进行过滤。- 类型：String- 必选：否- 示例：alarm-b60cf034-c3d6-4b01-xxxx-4e877ebb4751topicId- 按照【监控对象的日志主题ID】进行过滤。- 类型：String- 必选：否- 示例：6766f83d-659e-xxxx-a8f7-9104a1012743enable- 按照【启用状态】进行过滤。- 类型：String- 备注：enable参数值范围: 1, t, T, TRUE, true, True, 0, f, F, FALSE, false, False。 其它值将返回参数错误信息.- 必选：否- 示例：true每次请求的Filters的上限为10，Filter.Values的上限为5。 */
  Filters?: Filter[];
  /** 分页的偏移量，默认值为0。 */
  Offset?: number;
  /** 分页单页限制数目，默认值为20，最大值100。 */
...
declare interface DescribeDashboardSubscribesRequest {
  /** dashboardId：按照【仪表盘id】进行过滤。类型：String必选：否- 仪表盘id。通过 [获取仪表盘](https://cloud.tencent.com/document/api/614/95636)接口获取DashboardId。- 入参示例：dashboard-522a5609-1f41-4b11-8086-5afd1d7574f5每次请求的Filters的上限为10，Filter.Values的上限为100。 */
  Filters?: Filter[];
  /** 分页的偏移量，默认值为0。 */
  Offset?: number;
  /** 分页单页限制数目，默认值为20，最大值100。 */
...
declare interface DescribeDashboardsRequest {
  ...
  /** 分页单页限制数目，默认值为20，最大值100。 */
  Limit?: number;
  /** - dashboardId 按照【仪表盘id】进行过滤，类型：String， 必选：否。 - 示例值：dashboard-522a5609-1f41-4b11-8086-5afd1d7574f5- dashboardName 按照【仪表盘名字】进行模糊搜索过滤，类型：String，必选：否。 - 示例值：业务大盘- dashboardRegion 按照【仪表盘地域】进行过滤（兼容老的仪表盘），通过云API创建的仪表盘该属性，类型：String，必选：否。 - 参考 [地域和访问域名](https://cloud.tencent.com/document/product/614/18940) - 示例：ap-guangzhou- tagKey 按照【标签键】进行过滤，类型：String，必选：否。 - 示例值： ``` "Filters":[ { "Key": "tagKey", "Values": [ "tag-key-test" ] } ] ```- tag:tagKey 按照【标签键值对】进行过滤。tagKey使用具体的标签键进行替换，类型：String，必选：否， - 参考 [示例1](https://cloud.tencent.com/document/api/614/95636) 使用。 ``` "Filters": [ { "Key": "tag:tag-key-test", "Values": [ "12" ] } ] ```每次请求的Filters的上限为10，Filter.Values的上限为100。 */
  Filters?: Filter[];
  /** 按照topicId和regionId过滤。- topicId:日志主题Id。 - 通过[获取日志主题列表](https://cloud.tencent.com/document/product/614/56454)获取日志主题Id。 - 示例值：439a5304-08f9-484b-9c4d-46ff57133816- regionId - 1:广州 - 4:上海 - 5:中国香港 - 7:上海金融 - 8:北京 - 9:新加坡 - 11:深圳金融 - 15:硅谷 - 16:成都 - 17:法兰克福 - 18:首尔 - 19:重庆 - 22:弗吉尼亚 - 23:曼谷 - 25:东京 - 33:南京 - 36:天津 - 39:台北 - 46:北京金融 - 72:雅加达 - 74:圣保罗 - 78:上海自动驾驶云 */
...
declare interface DescribeRebuildIndexTasksRequest {
  ...
  /** 索引重建任务ID */
  TaskId?: string;
  /** 索引重建任务状态，不填返回所有状态任务列表，多种状态之间用逗号分隔。枚举值：0： 索引重建任务已创建1： 正在创建索引重建资源2： 已创建索引重建资源3： 重建中4： 暂停5： 重建完成6： 重建成功（可检索）7： 重建失败8： 任务取消9： 元数据和索引已删除 */
  Status?: string;
  /** 分页的偏移量，默认值为0。 */
  Offset?: number;
...
declare interface DescribeSplunkDeliversRequest {
  /** 日志主题Id通过获取日志主题列表获取日志主题Id。 */
  TopicId: string;
  /** taskId 按照【任务id】进行过滤。 类型：String 必选：否 name 按照【任务名称】进行过滤。 类型：String 必选：否 statusFlag 按照【状态】进行过滤。 类型：String 必选：否每次请求的Filters的上限为10，Filter.Values的上限为10。 */
  Filters?: Filter[];
  /** 分页的偏移量，默认值为0。 */
  Offset?: number;
  /** 分页单页限制数目，默认值为20，最大值100。 */
...
declare interface DescribeWebCallbacksRequest {
  /** - name按照【告警渠道回调配置名称】进行过滤。类型：String必选：否- webCallbackId按照【告警渠道回调配置ID】进行过滤。类型：String必选：否- type按照【告警渠道回调配置渠道类型】进行过滤。类型：String必选：否枚举值：WeCom，DingTalk，Lark，Http每次请求的Filters的上限为10，Filter.Values的上限为100。 */
  Filters?: Filter[];
  /** 分页的偏移量，默认值为0。 */
  Offset?: number;
  /** 分页单页限制数目，默认值为20，最大值100。 */
...
declare interface GetAlarmLogRequest {
  ...
  To: number;
  /** 查询过滤条件，例如：按告警策略ID查询：alert_id:"alarm-0745ec00-e605-xxxx-b50b-54afe61fc971"通过获取告警策略列表获取告警策略ID按监控对象ID查询：monitored_object:"823d8bfa-76a7-xxxx-8399-8cda74d4009b"通过获取告警策略列表获取监控对象ID按告警策略ID及监控对象ID查询：alert_id:"alarm-0745ec00-e605-xxxx-b50b-54afe61fc971" AND monitored_object:"823d8bfa-76a7-xxxx-8399-8cda74d4009b"按告警策略ID及监控对象ID查询支持SQL语句：(alert_id:"alarm-5ce45495-0918-4d58-xxxx-768134bf330c") AND (monitored_object:"3c514e84-6f1f-46ec-xxxx-05de6163f7fe") AND NOT condition_evaluate_result: "Skip" AND condition_evaluate_result:[* TO *] | SELECT count(*) as top50StatisticsTotalCount, count_if(condition_evaluate_result='ProcessError') as top50StatisticsFailureCount, count_if(notification_send_result!='NotSend') as top50NoticeTotalCount, count_if(notification_send_result='SendPartFail' or notification_send_result='SendFail') as top50NoticeFailureCount, alert_id, alert_name, monitored_object, topic_type, happen_threshold, alert_threshold, notify_template group by alert_id, alert_name, monitored_object,topic_type, happen_threshold, alert_threshold, notify_template order by top50StatisticsTotalCount desc limit 1该查询语句走Lucene语法（已废弃） */
  Query?: string;
  /** 查询过滤条件，例如：- 按告警策略ID查询：alert_id:"alarm-0745ec00-e605-xxxx-b50b-54afe61fc971" - 通过获取告警策略列表获取告警策略ID- 按监控对象ID查询：monitored_object:"823d8bfa-76a7-xxxx-8399-8cda74d4009b" - 通过获取告警策略列表获取监控对象ID- 按告警策略ID及监控对象ID查询：alert_id:"alarm-0745ec00-e605-xxxx-b50b-54afe61fc971" AND monitored_object:"823d8bfa-76a7-xxxx-8399-8cda74d4009b"- 按告警策略ID及监控对象ID查询支持SQL语句：(alert_id:"alarm-5ce45495-0918-4d58-xxxx-768134bf330c") AND (monitored_object:"3c514e84-6f1f-46ec-xxxx-05de6163f7fe") AND NOT condition_evaluate_result: "Skip" AND condition_evaluate_result:[* TO *] | SELECT count(*) as top50StatisticsTotalCount, count_if(condition_evaluate_result='ProcessError') as top50StatisticsFailureCount, count_if(notification_send_result!='NotSend') as top50NoticeTotalCount, count_if(notification_send_result='SendPartFail' or notification_send_result='SendFail') as top50NoticeFailureCount, alert_id, alert_name, monitored_object, topic_type, happen_threshold, alert_threshold, notify_template group by alert_id, alert_name, monitored_object,topic_type, happen_threshold, alert_threshold, notify_template order by top50StatisticsTotalCount desc limit 1该查询语句走CQL语法 */
  QueryString?: string;
  /** 单次查询返回的执行详情条数，最大值为1000 */
  Limit?: number;
  /** 透传上次接口返回的Context值，可获取后续更多日志，总计最多可获取1万条原始日志，过期时间1小时。注意：透传该参数时，请勿修改除该参数外的其它参数仅当检索分析语句(Query)不包含SQL时有效，SQL获取后续结果参考SQL LIMIT语法 */
  Context?: string;
...
declare interface ModifyAlarmShieldRequest {
  ...
  /** 屏蔽原因。 */
  Reason?: string;
  /** 规则状态。只有规则状态为生效中（status:1）时，才能将其修改为已失效（status:2）。枚举：0（未生效），1（生效中），2（已失效） */
  Status?: number;
}
...
declare interface ModifyConfigExtraRequest {
  ...
  /** 日志主题id- 通过[获取日志主题列表](https://cloud.tencent.com/document/product/614/56454)获取日志主题Id。 */
  TopicId?: string;
  /** 自建k8s-节点文件配置信息,包括文件路径、名称及元数据相关信息。- 详情参考 [HostFileInfo](https://cloud.tencent.com/document/api/614/56471) 文档。 */
  HostFile?: HostFileInfo;
  /** 采集配置标记。- 目前只支持label_k8s，用于标记自建k8s集群使用的采集配置- 详情参考 [ ContainerFileInfo](https://cloud.tencent.com/document/api/614/56471) 文档 */
  ContainerFile?: ContainerFileInfo;
  /** 自建k8s-容器标准输出信息，包括容器、命名空间等，- 详情参考 [ContainerStdoutInfo]( https://cloud.tencent.com/document/api/614/56471) 文档 */
  ContainerStdout?: ContainerStdoutInfo;
  /** 采集的日志类型，默认为minimalist_log。支持以下类型：- json_log代表：JSON-文件日志（详见[使用 JSON 提取模式采集日志](https://cloud.tencent.com/document/product/614/17419)）；- delimiter_log代表：分隔符-文件日志（详见[使用分隔符提取模式采集日志](https://cloud.tencent.com/document/product/614/17420)）；- minimalist_log代表：单行全文-文件日志（详见[使用单行全文提取模式采集日志](https://cloud.tencent.com/document/product/614/17421)）；- fullregex_log代表：单行完全正则-文件日志（详见[使用单行-完全正则提取模式采集日志](https://cloud.tencent.com/document/product/614/52365)）；- multiline_log代表：多行全文-文件日志（详见[使用多行全文提取模式采集日志](https://cloud.tencent.com/document/product/614/17422)）；- multiline_fullregex_log代表：多行完全正则-文件日志（详见[使用多行-完全正则提取模式采集日志](https://cloud.tencent.com/document/product/614/52366)）；- user_define_log代表：组合解析（适用于多格式嵌套的日志，详见[使用组合解析提取模式采集日志](https://cloud.tencent.com/document/product/614/61310)）。 */
  LogType?: string;
  /** 日志格式化方式，用于容器采集场景。- stdout-docker-json：用于docker容器采集场景- stdout-containerd：用于containerd容器采集场景 */
...
declare interface ModifyIndexRequest {
  /** 日志主题Id。- 通过[获取日志主题列表](https://cloud.tencent.com/document/product/614/56454)获取日志主题Id。 */
  TopicId: string;
  /** 索引状态。false：关闭索引， true：开启索引开启后可对日志进行检索分析，将产生索引流量、索引存储及相应费用。[费用详情](https://cloud.tencent.com/document/product/614/45802) */
  Status?: boolean;
  /** 索引规则 */
  Rule?: RuleInfo;
  /** 内置保留字段（`__FILENAME__`，`__HOSTNAME__`及`__SOURCE__`）是否包含至全文索引，默认为false，推荐设置为true* false:不包含* true:包含 */
...
declare interface SplitPartitionRequest {
  /** 日志主题Id- 通过[获取日志主题列表](https://cloud.tencent.com/document/product/614/56454)获取日志主题Id。 */
  TopicId: string;
  /** 待分裂分区Id- 通过[获取分区列表](https://cloud.tencent.com/document/product/614/56470)获取待分裂分区Id。 */
  PartitionId: number;
