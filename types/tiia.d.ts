...
declare interface CreateGroupRequest {
  ...
  MaxQps?: number;
  /** 图库类型，对应不同的图像搜索服务类型，默认为4。1～3为历史版本，不推荐。参数取值：4：相同图像搜索。5：商品图像搜索。6：相似图像搜索。 */
  GroupType?: number;
}
...
declare interface CropImageRequest {
  ...
  Width: number;
  /** 需要裁剪区域的高度，与Width共同组成所需裁剪的图片宽高比例；输入数字请请大于0、小于图片高度的像素值；宽高比例（Width : Height）会简化为最简分数，即如果Width输入10、Height输入20，会简化为1：2。Width : Height建议取值在[1, 2.5]之间，超过这个范围可能会影响效果； */
  Height: number;
  /** 图片URL地址。 图片限制： • 图片格式：PNG、JPG、JPEG。 • 图片大小：所下载图片经Base64编码后不超过4M。图片下载时间不超过3秒。 建议：• 图片像素：大于50*50像素，否则影响识别效果； • 长宽比：长边：短边<5； 接口响应时间会受到图片下载时间的影响，建议使用更可靠的存储服务，推荐将图片存储在腾讯云COS。 */
  ImageUrl?: string;
  /** 图片经过base64编码的内容。最大不超过4M。与ImageUrl同时存在时优先使用ImageUrl字段。**注意：图片需要base64编码，并且要去掉编码头部。** */
...
declare interface SearchImageRequest {
  ...
  ImageBase64?: string;
  /** 返回结果的数量，默认值为10，最大值为100。按照相似度分数由高到低排序。**注意：服务类型为相似图像搜索时返回数量限制为1，即返回top1的结果。** */
  Limit?: number;
  /** 返回结果的起始序号，默认值为0。 */
  Offset?: number;
  /** 匹配阈值。只有图片相似度分数超过MatchThreshold值的结果才会返回。默认值：• 相同图像搜索：50。• 商品图像搜索：28。• 相似图像搜索：56。建议：可以手动调整MatchThreshold值来控制输出结果的范围。如果发现无检索结果，建议调整为较低的阈值。 */
