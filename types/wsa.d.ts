...
declare interface SearchProRequest {
  ...
  Site?: string;
  /** 该参数将于2026.8.17日正式下线起始时间（用于过滤自然检索结果），精确到秒时间戳格式注意： mode=1模式下，参数无效；mode=0模式下，对所有结果生效；mode=2模式下，对输出的自然结果生效 */
  FromTime?: number;
  /** 该参数将于2026.8.17日正式下线结束时间（用于过滤自然检索结果），精确到秒时间戳格式注意：mode=1模式下，参数无效；mode=0模式下，对所有结果生效；mode=2模式下，对输出的自然结果生效 */
  ToTime?: number;
  /** cnt=10/20/30/40/50，最多可支持返回50条搜索结果，仅限尊享版使用 */
  Cnt?: number;
  /** Industry=gov/news/acad/finance，对应党政机关、权威媒体、学术（英文）、金融，仅限尊享版使用 */
  Industry?: string;
  /** 搜索时效范围（仅旗舰版参数）d[N]：最近N天，N取值1-30整数。m[N]：最近N月，N取值1-12整数。y[N]：最近N年，N取值1-5整数。示例说明：d1/m1/y1：当天/当月/当年。例如，2026.6.15分别传参d1/m1/y1进行搜索，则搜索结果的时间范围分别为“2026.6.15”/“2026.6”/“2026”，以此类推。d/m/y：N值为空时，默认N=1，即等效入参d1/m1/y1。未传参时，默认不生效。枚举值：d7： 最近七天m3： 最近三月y2： 最近两年d： 当天m： 当月y： 当年 */
  Freshness?: string;
...
declare interface SearchProResponse {
  /** 原始查询语 */
  Query?: string;
  /** 搜索结果页面详情，格式为json字符串。title：结果标题date：内容发布时间url：内容发布源urlpassage：标准摘要content：动态摘要（仅尊享版、旗舰版返回该字段）site：网站名称，部分不知名站点结果可能为空score：相关性得分，取值0～1，越靠近1表示越相关authority_level：权威度得分，取值0～5，数值越大表示越权威pics：图片列表，单个doc返回0～10条（仅旗舰版参数）caption：图片描述origin_url：源图url地favicon：网站图标链接，部分不知名站点结果可能为空deeplinks：附件子链信息，单个doc最多返回10条子链信息。（仅旗舰版参数，通过Deeplinks入参控制）title：子链标题url：子链地址 */
  Pages?: string[];
