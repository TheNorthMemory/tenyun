...
declare interface SubmitAutoRiggingJobRequest {
  /** 人形角色需采用标准姿态（A Pose 或 T Pose），尽量保持无动作状态。模型中不应包含人体以外的组件（如武器、坐骑、翅膀等），避免松散衣物、配饰及复杂发型。非人形角色建议上传二足、四足、鸟类等单一生命体模型，姿态应尽量简洁，无夸张或复杂动作，且不支持动作模版生成。3D模型文件必选其一，参考值：FBX：3D模型文件，格式为FBX，文件大小不超过60Mb；GLB：3D模型文件，格式为GLB，文件大小不超过60Mb； */
  File3D: InputFile3D;
...
declare interface SubmitHunyuan3DPartJobRequest {
  ...
  File?: InputFile3D;
  /** 组件生成模型版本，默认为1.5枚举值：1.5： 1.5模型默认值：1.5 */
  Model?: string;
  /** 是否分步骤进行组件生成；开启后，输入原始模型后，可生成带分割信息的模型以及分割数据，可对此信息编辑后再传入带有分割信息的模型和分割数据进行编辑后的组件生成；此参数默认关闭。 */
  PartSegmentationInfo?: string;
