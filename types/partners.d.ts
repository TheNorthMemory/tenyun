...
declare interface AgentDealNewElem {
  ...
  /** 订单状态，中文描述 */
  DealStatus?: string;
  /** 子订单状态(1-待支付,2-已支付,3-发货中,4-已发货,5-发货失败,6-已退款,7-已取消,8-已过期,9-已失效,12-支付中,13-退款中,30-处理中)控制台订单状态为大订单状态，是以上状态的聚合：未支付(1) 处理中(2,3,5,12,13,30) 已取消(7) 交易成功(4) 已过期(8) 已退款(6) 订单错误(9) */
  Status?: string;
  /** 产品名称 */
  GoodsName?: string;
  /** 客户备注 */
...
declare interface DescribeAgentDealsByCacheRequest {
  ...
  /** 0:下单时间降序；其他：下单时间升序 */
  Order?: number;
  /** 子订单状态(1-待支付,2-已支付,3-发货中,4-已发货,5-发货失败,6-已退款,7-已取消,8-已过期,9-已失效,12-支付中,13-退款中,30-处理中)控制台订单状态为以上状态的组合：未支付(1) 处理中(2,3,5,12,13,30) 已取消(7) 交易成功(4) 已过期(8) 已退款(6) 订单错误(9) */
  Status?: number;
  /** 下单人账号ID列表 */
  OwnerUins?: string[];
  /** 子订单号列表 */
