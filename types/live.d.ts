...
declare interface DescribeLiveStreamOnlineListRequest {
  ...
  /** 取得第几页，默认1。 */
  PageNum?: number;
  /** 每页大小，最大300000。 取值：10~300000之间的任意整数。默认值：10。注意：尽量传入比当前总流数大的PageSize, 一次性拉走全部在线流，减少访问频次，推荐1分钟拉取一次。 */
  PageSize?: number;
  /** 流名称，用于精确查询。 */
  StreamName?: string;
}
...
declare interface DescribeLiveStreamPushInfoListRequest {
  ...
  /** 页数，范围[1,10000]，默认值：1。 */
  PageNum?: number;
  /** 每页个数，范围：[1,100000]，默认值： 200。注意： 1. 这里尽量PageSize填大于预估在线流数，一次性全部把在线流拉过去，减少分页查询次数。2. 该接口用于拉取在线流，减少高频调用，推荐每分钟查询一次，结合推断流在线回调来维护在线流状态。 */
  PageSize?: number;
}
...
declare interface InsertTaskTemporaryFilesRequest {
  ...
  /** 用于多个文件插播在同一个轮播序号之后时，可指定队列顺序。0 - 最高优先级队列。默认值。1 - 中等优先级队列。2 - 最低优先级队列。场景示例：比如当前播放第2个文件，多个新文件插播在第2个文件之后，播完第二个轮播文件之后，优先播放InsertPriorityType为0的插播队列中的文件，再播放InsertPriorityType为1的插播队列中的文件，最后播放InsertPriorityType为2的插播队列文件。如果插播无队列优先级要求，则无需使用此参数。 */
  InsertPriorityType?: number;
  /** 插播上下文信息。由用户自定义传入。当播放到使用本此插播请求插入的文件时，在文件播放开始和文件播放结束的回调时，将此上下文信息通过创建任务携带的回调地址回调给用户。注意：如果有关联每个插播文件的信息需要回调时，可将需要回调的信息自行拼接到插播文件URL上，通过回调信息中的URL进行读取使用。 */
  InsertContext?: string;
}

declare interface InsertTaskTemporaryFilesResponse {
  /** 唯一请求 ID，每次请求都会返回。 */
  RequestId?: string;
}
...
declare interface ModifyLivePullStreamTaskRequest {
  ...
  StartTime?: string;
  /** 结束时间，注意：1. 结束时间必须大于开始时间；2. 结束时间和开始时间必须大于当前时间；3. 结束时间 和 开始时间 间隔必须小于30天。使用UTC格式时间，例如：2019-01-08T10:00:00Z。注意：北京时间值为 UTC 时间值 + 8 小时，格式按照 ISO 8601 标准表示，详见 [ISO 日期格式说明](https://cloud.tencent.com/document/product/267/38543)。4. 只修改EndTime进行续期， 不会影响正在进行中的任务。 */
  EndTime?: string;
  /** 点播拉流转推循环次数。-1：无限循环，直到任务结束。0：不循环。>0：具体循环次数。次数和时间以先结束的为准。注意：拉流源为点播，该配置生效。 */
  VodLoopTimes?: number;
  /** 点播更新SourceUrls后的播放方式：ImmediateNewSource：立即从更新的拉流源开始播放；ContinueBreakPoint：从上次断流url源的断点处继续，结束后再使用新的拉流源。注意：拉流源为点播，该配置生效。 */
...
declare interface ModifyPullStreamConfigRequest {
  ...
  ConfigId: string;
  /** 源 URL，用于拉流的地址。目前可支持直播流及点播文件。注意：多个点播 URL 之间使用空格拼接。目前上限支持10个 URL。支持拉流文件格式：FLV，RTMP，HLS，MP4。使用标准三层样式，如：http://test.com/live/stream.flv。 */
  FromUrl?: string;
  /** 目的 URL，用于推流的地址，目前限制该目标地址为腾讯域名。仅支持 RTMP 协议。使用标准三层样式，如：http://test.com/live/stream.flv。 */
  ToUrl?: string;
  /** 区域 ID：1-深圳。2-上海。3-天津。4-中国香港。如有改动，需同时传入IspId。 */
  AreaId?: number;
  /** 运营商 ID，1：电信。2：移动。3：联通。4：其他。AreaId为4的时候，IspId只能为其他。如有改动，需同时传入AreaId。 */
  IspId?: number;
  /** 开始时间。使用UTC格式时间，例如：2019-01-08T10:00:00Z。注意：北京时间值为 UTC 时间值 + 8 小时，格式按照 ISO 8601 标准表示，详见 ISO 日期格式说明。 */
  StartTime?: string;
  /** 结束时间，注意：结束时间必须大于开始时间；结束时间和开始时间必须大于当前时间；结束时间 和 开始时间 间隔必须小于七天。使用UTC格式时间，例如：2019-01-08T10:00:00Z。注意：北京时间值为 UTC 时间值 + 8 小时，格式按照 ISO 8601 标准表示，详见 ISO 日期格式说明。 */
  EndTime?: string;
}
