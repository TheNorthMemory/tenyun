...
declare interface DescribeQAResponse {
  ...
  /** 更新时间 */
  UpdateTime?: string;
  /** 状态 1-未校验 2,4-导入完成 5-发布失败 6-不采纳 7-审核中 8-审核失败 9-审核失败申诉后人工审核中 11-审核失败申诉后人工审核不通过 12-已过期 13-超量失效 14-超量失效恢复 19-学习中 20-学习失败 */
  Status?: number;
  /** 状态描述 */
  StatusDesc?: string;
  /** 分类ID */
...
declare interface ListQARequest {
  ...
  AcceptStatus?: number[];
  /** 发布状态(2,4导入成功 7审核中 8审核失败 9人工申述中 11人工申述失败 12已过期 13超量失效 14超量失效恢复)如果不填默认值为空数组，表示不筛选返回所有状态 */
  ReleaseStatus?: number[];
  /** 文档ID */
  DocBizId?: string;
