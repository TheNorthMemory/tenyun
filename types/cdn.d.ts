...
declare interface AvifAdapter {
  ...
  Switch?: string | null;
  /** 当原图是 avif 且客户端 Accept 头包含 image/avif 时，直接返回原图。当原图是 avif 且客户端 Accept 头不包含 image/avif 时但包含 image/webp，将 avif 转 webp 格式返回。如果 Accept 头不包含 image/webp, 则转 jpeg 返回。可用的枚举值： - []- ["webp"]- ["jpeg"]- ["webp", "jpeg"]"webp"：是否开启 avif 转 webp，"jpeg": 是否开启 avif 转 jpeg。如果 webp 和 jpeg 都开启的情况下，webp 必须在 jpeg 前面。 */
...
declare interface CacheConfigCache {
  ...
  /** 当源站返回Set-Cookie头部时，节点是否缓存该头部及bodyon：开启，不缓存该头部及bodyoff：关闭，遵循用户自定义的节点缓存规则默认为关闭状态 */
  IgnoreSetCookie: string | null;
  /** 当缓存过期后，是否开启源站 mtime 校验，配置值为equal、since、none 和 null。默认配置值为equal，会校验源站文件的mtime与长度。2024-09-12 18:00 之前创建的域名默认值 null，行为保持不变。equal：源站响应mtime必须和缓存mtime一致，若mtime值不一致，清除缓存。since：若源站响应mtime大于缓存mtime，清除缓存。none： 缓存过期回源重新获取文件mtime和长度后，不会校验源站响应mtime，若源站响应携带Content-Length头部，只有文件大小改变时才会更新缓存；若源站响应不携带Content-Length头部，会更新缓存。 */
  OriginMtimeCheckType?: string | null;
}
...
declare interface IpFilter {
  ...
  /** IP 黑白名单验证失败时返回的状态码。注意：请求拒绝时，平台默认响应514状态。支持自定义为403，404，609状态码，空值时或自定义的不在范围内，均默认为514.非514状态码将计入HTTPS计费统计，最终账单将按您的计费规则生成。若您开启了自定义状态码，则默认您认同HTTPS计费规则。 */
  ReturnCode?: number | null;
}
...
declare interface DescribeDistrictIspDataRequest {
  ...
  Metric: string;
  /** 指定省份/国家查询，最多支持指定10个省份/国家（注：指定条件查询只支持映射表范围）省份/国家编码可以查看 省份编码映射指定中国境内查询：不填充表示查询所有省份 */
  Districts?: number[];
  /** 指定运营商查询，只支持中国境内，不填充表示查询所有运营商（注：指定条件查询只支持映射表范围）运营商编码可以查看 运营商编码映射 */
  Isps?: number[];
  /** 指定协议查询，不填充表示查询所有协议all：所有协议http：指定查询 HTTP 对应指标https：指定查询 HTTPS 对应指标 */
  Protocol?: string;
  /** 指定IP协议查询，不填充表示查询所有协议all：所有协议ipv4：指定查询 ipv4 对应指标ipv6：指定查询 ipv6 对应指标 */
  IpProtocol?: string;
