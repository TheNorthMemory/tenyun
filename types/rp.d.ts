...
declare interface QueryRegisterProtectionRequest {
  /** 注册来源的外网 IP。 */
  RegisterIp: string;
  ...
  CookieHash?: string;
  /** 注册来源：0：其他。1：PC 网页。2：移动页面。3：App。4：微信公众号。 */
  ...
  /** 注册结果：0：失败。1：成功。 */
  Result?: string;
  /** 失败原因：0：其他。1：参数错误。2：帐号冲突。3：验证错误。 */
  Reason?: string;
  /** 登录耗时，单位：秒。 */
  ...
  RandNum?: string;
  /** 如果是微信小程序，该字段为以 ssesion_key 为 key 去签名随机数 radnNum 得到的值（hmac_sha256签名算法）。如果是微信公众号或第三方登录，则为授权的 access_token（注意：不是普通 access_token，具体看 微信官方文档）。 */
  WxToken?: string;
}

declare interface QueryRegisterProtectionResponse {
