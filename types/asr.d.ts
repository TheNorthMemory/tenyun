...
declare interface CreateAsrKeyWordLibRequest {
  ...
  Name: string;
  /** 词文件（纯文本文件）的二进制base64编码，以行分隔格式要求：TXT每行只有一个词，不满足格式则报错无法上传每个词限制5个汉字，15个字符，单个词库最多不超过100个词注意不要有空行，尤其是最后一行 */
  KeyWordFile: string;
}
...
declare interface CreateAsrVocabRequest {
  /** 热词表名称，长度在1-255之间, 注意: 仅允许中英文、数字、下划线、中英文标点、空格、横线 */
  Name: string;
  /** 热词表描述，长度在0-1000之间 */
  Description?: string;
...
declare interface CreateAsyncRecognitionTaskRequest {
  /** 引擎模型类型。• 16k_zh：中文普通话通用；• 16k_en：英语；• 16k_id：印度尼西亚语；• 16k_fil：菲律宾语；• 16k_th：泰语；• 16k_pt：葡萄牙语；• 16k_tr：土耳其语；• 16k_ar：阿拉伯语；• 16k_es：西班牙语；• 16k_hi：印地语；• 16k_fr：法语；• 16k_de：德语； */
  EngineType: string;
...
declare interface CreateRecTaskRequest {
  /** 引擎模型类型识别引擎采用分级计费方案，分为通用引擎、大模型1.0版引擎（原大模型版引擎）、大模型2.0版引擎。点击这里 查看不同引擎的计费说明。大模型2.0引擎：• 16k_zh_en_2.0：中英大模型引擎【大模型2.0版】。当前模型同时支持中文、英语、粤语、四川、陕西、河南、上海、湖南、湖北、安徽、闽南、潮汕等 31种方言 的识别，且支持说话人分离能力。该模型针对噪声大、回音大、人声小、人声远等低质量音频的识别准确率极大提升，点击这里对比中文普通话常规版本与中英大模型版本的识别效果；• 16k_zh_en_meeting：中英大模型引擎【大模型2.0版】。当前模型同时支持中文、英语、粤语、四川、陕西、河南、上海、湖南、湖北、安徽、闽南、潮汕等 31种方言 的识别，适用于多人对话场景，使用时请开启说话人分离。该模型在多说话人重叠发声场景下，转写准确率与说话人分离准确率均表现突出，点击这里对比中文普通话常规版本与中英大模型版本的识别效果；大模型1.0引擎：• 8k_zh_large：中文电话场景专用大模型引擎【大模型1.0版】。当前模型同时支持中文、上海话、四川话、武汉话、贵阳话、昆明话、西安话、郑州话、太原话、兰州话、银川话、西宁话、南京话、合肥话、南昌话、长沙话、苏州话、杭州话、济南话、天津话、石家庄话、黑龙江话、吉林话、辽宁话、闽南语、客家话、粤语、南宁话方言识别，通过显著提升模型参数规模与语言建模能力，实现对电话音频中复杂场景（如口音干扰、背景噪声）的高精度识别，识别准确率较常规版本大幅提升。点击这里 对比常规版本与普方大模型效果；• 16k_zh_en：中英大模型引擎【大模型1.0版】。当前模型同时支持中文、英语、粤语、四川、陕西、河南、上海、湖南、湖北、安徽、闽南、潮汕等 31种方言 的识别，针对噪声大、回音大、人声小、人声远等低质量音频的识别准确率极大提升，点击这里对比中文普通话常规版本与中英大模型版本的识别效果；• 16k_multi_lang：多语种大模型引擎【大模型1.0版】。当前模型同时支持英语、日语、韩语、阿拉伯语、菲律宾语、法语、印地语、印尼语、马来语、葡萄牙语、西班牙语、泰语、土耳其语、越南语、德语的识别，可实现15个语种的自动识别(句子/段落级别)；• 16k_en_large：英语大模型引擎。通用引擎：• 8k_zh：中文电话通讯；• 8k_en：英文电话通讯；注意：电话通讯场景请务必使用8k引擎，电话通讯场景以外的其它识别场景，请务必使用16k引擎。如您有电话通讯场景识别需求，但发现需求语种仅支持16k，可将8k音频传入下方16k引擎，亦能获取识别结果。但16k引擎并非基于电话通讯数据训练，无法承诺此种调用方式的识别效果，需由您自行验证识别结果是否可用。• 16k_zh：中文普通话通用引擎，支持中文普通话和少量英语，使用丰富的中文普通话语料训练，覆盖场景广泛，适用于除电话通讯外的所有中文普通话识别场景；• 16k_en：英语；• 16k_yue：粤语；• 16k_zh-PY：中英粤混合引擎，使用一个引擎同时识别中文普通话、英语、粤语三个语言;• 16k_zh-TW：中文繁体；• 16k_ja：日语；• 16k_ko：韩语；• 16k_vi：越南语；• 16k_ms：马来语；• 16k_id：印度尼西亚语；• 16k_fil：菲律宾语；• 16k_th：泰语；• 16k_pt：葡萄牙语；• 16k_tr：土耳其语；• 16k_ar：阿拉伯语；• 16k_es：西班牙语；• 16k_hi：印地语；• 16k_fr：法语；• 16k_zh_medical：中文医疗引擎；• 16k_de：德语；枚举值：16k_zh_en_2.0： 中文普通话通用引擎，支持中文普通话和少量英语，使用丰富的中文普通话语料训练，覆盖场景广泛，适用于除电话通讯外的所有中文普通话识别场景 */
  EngineModelType: string;
  /** 识别声道数1：单声道（16k音频仅支持单声道，请勿设置为双声道）；2：双声道（仅支持8k电话音频，且双声道应分别为通话双方）注意：• 16k音频：仅支持单声道识别，需设置ChannelNum=1；• 8k电话音频：支持单声道、双声道识别，建议设置ChannelNum=2，即双声道。双声道能够物理区分说话人、避免说话双方重叠产生的识别错误，能达到最好的说话人分离效果和识别效果。设置双声道后，将自动区分说话人，因此无需再开启说话人分离功能，相关参数（SpeakerDiarization、SpeakerNumber）使用默认值即可，返回的ResultDetail中的speakerId的值为0代表左声道，值为1代表右声道。 */
  ChannelNum: number;
  /** 识别结果返回样式0：基础识别结果（仅包含有效人声时间戳，无词粒度的详细识别结果）；1：基础识别结果之上，增加词粒度的详细识别结果（包含词级别时间戳、语速值，不含标点）；2：基础识别结果之上，增加词粒度的详细识别结果（包含词级别时间戳、语速值和标点）；3：基础识别结果之上，增加词粒度的详细识别结果（包含词级别时间戳、语速值和标点），且识别结果按标点符号分段，适用字幕场景；4：【增值付费功能】基础识别结果之上，增加词粒度的详细识别结果（包含词级别时间戳、语速值和标点），且识别结果按nlp语义分段，适用会议、庭审记录转写等场景，仅支持8k_zh/16k_zh引擎5：【增值付费功能】基础识别结果之上，增加词粒度的详细识别结果（包含词级别时间戳、语速值和标点），并输出口语转书面语转写结果，该结果去除语气词、重复词、精简冗余表达，并修正发言人口误，实现口语转书面语的效果，适用于线上、线下会议直接总结为书面会议纪要的场景，仅支持8k_zh/16k_zh引擎注意：以下引擎仅支持参数值设置为0：16k_multi_lang、16k_ja、16k_ko、16k_vi、16k_ms、16k_id、16k_fil、16k_th、16k_pt、16k_tr、16k_ar、16k_es、16k_hi、16k_fr、16k_zh_medical、16k_de如果传入参数值4，需确保账号已购买语义分段资源包，或账号开启后付费；若当前账号已开启后付费功能，并传入参数值4，将自动计费如果传入参数值5，需确保账号已购买口语转书面语资源包，或账号开启后付费；若当前账号已开启后付费功能，并传入参数值5，将自动计费自动计费 */
  ResTextFormat: number;
  /** 音频数据来源0：音频URL；1：音频数据（post body） */
  SourceType: number;
  /** 音频数据base64编码当 SourceType 值为 1 时须填写该字段，为 0 时不需要填写注意：音频数据要小于5MB（含） */
  Data?: string;
  /** 数据长度（此数据长度为数据未进行base64编码时的长度） */
  DataLen?: number;
  /** 音频URL的地址（需要公网环境浏览器可下载）当 SourceType 值为 0 时须填写该字段，为 1 时不需要填写注意：请确保录音文件时长在5个小时（含）之内，否则可能识别失败；请保证文件的下载速度，否则可能下载失败 */
  Url?: string;
  /** 回调 URL用户自行搭建的用于接收识别结果的服务URL回调格式和内容详见：录音识别回调说明注意：如果用户使用轮询方式获取识别结果，则无需提交该参数建议在回调URL中带上您的业务ID等信息，以便处理业务逻辑 */
  CallbackUrl?: string;
  /** 是否开启说话人分离0：不开启；1：开启（仅支持以下引擎：8k_zh/8k_zh_large/16k_zh_en_2.0/16k_zh_en_meeting/16k_zh_en/16k_zh/16k_ms/16k_en/16k_id/16k_zh_dialect/16k_es/16k_fr/16k_ja/16k_ko，且ChannelNum=1时可用）；3: 开启角色分离，需配合SpeakerRoles参数使用（增值服务，仅支持16k_zh_en_2.0、16k_zh_en引擎，可支持传入声纹对录音文件内的说话人进行角色认证）默认值为 0注意：8k双声道电话音频请按 ChannelNum 识别声道数 的参数描述使用默认值。 */
  SpeakerDiarization?: number;
  /** 说话人分离人数需配合开启说话人分离使用，不开启无效，取值范围：0-100：自动分离（最多分离出20个人）；1-10：指定人数分离；默认值为 0注:16k引擎不支持指定人数分离 */
  SpeakerNumber?: number;
  /** 热词表id如不设置该参数，将自动生效默认热词表；如设置该参数，将生效对应id的热词表；点击这里查看热词表配置方法 */
  HotwordId?: string;
  /** 热词增强功能（目前仅支持中文普通话引擎）1：开启热词增强功能注意：热词增强功能开启后，将对传入的热词表id开启同音替换功能，可以在这里查看热词表配置方法。效果举例：在热词表中配置“蜜制”一词，并开启增强功能，与“蜜制”（mìzhì）同音同调的“秘制”（mìzhì）的识别结果会被强制替换成“蜜制”。建议客户根据实际的业务需求开启该功能 */
  ReinforceHotword?: number;
  /** 自学习定制模型 id如设置了该参数，将生效对应id的自学习定制模型；点击这里查看自学习定制模型配置方法 */
  CustomizationId?: string;
  /** 【增值付费功能】情绪识别能力（目前仅支持 16k_zh , 16k_zh_en_2.0 , 16k_zh_en , 8k_zh ）0：不开启；1：开启情绪识别，但不在文本展示情绪标签；2：开启情绪识别，并且在文本展示情绪标签（该功能需要设置ResTextFormat 大于0）默认值为0支持的情绪分类为：高兴、伤心、愤怒注意：1. 本功能为增值服务，需将参数设置为1或2时方可按对应方式生效；2. 如果传入参数值1或2，需确保账号已购买情绪识别资源包，或账号开启后付费；若当前账号已开启后付费功能，并传入参数值1或2，将自动计费）；3. 参数设置为0时，无需购买资源包，也不会消耗情绪识别对应资源 */
  ...
  /** 传入额外参数，默认为空字符串，如果传入必须是合法的 JSON 字符串。可用字段有：Domain：提升特定领域识别效果0：不开启1：科技领域2：电影领域3：歌曲领域默认值为0示例： {"Domain":3}增强歌曲领域识别效果 */
  Extra?: string;
...
declare interface UpdateAsrKeyWordLibRequest {
  ...
  /** 词表名称，长度在1-20之间仅限中英文数字-_ */
  Name: string;
  /** 词文件（纯文本文件）以行分隔 ，进行二进制base64编码格式要求：TXT 每行只有一个词，不满足格式则报错无法上传 每个词最多5个汉字或15个字符，单个词库最多不超过100个词此参数为空则只更新词表名称 */
  KeyWordFile: string;
}
