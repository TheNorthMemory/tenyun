...
declare interface CreateRecTaskRequest {
  ...
  ChannelNum: number;
  /** 识别结果返回形式0： 识别结果文本(含分段时间戳)；1：词级别粒度的[详细识别结果](https://cloud.tencent.com/document/api/1093/37824)(不含标点，含语速值)；2：词级别粒度的详细识别结果（包含标点、语速值）；3: 标点符号分段，包含每段时间戳，特别适用于字幕场景（包含词级时间、标点、语速值）4：【增值付费功能】对识别结果按照语义分段，并展示词级别粒度的详细识别结果，仅支持8k_zh、16k_zh引擎，需购买对应资源包使用（注意：如果账号后付费功能开启并使用此功能，将[自动计费](https://cloud.tencent.com/document/product/1093/35686)） */
  ResTextFormat: number;
  /** 语音数据来源0：语音 URL；1：语音数据（post body） */
  SourceType: number;
  /** 是否开启说话人分离0：不开启；1：开启（仅支持8k_zh/16k_zh，且ChannelNum=1时可用）；默认值为 0注意：8k双声道电话音频请按 **ChannelNum 识别声道数** 的参数描述使用默认值 */
  SpeakerDiarization?: number;
  /** 说话人分离人数**需配合开启说话人分离使用，不开启无效**，取值范围：0-100：自动分离（最多分离出20个人）；1-10：指定人数分离；默认值为 0注意：此功能结果仅供参考 */
  SpeakerNumber?: number;
  /** 回调 URL：用户自行搭建的用于接收识别结果的服务URL。回调格式和内容详见：[录音识别回调说明](https://cloud.tencent.com/document/product/1093/52632)如果用户使用轮询方式获取识别结果，则无需提交该参数。 */
  CallbackUrl?: string;
