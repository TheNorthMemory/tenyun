...
declare interface CreateRecTaskRequest {
  ...
  CallbackUrl?: string;
  /** 是否开启说话人分离0：不开启；1：开启（仅支持以下引擎：8k_zh/8k_zh_large/16k_zh_en_2.0/16k_zh_en/16k_zh/16k_ms/16k_en/16k_id/16k_zh_dialect/16k_es/16k_fr/16k_ja/16k_ko，且ChannelNum=1时可用）；3: 开启角色分离，需配合SpeakerRoles参数使用（增值服务，仅支持16k_zh_en_2.0、16k_zh_en引擎，可支持传入声纹对录音文件内的说话人进行角色认证）默认值为 0注意：8k双声道电话音频请按 ChannelNum 识别声道数 的参数描述使用默认值 */
